...
from differential_evolution import DifferentialEvolution  # Using DE instead of ANS
import time
...
# Run Differential Evolution on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    de = DifferentialEvolution(objective_function=func, bounds=bounds, max_generations=100, pop_size=50)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = de.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"DifferentialEvolution on {name} Function")
    plt.legend()
    plt.savefig(f"DE_{name}.png")
