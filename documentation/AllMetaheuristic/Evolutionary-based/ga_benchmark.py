...
from genetic_algorithm import GeneticAlgorithm  # Ensure this is correctly implemented
import time
...
# Run GeneticAlgorithm on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    ga = GeneticAlgorithm(objective_function=func, dim=2, bounds=bounds, num_pop=20, num_iter=100, mode='min')

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = ga.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"GeneticAlgorithm on {name} Function")
    plt.legend()
    plt.savefig(f"GA_{name}.png")
