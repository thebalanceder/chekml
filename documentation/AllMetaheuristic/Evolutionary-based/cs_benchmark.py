...
from clonal_selection import ClonalSelectionAlgorithm  # 👈 Import your CSA model
import time
...
# Run CSA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    csa = ClonalSelectionAlgorithm(objective_function=func, dim=2, bounds=bounds)  # 👈 Replaced BSA with CSA
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"CSA on {name} Function")
    plt.legend()
    plt.savefig(f"CSA_{name}.png")
