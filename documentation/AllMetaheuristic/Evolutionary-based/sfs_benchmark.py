...
from stochastic_fractal_search import StochasticFractalSearch  # Ensure this is correctly implemented
import time
...
# Run StochasticFractalSearch on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    sfs = StochasticFractalSearch(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = sfs.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Stochastic Fractal Search on {name} Function")
    plt.legend()
    plt.savefig(f"SFS_{name}.png")
