...
from partial_reinforcement_optimizer import PartialReinforcementOptimizer
import time
...
# Run PRO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    pro = PartialReinforcementOptimizer(objective_function=func, dim=2, bounds=bounds, 
                                       population_size=30, max_evaluations=100, reinforcement_rate=0.7)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = pro.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"PRO on {name} Function")
    plt.legend()
    plt.savefig(f"PRO_{name}.png")
