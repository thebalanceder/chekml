...
from stochastic_paint_optimizer import StochasticPaintOptimizer
...
# Run Stochastic Paint Optimizer Algorithm
spo = StochasticPaintOptimizer(objective_function=rastrigin_function, dim=2, bounds=bounds)
best_solution, best_value, history = spo.optimize()
...
plt.xlabel("X1")
plt.ylabel("X2")
plt.title("Stochastic Paint Optimizer on Rastrigin Function")
plt.legend()
plt.savefig("SPO.png")
