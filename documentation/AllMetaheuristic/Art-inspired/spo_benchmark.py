...
from stochastic_paint_optimizer import StochasticPaintOptimizer  # Replace with the actual filename (without .py)
import time
...
# Run SPO on each benchmark function
for name, func in benchmark_functions.items():
    ...
    spo = StochasticPaintOptimizer(
        ...
        lb=lb,
        ub=ub,
        problem_size=2,  # Dimensionality of the problem
        batch_size=50,
        epoch=100
    )
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"SPO on {name} Function")
    plt.legend()
    plt.savefig(f"SPO_{name}.png")
