...
import time
from iterative_local_search import IterativeLocalSearch  # Make sure this matches the actual filename
...
# Run ILS on each benchmark function
for name, func in benchmark_functions.items():
    ...
    best_solution, best_value, history = ils.optimize()
    end_time = time.time()
    execution_time = end_time - start_time
    execution_times[name] = execution_time

    # Extract search path for plotting
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Iterated Local Search on {name} Function")
    plt.legend()
    plt.savefig(f"ILS_{name}.png")
