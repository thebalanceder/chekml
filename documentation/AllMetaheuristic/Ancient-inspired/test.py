...
from dujiangyan_optimizer import DujiangyanOptimizer
import time

# Define benchmark functions (same as before)
...
bounds = [(-5.0, 5.0), (-5.0, 5.0)]  # Must use float values
...
# Run DISO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    diso = DujiangyanOptimizer(dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value = diso.optimize(func)  # Pass Python function directly
    end_time = time.time()
