...
from great_wall_construction_optimizer import GreatWallConstructionOptimizer
...
# Run Great Wall Construction Algorithm
gwca = GreatWallConstructionOptimizer(objective_function=rastrigin_function, dim=2, bounds=bounds)
...
# Plot search path
plt.plot(x_vals, y_vals, marker="o", markersize=3, color="brown", linestyle="dashed", alpha=0.7, label="Search Path")
plt.scatter(best_solution[0], best_solution[1], color="white", marker="*", s=100, label="Best Solution")

plt.xlabel("X1")
plt.ylabel("X2")
plt.title("Great Wall Construction Algorithm on Rastrigin Function")
plt.legend()
plt.savefig("GWCA.png")
