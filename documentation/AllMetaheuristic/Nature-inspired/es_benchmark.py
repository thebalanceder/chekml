...
from eagle_strategy import EagleStrategyOptimizer
import time
...
# Run Eagle Strategy on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    optimizer = EagleStrategyOptimizer(
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Eagle Strategy on {name} Function")
    plt.legend()
    plt.savefig(f"EagleStrategy_{name}.png")
