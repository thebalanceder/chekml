...
from political_optimizer import PoliticalOptimizer
import time
...
# Run Political Optimizer on each benchmark function
for name, func in benchmark_functions.items():
    ...
    # Extract search history (best solution positions at each iteration)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Political Optimizer on {name} Function")
    plt.legend()
    plt.savefig(f"PO_{name}.png")
