...
from cuckoo_optimization import CuckooOptimizer
import time
...
# Run CO on each benchmark function
for name, func in benchmark_functions.items():
    ...
    coa = CuckooOptimizer(
        ...
        min_eggs=2,
        max_eggs=4,
        max_cuckoos=15,  # Adjusted for balance
        radius_coeff=5.0,
        motion_coeff=9.0,
        knn_cluster_num=2,  # Adjusted for better clustering
        variance_threshold=1e-13
    )
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"CO on {name} Function")
    plt.legend()
    plt.savefig(f"CO_{name}.png")
