...
from monkey_search_optimization import MonkeySearchOptimizer
import time
...
# Run MSA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    msa = MonkeySearchOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_fitness, history = msa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"MSA on {name} Function")
    plt.legend()
    plt.savefig(f"MSA_{name}.png")
