...
from jaguar_algorithm import JaguarAlgorithmOptimizer
import time
...
# Run Jaguar Algorithm on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    ja = JaguarAlgorithmOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = ja.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Jaguar Algorithm on {name} Function")
    plt.legend()
    plt.savefig(f"JA_{name}.png")
