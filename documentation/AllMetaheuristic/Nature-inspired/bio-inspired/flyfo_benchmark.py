...
from flying_fox_optimization import FlyingFoxOptimizer
import time
...
# Run FFO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    ffo = FlyingFoxOptimizer(objective_function=func, dim=2, bounds=bounds, max_evals=100)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = ffo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"FlyFO on {name} Function")
    plt.legend()
    plt.savefig(f"FlyFO_{name}.png")
