...
from ant_lion_optimizer import AntLionOptimization
import time
...
# Run ALO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    diso = AntLionOptimization(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"ALO on {name} Function")
    plt.legend()
    plt.savefig(f"ALO_{name}.png")
