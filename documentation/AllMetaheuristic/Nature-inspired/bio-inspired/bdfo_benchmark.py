...
from bottlenose_dolphin_foraging_optimizer import BottlenoseDolphinForagingOptimizer
import time
...
# Run BottlenoseDolphinForagingOptimizer on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    optimizer = BottlenoseDolphinForagingOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Bottlenose Dolphin Optimizer on {name} Function")
    plt.legend()
    plt.savefig(f"Bottlenose_{name}.png")
    plt.close()
