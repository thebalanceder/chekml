...
from krill_herd_algorithm import KrillHerdOptimizer
import time
...
# Run Krill Herd Optimizer on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    kho = KrillHerdOptimizer(
    ...
    best_solution, best_value, history = kho.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Krill Herd Optimizer on {name} Function")
    plt.legend()
    plt.savefig(f"KHO_{name}.png")
