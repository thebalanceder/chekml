...
import time
from red_deer_algorithm import RedDeerAlgorithm
...
# Run RDA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    rda = RedDeerAlgorithm(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = rda.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"RDA on {name} Function")
    plt.legend()
    plt.savefig(f"RDA_{name}.png")
