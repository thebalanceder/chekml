...
from keshtel_algorithm import KeshtelOptimizer
import time
...
# Run Keshtel on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    keshtel = KeshtelOptimizer(
    ...
    best_solution, best_value, history = keshtel.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Keshtel Algorithm on {name} Function")
    plt.legend()
    plt.savefig(f"Keshtel_{name}.png")
