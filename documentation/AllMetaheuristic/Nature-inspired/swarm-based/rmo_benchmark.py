...
from radial_movement_optimization import RadialMovementOptimizer
import time
...
# Run RMO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    rmo = RadialMovementOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"RMO on {name} Function")
    plt.legend()
    plt.savefig(f"RMO_{name}.png")
