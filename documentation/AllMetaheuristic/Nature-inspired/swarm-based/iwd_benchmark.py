...
from intelligent_water_drops import IntelligentWaterDropOptimizer
import time
...
# Run IWD on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    iwd = IntelligentWaterDropOptimizer(
    ...
    best_solution, best_value, history = iwd.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"IWD on {name} Function")
    plt.legend()
    plt.savefig(f"IWD_{name}.png")
