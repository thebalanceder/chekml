...
from locust_swarm_algorithm import LocustSwarmOptimizer
import time
...
# Run Locust Swarm Algorithm on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    lso = LocustSwarmOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = lso.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Locust Swarm Algorithm on {name} Function")
    plt.legend()
    plt.savefig(f"LSO_{name}.png")
