...
from hunter_prey_optimizer import HunterPreyOptimizer
import time
...
# Run HPO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    hpo = HunterPreyOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, convergence_curve, history = hpo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"HPO on {name} Function")
    plt.legend()
    plt.savefig(f"HPO_{name}.png")
