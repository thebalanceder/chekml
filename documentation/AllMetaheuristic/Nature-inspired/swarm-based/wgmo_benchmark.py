...
from wild_geese_migration_optimization import WildGeeseMigrationOptimizer
import time
...
# Run Wild Geese Migration Optimization on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    wgo = WildGeeseMigrationOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_fitness, history = wgo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"WGO on {name} Function")
    plt.legend()
    plt.savefig(f"WGO_{name}.png")
