...
from locust_swarm_algorithm import LocustSwarmAlgorithm
...
# Run Locust Swarm Algorithm
lsa = LocustSwarmAlgorithm(objective_function=rastrigin_function, dim=2, bounds=bounds)
best_solution, best_value, history = lsa.optimize()
...
plt.xlabel("X1")
plt.ylabel("X2")
plt.title("Locust Swarm Algorithm on Rastrigin Function")
plt.legend()
plt.savefig("LSA.png")
