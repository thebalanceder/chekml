...
from soccer_league_competition_optimizer import SoccerLeagueCompetitionOptimizer
import time
...
# Run SLCO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    slco = SoccerLeagueCompetitionOptimizer(objective_function=func, dim=2, bounds=bounds, 
                                           n_team=10, n_main_player=5, n_reserve_player=3, max_iter=100)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = slco.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"SLCO on {name} Function")
    plt.legend()
    plt.savefig(f"SLCO_{name}.png")
