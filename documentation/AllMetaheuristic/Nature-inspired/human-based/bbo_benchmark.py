...
from biogeography_based_optimization import BiogeographyBasedOptimizer
import time
...
# Run BBO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    bbo = BiogeographyBasedOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = bbo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"BBO on {name} Function")
    plt.legend()
    plt.savefig(f"BBO_{name}.png")
