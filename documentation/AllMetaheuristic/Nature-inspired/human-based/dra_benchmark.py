...
from divine_religions_algorithm import DivineReligionsAlgorithm
import time
...
# Run DRA on each benchmark function
for name, func in benchmark_functions.items():
    ...
    x_vals = [h[1][0] for h in dra.history]
    y_vals = [h[1][1] for h in dra.history]
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"DRA on {name} Function")
    plt.legend()
    plt.savefig(f"DRA_{name}.png")
