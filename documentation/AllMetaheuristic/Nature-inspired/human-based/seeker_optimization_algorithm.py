...
class SeekerOptimizationAlgorithm:
    def __init__(self, objective_function, dim=3, bounds=None, population_size=None, max_fes=9000, 
                 num_regions=3, mu_max=0.99, mu_min=0.0111, w_max=0.9, w_min=0.4):
        """
        ...
        self.dim = dim
        self.bounds = np.array(bounds) if bounds else np.array([[0.2, 1], [22, 30], [100, 108]])
        self.population_size = population_size if population_size else int(3 * (10 + 2 * np.sqrt(dim)))
        ...
        for region in range(num_regions):
            start = int((region) * self.population_size / num_regions)
            end = int((region + 1) * self.population_size / num_regions)
            self.start_reg.append(start)
            self.end_reg.append(end)
            self.size_reg.append(end - start)
    ...
    def initialize_population(self):
        ...
        self.pbest_fun = np.full(self.population_size, np.inf)
        self.lbest_s = np.zeros((self.num_regions, self.dim))
        self.lbest_fun = np.full(self.num_regions, np.inf)
        self.gbest_s = np.zeros(self.dim)
        self.gbest_fun = np.inf

    def evaluate_population(self, fes):
        """Evaluate fitness of the population and update best solutions."""
        for s in range(self.population_size):
            self.fitness[s] = self.objective_function(self.population[s])
            fes += 1

            # Update personal best
            if self.fitness[s] < self.pbest_fun[s]:
                self.pbest_fun[s] = self.fitness[s]
                self.pbest_s[s] = self.population[s].copy()

            # Update global best
            if self.fitness[s] < self.gbest_fun:
                self.gbest_fun = self.fitness[s]
                self.gbest_s = self.population[s].copy()

            self.history.append((fes, self.gbest_s.copy(), self.gbest_fun))
        return fes

    def update_region_bests(self):
    ...
    def optimize(self, runs=30):
        """Run the Seeker Optimization Algorithm for multiple runs."""
        final_gbest_fun = np.zeros(runs)
        final_gbest_s = np.zeros((runs, self.dim))
        final_time = np.zeros(runs)
        func_value_curve = np.full((runs, self.max_fes), np.inf)
        gbest_curve = [[] for _ in range(runs)]
        time_curve = np.zeros((runs, self.max_fes))
        ...
        for run in range(runs):
            ...
            while fes < self.max_fes:
                ...
                for region in range(self.num_regions):
                    ...
                    sorted_indices = [i + self.start_reg[region] for i in sorted_indices]

                    rand_en = max(1, int(np.ceil(np.random.rand() * (self.size_reg[region] - 1))))
                    ...
                    for s in region_indices:
                        idx_local = sorted_indices.index(s)
                        mu_s = mu + (1 - mu) * np.random.rand(self.dim)

                        x_pdirect = self.pbest_s[s] - self.population[s]
                        g1 = 1 if self.lbest_fun[region] < self.fitness[s] else 0
                        x_ldirect1 = self.lbest_s[region] - self.population[s] if g1 else np.zeros(self.dim)
                        g2 = 1 if self.fitness[sorted_indices[-1]] < self.fitness[s] else 0
                        x_ldirect2 = self.population[sorted_indices[-1]] - self.population[s] if g2 else np.zeros(self.dim)

                        # Temporal direction
                        f_values = [f_t_2[s], f_t_1[s], self.fitness[s]]
                        e_values = [e_t_2[s], e_t_1[s], self.population[s]]
                        order_idx = np.argsort(f_values)
                        x_tdirect = e_values[order_idx[0]] - e_values[order_idx[-1]]

                        flag_direct = [1, 1, g1, g2]  # t, p, g1, g2
                        x_signs = [np.sign(x_tdirect), np.sign(x_pdirect), 
                                  np.sign(x_ldirect1), np.sign(x_ldirect2)]
                        
                        select_sign = [i for i, flag in enumerate(flag_direct) if flag > 0]
                        num_sign = len(select_sign)
                        num_pone = np.zeros(self.dim)
                        num_none = np.zeros(self.dim)
                        ...
                        prob_zeros = np.ones(self.dim)
                        rand_roulette = np.random.rand(self.dim)
                        
                        x_direct = np.where(rand_roulette <= prob_pone, 1,
                                          np.where(rand_roulette <= prob_none, -1, 0))
                        
                        sign_x = np.sign(x_direct)
                        sign_x = np.where(self.population[s] > self.bounds[:, 1], -1, sign_x)
                        sign_x = np.where(self.population[s] < self.bounds[:, 0], 1, sign_x)
                        
                        zero_idx = np.where(sign_x == 0)[0]
                        if len(zero_idx) > 0:
                            sign_x[zero_idx] = np.random.choice([1, -1], size=len(zero_idx))
                        
                        r_temp = sign_x * (en_temp * (-2 * np.log(mu_s)) ** 0.5)
                ...
                for s in range(self.population_size):
                    ...
                    time_curve[run, min(fes-1, self.max_fes-1)] = time.process_time() - start_time
            ...
            final_time[run] = time.process_time() - start_time
...
if __name__ == "__main__":
    def objective_function(x):
        return np.sum(x ** 2)  # Example objective function (minimize sum of squares)

    soa = SeekerOptimizationAlgorithm(
        objective_function=objective_function,
        dim=3,
        bounds=[(0.2, 1), (22, 30), (100, 108)],
        max_fes=9000,
        num_regions=3
    )
    
    gbest_s, gbest_fun, func_curve, gbest_curve, time_curve = soa.optimize(runs=1)
    print(f"Best solution: {gbest_s[0]}")
    print(f"Best fitness: {gbest_fun[0]}")
