...
from squid_game_optimizer import SquidGameOptimizer  # Import the SquidGameOptimizer
import time
...
# Run SGO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    sgo = SquidGameOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = sgo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"SGO on {name} Function")
    plt.legend()
    plt.savefig(f"SGO_{name}.png")
