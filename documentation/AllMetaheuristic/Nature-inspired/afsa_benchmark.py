...
from artificial_fish_swarm_algorithm import ArtificialFishSwarmOptimizer
import time
...
# Run AFSA on each benchmark function
for name, func in benchmark_functions.items():
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"AFSA on {name} Function")
    plt.legend()
    plt.savefig(f"AFSA_{name}.png")
