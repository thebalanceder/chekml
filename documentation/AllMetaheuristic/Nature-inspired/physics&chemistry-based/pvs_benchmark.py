...
from population_vortex_search import PopulationVortexSearch
import time
...
# Run PVS on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    pvs = PopulationVortexSearch(objective_function=func, dim=2, bounds=bounds, max_iter=1000)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = pvs.optimize()
    end_time = time.time()
    execution_time = end_time - start_time
    execution_times[name] = execution_time

    # Extract search history
    x_vals = [h["best_solution"][0] for h in history]
    y_vals = [h["best_solution"][1] for h in history]
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"PVS on {name} Function")
    plt.legend()
    plt.savefig(f"PVS_{name}.png")
