...
from turbulent_flow_water_based_optimization import TurbulentFlowWaterOptimizer
import time
...
for name, func in benchmark_functions.items():
    ...
    best_solution, best_value, history = tfwo.optimize()
    end_time = time.time()
    execution_time = end_time - start_time
    execution_times[name] = execution_time

    # Extract search history for plotting
    x_vals = []
    y_vals = []
    for i in range(tfwo.max_iter):
        for wp in tfwo.whirlpools:
            x_vals.append(wp["position"][0])
            y_vals.append(wp["position"][1])
            for obj in wp["objects"]:
                x_vals.append(obj["position"][0])
                y_vals.append(obj["position"][1])
