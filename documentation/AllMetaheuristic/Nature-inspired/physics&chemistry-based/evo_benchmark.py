...
# Run Energy Valley Optimizer on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    evo = EnergyValleyOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Energy Valley Optimizer on {name} Function")
    plt.legend()
    plt.savefig(f"EVO_{name}.png")
