...
from waterwheel_plant_algorithm import WaterwheelPlantOptimizer
import time
...
# Run WWPA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    wwpa = WaterwheelPlantOptimizer(
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"WWPA on {name} Function")
    plt.legend()
    plt.savefig(f"WWPA_{name}.png")
