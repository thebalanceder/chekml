...
from artificial_root_foraging_optimizer import ArtificialRootForagingOptimizer
import time
...
# Run ARFO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    harfo = ArtificialRootForagingOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = harfo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"ARFO on {name} Function")
    plt.legend()
    plt.savefig(f"ARFO_{name}.png")
