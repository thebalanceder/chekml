...
from adaptive_dimensional_search import AdaptiveDimensionSearch
import time
...
# Run ADDR on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    addr = AdaptiveDimensionSearch(
        performance_function=func,
        dim=2,
        bounds=bounds,
        max_collocation_points=1000,
        epsilon_c=0.01,
        gamma_c=0.01,
        eta_c=0.01,
        mcs_samples=10000
    ...
    best_solution, best_value, history = addr.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"ADS on {name} Function")
    plt.legend()
    plt.savefig(f"ADS_{name}.png")
    plt.close()
