...
from bacterial_colony_optimization import BacterialColonyOptimizer
import time
...
# Run BCO on each benchmark function
for name, func in benchmark_functions.items():
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"BCO on {name} Function")
    plt.legend()
    plt.savefig(f"BCO_{name}.png")
