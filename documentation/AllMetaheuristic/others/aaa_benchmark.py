...
from artificial_algae_algorithm import ArtificialAlgaeAlgorithm
import time
...
# Run AAA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    aaa = ArtificialAlgaeAlgorithm(objective_function=func, dim=2, bounds=bounds, 
                                  population_size=50, max_iter=100, step_size=0.1)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = aaa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"AAA on {name} Function")
    plt.legend()
    plt.savefig(f"AAA_{name}.png")
