...
from cricket_algorithm import CricketAlgorithm
import time
...
# Run Cricket Algorithm on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    cricket = CricketAlgorithm(fun=func, dim=2, bounds=bounds, population_size=25, alpha=0.5, max_iter=100)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, iterations = cricket.optimize()
    end_time = time.time()
    execution_time = end_time - start_time
    execution_times[name] = execution_time

    # Extract search history (tracking best solution per iteration)
    # Since CricketAlgorithm doesn't store history, we simulate it by re-running with logging
    history = []
    temp_cricket = CricketAlgorithm(fun=func, dim=2, bounds=bounds, population_size=25, alpha=0.5, max_iter=100)
    temp_cricket.initialize_solutions()
    for _ in range(100):  # Match max_iter
        temp_cricket.optimize()  # Run one iteration
        history.append((_, temp_cricket.best_solution.copy(), temp_cricket.best_fitness))
        if temp_cricket.best_fitness < 1e-6:
            break
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Cricket Algorithm on {name} Function")
    plt.legend()
    plt.savefig(f"Cricket_{name}.png")
