...
def schwefel_function(x_ptr):
    x = np.ctypeslib.as_array(x_ptr, shape=(2,))
    return 418.9829 * len(x) - sum([xi * np.sin(np.sqrt(abs(xi))) for xi in x])

def levy_function(x_ptr):
    x = np.ctypeslib.as_array(x_ptr, shape=(2,))
    w = 1 + (x - 1) / 4
...
def michalewicz_function(x_ptr, m=10):
    x = np.ctypeslib.as_array(x_ptr, shape=(2,))
    return -sum(np.sin(x) * (np.sin(((np.arange(len(x)) + 1) * x**2) / np.pi))**(2 * m))

def shubert_function(x_ptr):
    x = np.ctypeslib.as_array(x_ptr, shape=(2,))
    return np.prod([
...
for name, func in benchmark_functions.items():
    ...
    optimizer = Wrapper(dim=2, population_size=50, max_iter=100, bounds=bounds, method="SFO")
