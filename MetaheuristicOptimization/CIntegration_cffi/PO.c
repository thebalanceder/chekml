...
void election_phase(Optimizer *opt, double *fitness, double *a_winners, int *a_winner_indices) {
    ...
    for (int a = 0; a < areas; a++) {
        ...
        double *dst = a_winners + a * opt->dim;
...
void parliamentarism_phase(Optimizer *opt, double *fitness, double *a_winners, int *a_winner_indices, ObjectiveFunction objective_function, double *temp_pos) {
    ...
    for (int a = 0; a < areas; a++) {
        ...
        double *winner_pos = a_winners + a * opt->dim;
        for (int j = 0; j < opt->dim; j++) {
        ...
        double *to_winner = a_winners + to_area * opt->dim;
        for (int j = 0; j < opt->dim; j++) {
            double distance = fabs(to_winner[j] - new_winner[j]);
            new_winner[j] = to_winner[j] + (2.0 * rand_double(0.0, 1.0) - 1.0) * distance;
...
void PO_optimize(void *opt_void, ObjectiveFunction objective_function) {
    ...
    for (int t = 0; t < opt->max_iter; t++) {
        // Swap aux_fitness and fitness pointers to avoid copying
        double *temp_fitness = aux_fitness;
        aux_fitness = fitness;
        fitness = temp_fitness;

        // Update prev_positions selectively
        for (int i = 0; i < opt->population_size; i++) {
            memcpy(prev_positions + i * opt->dim, opt->population[i].position, opt->dim * sizeof(double));
        }
        ...
        election_phase(opt, fitness, a_winners, a_winner_indices);
        government_formation_phase(opt, fitness, objective_function, temp_pos);
        parliamentarism_phase(opt, fitness, a_winners, a_winner_indices, objective_function, temp_pos);
