...
void rand_double_simd(double *result, double min, double max) {
    // Generate 2 random doubles
    __m128d r = _mm_set_pd(rand_double(min, max), rand_double(min, max));
    _mm_storeu_pd(result, r);
}
...
void elimination_phase(Optimizer *opt) {
    ...
    // Sort the population by fitness in descending order (worst Solutions first)
    #pragma omp parallel
    {
        #pragma omp single
        {
