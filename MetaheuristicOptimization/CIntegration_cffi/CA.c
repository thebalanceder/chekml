...
// Function to generate a random double between min and max (inline for performance)
...
double CA_CoefCalculate(double F, double T) {
    ...
    double alpha = 8.686 * freq_hum_sq * (
        ...
        pow(tempr, -2.5) * (
            0.01275 * (exp(-2239.1 / temp) / (frO + freq_hum_sq / frO)) +
            0.1068 * (exp(-3352.0 / temp) / (frN + freq_hum_sq / frN))
        )
    );
