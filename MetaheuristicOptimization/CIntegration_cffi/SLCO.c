...
void initialize_league(SLCO_Optimizer *opt) {
    ...
    opt->all_players = (Player *)malloc(total_players * sizeof(Player));
    opt->temp_buffer = (double *)malloc(opt->base.dim * sizeof(double));
    ...
    for (int i = 0; i < league->n_teams; i++) {
        league->teams[i].main_players = (Player *)malloc(league->n_main_players * sizeof(Player));
        league->teams[i].reserve_players = (Player *)malloc(league->n_reserve_players * sizeof(Player));
        ...
        for (int j = 0; j < league->n_main_players; j++) {
            league->teams[i].main_players[j].position = (double *)malloc(opt->base.dim * sizeof(double));
        ...
        for (int j = 0; j < league->n_reserve_players; j++) {
            league->teams[i].reserve_players[j].position = (double *)malloc(opt->base.dim * sizeof(double));
...
void winner_function_main(SLCO_Optimizer *opt) {
    ...
    for (int i = 0; i < league->n_main_players; i++) {
        ...
        for (int j = 0; j < opt->base.dim; j++) {
            xxx_pos[j] = (w * league->teams[winner].main_players[i].position[j] +
                          SLCO_ALPHA * ((double)rand() / RAND_MAX) * 
                          (league->teams[winner].main_players[0].position[j] - 
                           league->teams[winner].main_players[i].position[j]) +
                          SLCO_BETA * ((double)rand() / RAND_MAX) * 
                          (league->teams[0].main_players[0].position[j] - 
                           league->teams[winner].main_players[i].position[j]));
        ...
        if (cost < league->teams[winner].main_players[i].cost) {
            ...
            cost = opt->objective_function(xxx_pos);
            if (cost < league->teams[winner].main_players[i].cost) {
                for (int j = 0; j < opt->base.dim; j++) {
                    league->teams[winner].main_players[i].position[j] = xxx_pos[j];
                }
...
void winner_function_reserve(SLCO_Optimizer *opt) {
    ...
    int last_reserve_idx = league->n_reserve_players - 1;
    double *gravity = (double *)malloc(opt->base.dim * sizeof(double));
    if (!gravity) {
    ...
    double beta = 0.9 + 0.1 * ((double)rand() / RAND_MAX);
    for (int j = 0; j < opt->base.dim; j++) {
        double x = league->teams[winner].reserve_players[last_reserve_idx].position[j];
    ...
    if (cost < league->teams[winner].reserve_players[last_reserve_idx].cost) {
        for (int j = 0; j < opt->base.dim; j++) {
            league->teams[winner].reserve_players[last_reserve_idx].position[j] = rq[j];
        }
        league->teams[winner].reserve_players[last_reserve_idx].cost = cost;
    } else {
        beta = 0.45;
        for (int j = 0; j < opt->base.dim; j++) {
            double x = league->teams[winner].reserve_players[last_reserve_idx].position[j];
            rq[j] = gravity[j] + beta * (x - gravity[j]);
            rq[j] = fmin(fmax(rq[j], opt->base.bounds[2 * j]), opt->base.bounds[2 * j + 1]);
        }
        cost = opt->objective_function(rq);
        if (cost < league->teams[winner].reserve_players[last_reserve_idx].cost) {
            for (int j = 0; j < opt->base.dim; j++) {
                league->teams[winner].reserve_players[last_reserve_idx].position[j] = rq[j];
            }
            ...
            for (int j = 0; j < opt->base.dim; j++) {
                double r = (double)rand() / RAND_MAX;
                league->teams[winner].reserve_players[last_reserve_idx].position[j] = 
                    opt->base.bounds[2 * j] + r * (opt->base.bounds[2 * j + 1] - opt->base.bounds[2 * j]);
            }
            league->teams[winner].reserve_players[last_reserve_idx].cost = 
                opt->objective_function(league->teams[winner].reserve_players[last_reserve_idx].position);
        }
    }
...
void loser_function(SLCO_Optimizer *opt) {
    League *league = &opt->league;
    int loser = (league->best_team_idx + 1) % league->n_teams; // Simple loser selection
    double *new_pos = opt->temp_buffer;

    // Mutate three random main players
    int indices[3];
    for (int i = 0; i < 3; i++) {
        indices[i] = rand() % league->n_main_players;
    }

    for (int i = 0; i < 3; i++) {
        int idx = indices[i];
        int n_mu = (int)ceil(SLCO_MUTATION_PROB * opt->base.dim);
        int *r_indices = (int *)malloc(n_mu * sizeof(int));
        if (!r_indices) {
        ...
        for (int k = 0; k < n_mu; k++) {
            new_pos[r_indices[k]] += sigma * (((double)rand() / RAND_MAX) - 0.5);
