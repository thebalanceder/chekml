...
from joa_algorithm import JointOperationsAlgorithm
import time
...
# Define search space bounds as a NumPy array
bounds = np.array([[-5, 5], [-5, 5]], dtype=np.float64)  # 2D
...
# Run JOA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    joa = JointOperationsAlgorithm(objective_function=func, num_variables=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = joa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"JOA on {name} Function")
    plt.legend()
    plt.savefig(f"JOA_{name}.png")
    plt.close()
