...
from cucs_algorithm import CuckooSearchOptimizer
import time
...
# Run Cuckoo Search on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    cuckoo = CuckooSearchOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = cuckoo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Cuckoo Search on {name} Function")
    plt.legend()
    plt.savefig(f"Cuckoo_Search_{name}.png")
