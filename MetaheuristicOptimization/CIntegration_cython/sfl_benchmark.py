...
from shuffled_frog_leaping import FrogLeapingOptimizer
import time
...
bounds = (-5, 5)  # 2D bounds as a single tuple for FrogLeapingOptimizer
...
# Run SFLA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    sfla = FrogLeapingOptimizer(
    ...
    best_solution, best_value, history = sfla.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"SFLA on {name} Function")
    plt.legend()
    plt.savefig(f"SFLA_{name}.png")
