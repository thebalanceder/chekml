...
from ba_algorithm import BatAlgorithm
import time
...
# Run BA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    diso = BatAlgorithm(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"BA on {name} Function")
    plt.legend()
    plt.savefig(f"BA {name}.png")
