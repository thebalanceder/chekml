...
from amo_algorithm import AnimalMigrationOptimizer
import time
...
bounds = np.array([(-5, 5), (-5, 5)], dtype=np.double)  # 2D
...
# Run AMO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    amo = AnimalMigrationOptimizer(objective_function=func, dim=2, bounds=bounds, max_fes=100)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = amo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"AMO on {name} Function")
    plt.legend()
    plt.savefig(f"AMO_{name}.png")
