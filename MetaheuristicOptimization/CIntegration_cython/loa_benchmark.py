...
from loa_algorithm import LionOptimizationAlgorithm
import time
...
# Run LOA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    loa = LionOptimizationAlgorithm(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = loa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"LOA on {name} Function")
    plt.legend()
    plt.savefig(f"LOA_{name}.png")
