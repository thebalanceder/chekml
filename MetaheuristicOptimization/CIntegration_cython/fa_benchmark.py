...
from fa_algorithm import FireworksAlgorithm
import time
...
# Run FWA on each benchmark function
for name, func in benchmark_functions.items():
    ...
    best_solution, best_value, history = fwa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"FWA on {name} Function")
    plt.legend()
    plt.savefig(f"FWA_{name}.png")
