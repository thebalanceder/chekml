...
from sa_algorithm import SimulatedAnnealing
import time
...
# Neighborhood generator for SA
def make_generator(bounds):
    ...
    def generator(x):
        ...
        perturbation[idx] = np.random.uniform(-0.5, 0.5)
        return np.clip(x + perturbation, lower_bounds, upper_bounds)
...
# Run SA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    dim = 2
    generator = make_generator(bounds)
    ...
    best_solution, best_value, history = sa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Simulated Annealing on {name} Function")
    plt.legend()
