...
from ls_algorithm import LocalSearch
import time
...
# Run LocalSearch on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    local_search = LocalSearch(objective_function=func, dim=2, bounds=bounds, max_iter=100, step_size=0.1, neighbor_count=10)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = local_search.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Local Search on {name} Function")
    plt.legend()
    plt.savefig(f"LocalSearch_{name}.png")
