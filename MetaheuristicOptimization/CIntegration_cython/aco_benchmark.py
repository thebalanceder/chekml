...
# Run ACO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    diso = AntColonyOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"ACO on {name} Function")
    plt.legend()
    plt.savefig(f"ACO_{name}.png")
