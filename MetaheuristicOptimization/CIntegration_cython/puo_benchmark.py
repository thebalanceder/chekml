...
from puo_algorithm import PumaOptimizer
import time
...
# Run Puma Optimizer on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    puma = PumaOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = puma.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Puma Optimizer on {name} Function")
    plt.legend()
    plt.savefig(f"Puma_{name}.png")
