...
from ffo_algorithm import FruitFlyOptimizationAlgorithm
import time
...
bounds = np.array([[-5, 5], [-5, 5]])  # 2D, as NumPy array for Cython
...
# Run FOA on each benchmark function
for name, func in benchmark_functions.items():
    ...
    best_solution, best_value, history = foa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"FOA on {name} Function")
    plt.legend()
    plt.savefig(f"FOA_{name}.png")
    plt.close()
