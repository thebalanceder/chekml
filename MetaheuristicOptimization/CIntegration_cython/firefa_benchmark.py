...
from firefa_algorithm import FireflyAlgorithm
import time
...
# Run Firefly Algorithm on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    fa = FireflyAlgorithm(objective_function=func, dim=2, bounds=bounds,
                         population_size=20, max_iter=100, alpha=1.0, beta0=1.0, gamma=0.01, theta=0.97)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = fa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Firefly Algorithm on {name} Function")
    plt.legend()
    plt.savefig(f"firefa_{name}.png")
