...
def svm_objective_function(solution, dim=3):
    ...
    logging.info(f"Received solution: {solution}, type: {type(solution)}, shape: {getattr(solution, 'shape', 'N/A')}")
...
def evaluate_wrapper(wrapper_module, module_name, dim=10, runs=3, method="DISO"):
...
def main(method="DISO"):
    ...
    for module_path in modules:
        ...
        try:
            ...
            scores = evaluate_wrapper(wrapper_module, module_name, method=method)
            if scores:
