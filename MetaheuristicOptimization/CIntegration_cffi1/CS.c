...
static inline void copy_position(double* dest, const double* src, int dim) {
    int d = 0;
    for (; d <= dim - 4; d += 4) {
        __m256d vec = _mm256_loadu_pd(&src[d]);
        _mm256_storeu_pd(&dest[d], vec);
    }
    for (; d < dim; d++) dest[d] = src[d];
}
...
static void hypermutation(Solution* clones, int total_clones, int dim, const double* bounds) {
    for (int i = 0; i < total_clones; i++) {
        ...
        for (int d = 0; d <= dim - 4; d += 4) {
            ...
            __m256d threshold = _mm256_set1_pd(MUTATION_PROBABILITY);

            __m256d mask = _mm256_cmp_pd(r, threshold, _CMP_LT_OS);

            __m256d minv = _mm256_set_pd(bounds[2 * (d + 3)], bounds[2 * (d + 2)], bounds[2 * (d + 1)], bounds[2 * d]);
            __m256d maxv = _mm256_set_pd(bounds[2 * (d + 3) + 1], bounds[2 * (d + 2) + 1], bounds[2 * (d + 1) + 1], bounds[2 * d + 1]);

            __m256d randv = _mm256_set_pd(
                fast_rand(bounds[2 * (d + 3)], bounds[2 * (d + 3) + 1]),
                fast_rand(bounds[2 * (d + 2)], bounds[2 * (d + 2) + 1]),
                fast_rand(bounds[2 * (d + 1)], bounds[2 * (d + 1) + 1]),
                fast_rand(bounds[2 * d], bounds[2 * d + 1])
            );

            __m256d oldv = _mm256_loadu_pd(&pos[d]);
            __m256d newv = _mm256_blendv_pd(oldv, randv, mask);
            _mm256_storeu_pd(&pos[d], newv);
        }
        for (int d = (dim / 4) * 4; d < dim; d++) {
            if (((double)rand() / RAND_MAX) < MUTATION_PROBABILITY) {
                pos[d] = fast_rand(bounds[2 * d], bounds[2 * d + 1]);
            }
        }
    }
}
...
static void replace_worst(Optimizer* opt, int replace_count) {
    ...
    for (int i = 0; i < replace_count; i++) {
        ...
        for (int d = 0; d <= dim - 4; d += 4) {
            __m256d randv = _mm256_set_pd(
                fast_rand(opt->bounds[2 * (d + 3)], opt->bounds[2 * (d + 3) + 1]),
                fast_rand(opt->bounds[2 * (d + 2)], opt->bounds[2 * (d + 2) + 1]),
                fast_rand(opt->bounds[2 * (d + 1)], opt->bounds[2 * (d + 1) + 1]),
                fast_rand(opt->bounds[2 * d], opt->bounds[2 * d + 1])
            );
            _mm256_storeu_pd(&pos[d], randv);
        }
        for (int d = (dim / 4) * 4; d < dim; d++) {
