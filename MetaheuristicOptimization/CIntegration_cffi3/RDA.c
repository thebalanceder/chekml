...
void RDA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Red Deer Algorithm optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("RDA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open RDA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: RDA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read RDA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: RDA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    RDACLContext cl_ctx = {0};
    RDA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    RDAContext ctx = {0};
    RDA_init_context(&ctx, opt, &cl_ctx);
    ...
    while (func_count < max_evals) {
        ...
        cl_float step_size = RDA_STEP_SIZE;
        cl_float p_exploration = RDA_P_EXPLORATION;
        ...
        clSetKernelArg(cl_ctx.update_kernel, 6, sizeof(cl_float), &p_exploration);
    ...
    RDA_cleanup_context(&ctx, &cl_ctx);
    RDA_cleanup_cl(&cl_ctx);
