...
void WOA_init_cl(WOACLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->update_kernel = clCreateKernel(cl_ctx->program, "update_whales", &err);
    check_cl_error(err, "clCreateKernel update_whales");
}
...
void WOA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    ...
    // Read kernel source
    FILE *fp = fopen("WOA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open WOA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: WOA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read WOA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: WOA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    WOACLContext cl_ctx = {0};
    WOA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    WOAContext ctx = {0};
    WOA_init_context(&ctx, opt, &cl_ctx);
    ...
    while (func_count < max_evals) {
        ...
        // Compute a and a2
        float a = WOA_A_INITIAL - ((float)iteration * WOA_A_INITIAL / (float)opt->max_iter);
        float a2 = WOA_A2_INITIAL + ((float)iteration * (WOA_A2_FINAL - WOA_A2_INITIAL) / (float)opt->max_iter);
        ...
        clSetKernelArg(cl_ctx.update_kernel, 6, sizeof(cl_float), &a2);
        ...
        check_cl_error(err, "clEnqueueNDRangeKernel update_whales");
        clFinish(cl_ctx.queue);
    ...
    WOA_cleanup_context(&ctx, &cl_ctx);
    WOA_cleanup_cl(&cl_ctx);
