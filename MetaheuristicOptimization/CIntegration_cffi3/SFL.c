...
// Preprocess kernel source (placeholder, assuming no specific preprocessing needed)
...
void SFL_init_cl(SFLCLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->update_memeplex_kernel = clCreateKernel(cl_ctx->program, "update_memeplex", &err);
    check_cl_error(err, "clCreateKernel update_memeplex");
}

void SFL_cleanup_cl(SFLCLContext *cl_ctx) {
    if (cl_ctx->update_memeplex_kernel) clReleaseKernel(cl_ctx->update_memeplex_kernel);
...
// CPU-based quicksort for sorting population
...
static int partition(float *fitness, int *indices, int low, int high) {
...
static void quicksort_fitness(float *fitness, int *indices, int low, int high) {
    if (low < high) {
        int pi = partition(fitness, indices, low, high);
...
void SFL_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Shuffled Frog Leaping Algorithm optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("SFL.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open SFL.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: SFL.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read SFL.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: SFL.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    SFLCLContext cl_ctx = {0};
    SFL_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    SFLContext ctx = {0};
    SFL_init_context(&ctx, opt, &cl_ctx);
    ...
    int max_evals = opt->max_iter > 0 ? opt->max_iter * pop_size : SFL_MAX_EVALS_DEFAULT;
    ...
    float *probabilities = (float *)malloc(SFL_MEMEPLEX_SIZE * sizeof(float));
    float sum_P = 0.0f;
    for (int i = 0; i < SFL_MEMEPLEX_SIZE; i++) {
        probabilities[i] = 2.0f * (SFL_MEMEPLEX_SIZE + 1 - (i + 1)) / (SFL_MEMEPLEX_SIZE * (SFL_MEMEPLEX_SIZE + 1));
    ...
    // Initialize memeplex indices
    int *memeplex_indices = (int *)malloc(SFL_NUM_MEMEPLEXES * SFL_MEMEPLEX_SIZE * sizeof(int));
    for (int m = 0; m < SFL_NUM_MEMEPLEXES; m++) {
        for (int i = 0; i < SFL_MEMEPLEX_SIZE; i++) {
            memeplex_indices[m * SFL_MEMEPLEX_SIZE + i] = i * SFL_NUM_MEMEPLEXES + m;
        }
    }

    cl_mem indices_buf = clCreateBuffer(cl_ctx.context, CL_MEM_READ_WRITE, SFL_NUM_MEMEPLEXES * SFL_MEMEPLEX_SIZE * sizeof(cl_int), NULL, &err);
    check_cl_error(err, "clCreateBuffer indices");
    err = clEnqueueWriteBuffer(cl_ctx.queue, indices_buf, CL_TRUE, 0, SFL_NUM_MEMEPLEXES * SFL_MEMEPLEX_SIZE * sizeof(cl_int), memeplex_indices, 0, NULL, NULL);
    check_cl_error(err, "clEnqueueWriteBuffer indices");

    cl_mem prob_buf = clCreateBuffer(cl_ctx.context, CL_MEM_READ_WRITE, SFL_MEMEPLEX_SIZE * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer probabilities");
    err = clEnqueueWriteBuffer(cl_ctx.queue, prob_buf, CL_TRUE, 0, SFL_MEMEPLEX_SIZE * sizeof(cl_float), probabilities, 0, NULL, NULL);
    check_cl_error(err, "clEnqueueWriteBuffer probabilities");
    ...
    while (func_count < max_evals) {
        ...
        quicksort_fitness(fitness, indices, 0, pop_size - 1);

        float *sorted_fitness = (float *)malloc(pop_size * sizeof(float));
        float *positions = (float *)malloc(pop_size * dim * sizeof(float));
        err = clEnqueueReadBuffer(cl_ctx.queue, ctx.population, CL_TRUE, 0, pop_size * dim * sizeof(cl_float), positions, 0, NULL, &read_event);
        check_cl_error(err, "clEnqueueReadBuffer population");
        float *sorted_positions = (float *)malloc(pop_size * dim * sizeof(float));
        for (int i = 0; i < pop_size; i++) {
            sorted_fitness[i] = fitness[indices[i]];
            for (int j = 0; j < dim; j++) {
                sorted_positions[i * dim + j] = positions[indices[i] * dim + j];
            }
        }
        err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.fitness, CL_TRUE, 0, pop_size * sizeof(cl_float), sorted_fitness, 0, NULL, &write_event);
        check_cl_error(err, "clEnqueueWriteBuffer fitness");
        err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.population, CL_TRUE, 0, pop_size * dim * sizeof(cl_float), sorted_positions, 0, NULL, &write_event);
        check_cl_error(err, "clEnqueueWriteBuffer population");
        free(fitness); free(indices); free(sorted_fitness); free(positions); free(sorted_positions);
        ...
        int num_parents = (int)(SFL_NUM_PARENTS_RATIO * SFL_MEMEPLEX_SIZE);
        if (num_parents < 2) num_parents = 2;
        int memeplex_size = SFL_MEMEPLEX_SIZE; // Store macro value in variable
        for (int m = 0; m < SFL_NUM_MEMEPLEXES; m++) {
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 0, sizeof(cl_mem), &ctx.population);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 1, sizeof(cl_mem), &ctx.fitness);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 2, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 3, sizeof(cl_mem), &ctx.best_position);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 4, sizeof(cl_mem), &ctx.random_seeds);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 5, sizeof(cl_mem), &indices_buf);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 6, sizeof(cl_mem), &prob_buf);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 7, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 8, sizeof(cl_int), &memeplex_size);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 9, sizeof(cl_int), &num_parents);
            clSetKernelArg(cl_ctx.update_memeplex_kernel, 10, sizeof(cl_int), &m);

            size_t memeplex_work_size = SFL_MEMEPLEX_SIZE;
            err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.update_memeplex_kernel, 1, NULL, &memeplex_work_size, NULL, 0, NULL, &update_event);
            check_cl_error(err, "clEnqueueNDRangeKernel update_memeplex");
            clFinish(cl_ctx.queue);
    ...
    SFL_cleanup_context(&ctx, &cl_ctx);
    SFL_cleanup_cl(&cl_ctx);
