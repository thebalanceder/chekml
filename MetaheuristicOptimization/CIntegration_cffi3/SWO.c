...
// Preprocess kernel source (stub, can be extended if needed)
...
void SWO_init_cl(SWOCLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    check_cl_error(err, "clCreateKernel hunting_phase");
...
void SWO_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Spider Wasp Optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("SWO.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open SWO.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: SWO.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read SWO.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: SWO.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    SWOCLContext cl_ctx = {0};
    SWO_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    SWOContext ctx = {0};
    SWO_init_context(&ctx, opt, &cl_ctx);
    ...
    while (func_count < max_evals) {
        ...
        // Randomly choose hunting or mating phase
        cl_kernel selected_kernel = (rand() / (float)RAND_MAX < SWO_TRADE_OFF) ? cl_ctx.hunting_kernel : cl_ctx.mating_kernel;
        double *phase_time = (selected_kernel == cl_ctx.hunting_kernel) ? &prof.hunting_time : &prof.mating_time;

        cl_event phase_event;
        clSetKernelArg(selected_kernel, 0, sizeof(cl_mem), &ctx.population);
        clSetKernelArg(selected_kernel, 1, sizeof(cl_mem), &ctx.fitness);
        clSetKernelArg(selected_kernel, 2, sizeof(cl_mem), &ctx.bounds);
        clSetKernelArg(selected_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
        if (selected_kernel == cl_ctx.hunting_kernel) {
            clSetKernelArg(selected_kernel, 4, sizeof(cl_mem), &ctx.best_position);
        }
        clSetKernelArg(selected_kernel, selected_kernel == cl_ctx.hunting_kernel ? 5 : 4, sizeof(cl_int), &dim);
        clSetKernelArg(selected_kernel, selected_kernel == cl_ctx.hunting_kernel ? 6 : 5, sizeof(cl_int), &pop_size);

        err = clEnqueueNDRangeKernel(cl_ctx.queue, selected_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &phase_event);
        check_cl_error(err, selected_kernel == cl_ctx.hunting_kernel ? "clEnqueueNDRangeKernel hunting_phase" : "clEnqueueNDRangeKernel mating_phase");
        clFinish(cl_ctx.queue);
    ...
    SWO_cleanup_context(&ctx, &cl_ctx);
    SWO_cleanup_cl(&cl_ctx);
