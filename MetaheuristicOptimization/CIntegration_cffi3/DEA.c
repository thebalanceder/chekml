/* DEA.c - GPU-Optimized Dolphin Echolocation Algorithm with Flexible Objective Function */
#include "DEA.h"
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
static const char* dea_kernel_source =
    ...
    // Atomic add for float (workaround for OpenCL)
    ...
    "}\n"
    // Initialize population and alternatives
    "__kernel void initialize_locations(\n"
    "    __global float* population,\n"
    "    __global float* alternatives,\n"
    "    __global const float* bounds,\n"
    "    const int dim,\n"
    "    const int population_size,\n"
    "    const int alternatives_per_dim,\n"
    "    uint seed)\n"
    "{\n"
    ...
    "    }\n"
    "    if (id < dim) {\n"
    "        float lower = bounds[2 * id];\n"
    "        float upper = bounds[2 * id + 1];\n"
    "        for (int k = 0; k < alternatives_per_dim; k++) {\n"
    "            alternatives[id * alternatives_per_dim + k] = lower + (upper - lower) * k / (alternatives_per_dim - 1);\n"
    "        }\n"
    "    }\n"
    "}\n"
    // Calculate accumulative fitness
    "__kernel void calculate_accumulative_fitness(\n"
    "    __global const float* population,\n"
    "    __global const float* fitness,\n"
    "    __global const float* alternatives,\n"
    "    __global float* accum_fitness,\n"
    "    const int dim,\n"
    "    const int population_size,\n"
    "    const int alternatives_per_dim,\n"
    ...
    "        int i = id / dim;\n"
    "        int j = id % dim;\n"
    "        float loc_value = population[i * dim + j];\n"
    "        int alt_idx = 0;\n"
    "        float min_diff = fabs(alternatives[j * alternatives_per_dim] - loc_value);\n"
    "        for (int k = 1; k < alternatives_per_dim; k++) {\n"
    "            float diff = fabs(alternatives[j * alternatives_per_dim + k] - loc_value);\n"
    "            if (diff < min_diff) {\n"
    "                min_diff = diff;\n"
    "                alt_idx = k;\n"
    "            }\n"
    ...
    "            if (neighbor_idx >= 0 && neighbor_idx < alternatives_per_dim) {\n"
    "                float weight = (10.0f - fabs((float)k)) / 10.0f;\n"
    "                atomic_add_float(&accum_fitness[j * alternatives_per_dim + neighbor_idx], weight * fitness[i]);\n"
    "            }\n"
    "        }\n"
    "    }\n"
    "}\n"
    // Update probabilities
    "__kernel void update_probabilities(\n"
    "    __global const float* best_position,\n"
    "    __global const float* alternatives,\n"
    "    __global const float* accum_fitness,\n"
    ...
    "    if (j < dim) {\n"
    "        float t = (float)loop / max_loops;\n"
    "        float convergence_prob = pow(t, DEA_CONVERGENCE_POWER);\n"
    "        float loc_value = best_position[j];\n"
    "        int best_alt_idx = 0;\n"
    "        float min_diff = fabs(alternatives[j * alternatives_per_dim] - loc_value);\n"
    "        for (int k = 1; k < alternatives_per_dim; k++) {\n"
    "            float diff = fabs(alternatives[j * alternatives_per_dim + k] - loc_value);\n"
    ...
    "        float total_af = 0.0f;\n"
    "        for (int k = 0; k < alternatives_per_dim; k++) {\n"
    "            total_af += accum_fitness[j * alternatives_per_dim + k];\n"
    "        }\n"
    "        if (total_af == 0.0f) {\n"
    "            for (int k = 0; k < alternatives_per_dim; k++) {\n"
    "                probabilities[j * alternatives_per_dim + k] = 1.0f / alternatives_per_dim;\n"
    "            }\n"
    "        } else {\n"
    "            probabilities[j * alternatives_per_dim + best_alt_idx] = convergence_prob;\n"
    "            float remaining_prob = 1.0f - convergence_prob;\n"
    "            float prob_sum = convergence_prob;\n"
    "            for (int k = 0; k < alternatives_per_dim; k++) {\n"
    "                if (k != best_alt_idx) {\n"
    "                    float prob = (accum_fitness[j * alternatives_per_dim + k] / total_af) * remaining_prob;\n"
    "                    prob = max(prob, DEA_PROBABILITY_THRESHOLD);\n"
    "                    probabilities[j * alternatives_per_dim + k] = prob;\n"
    "                    prob_sum += prob;\n"
    "                }\n"
    "            }\n"
    "            if (prob_sum > 0.0f) {\n"
    "                for (int k = 0; k < alternatives_per_dim; k++) {\n"
    "                    probabilities[j * alternatives_per_dim + k] /= prob_sum;\n"
    "                }\n"
    "            }\n"
    "        }\n"
    "    }\n"
    ...
    "__kernel void select_new_locations(\n"
    "    __global float* population,\n"
    "    __global const float* alternatives,\n"
    ...
    "        uint local_seed = seed + id * dim;\n"
    "        for (int j = 0; j < dim; j++) {\n"
    "            if (local_id < alternatives_per_dim) {\n"
    "                local_probs[local_id] = probabilities[j * alternatives_per_dim + local_id];\n"
    "            }\n"
    ...
    "            int selected_idx = 0;\n"
    "            for (int k = 0; k < alternatives_per_dim; k++) {\n"
    "                cum_prob += local_probs[k];\n"
    "                if (r <= cum_prob) {\n"
    "                    selected_idx = k;\n"
    "                    break;\n"
    "                }\n"
    "            }\n"
    "            population[id * dim + j] = alternatives[j * alternatives_per_dim + selected_idx];\n"
    "        }\n"
    "    }\n"
    "}\n"
    // Find best solution
...
void DEA_optimize(Optimizer* opt, ObjectiveFunction objective_function) {
    cl_int err;
    cl_program program = NULL;
    cl_kernel init_kernel = NULL, accum_fitness_kernel = NULL, update_probs_kernel = NULL, select_kernel = NULL, best_kernel = NULL;
    cl_mem bounds_buffer = NULL, alternatives_buffer = NULL, accum_fitness_buffer = NULL, probabilities_buffer = NULL, best_position_buffer = NULL, best_idx_buffer = NULL;
    float* bounds_float = NULL, *population = NULL, *fitness = NULL, *alternatives = NULL, *accum_fitness = NULL, *probabilities = NULL, *best_position = NULL;
    ...
    const int alternatives_per_dim = DEA_ALTERNATIVES_PER_DIM;
    if (dim < 1 || population_size < 1 || max_iter < 1 || alternatives_per_dim < 1) {
        fprintf(stderr, "Error: Invalid dim (%d), population_size (%d), max_iter (%d), or alternatives_per_dim (%d)\n", 
                dim, population_size, max_iter, alternatives_per_dim);
        exit(EXIT_FAILURE);
    }
    if (dim > 32) {
    ...
    effective_radius = (float)(DEA_EFFECTIVE_RADIUS_FACTOR * min_search_size);
    ...
    program = clCreateProgramWithSource(new_context, 1, &dea_kernel_source, NULL, &err);
    if (err != CL_SUCCESS || !program) {
        fprintf(stderr, "Error creating DEA program: %d\n", err);
        clReleaseCommandQueue(new_queue);
        clReleaseContext(new_context);
        exit(EXIT_FAILURE);
    }
    ...
    if (err != CL_SUCCESS) {
        ...
        if (log) {
            ...
            fprintf(stderr, "Error building DEA program: %d\nBuild log:\n%s\n", err, log);
            free(log);
        }
    ...
    // Create kernels
    init_kernel = clCreateKernel(program, "initialize_locations", &err);
    accum_fitness_kernel = clCreateKernel(program, "calculate_accumulative_fitness", &err);
    update_probs_kernel = clCreateKernel(program, "update_probabilities", &err);
    select_kernel = clCreateKernel(program, "select_new_locations", &err);
    best_kernel = clCreateKernel(program, "find_best", &err);
    if (err != CL_SUCCESS || !init_kernel || !accum_fitness_kernel || !update_probs_kernel || !select_kernel || !best_kernel) {
        fprintf(stderr, "Error creating DEA kernels: %d\n", err);
        goto cleanup;
    }

    // Query kernel work-group size
    size_t init_preferred_multiple, accum_preferred_multiple, probs_preferred_multiple, select_preferred_multiple, best_preferred_multiple;
    ...
    err |= clGetKernelWorkGroupInfo(accum_fitness_kernel, gpu_device, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, 
                                    sizeof(size_t), &accum_preferred_multiple, NULL);
    err |= clGetKernelWorkGroupInfo(update_probs_kernel, gpu_device, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, 
                                    sizeof(size_t), &probs_preferred_multiple, NULL);
    err |= clGetKernelWorkGroupInfo(select_kernel, gpu_device, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, 
                                    sizeof(size_t), &select_preferred_multiple, NULL);
    ...
    if (err != CL_SUCCESS) {
        ...
        printf("Preferred work-group size multiple: init=%zu, accum=%zu, probs=%zu, select=%zu, best=%zu\n", 
               init_preferred_multiple, accum_preferred_multiple, probs_preferred_multiple, select_preferred_multiple, best_preferred_multiple);
    }
    ...
    alternatives = (float*)malloc(dim * alternatives_per_dim * sizeof(float));
    accum_fitness = (float*)calloc(dim * alternatives_per_dim, sizeof(float));
    probabilities = (float*)calloc(dim * alternatives_per_dim, sizeof(float));
    ...
    if (!bounds_float || !population || !fitness || !alternatives || !accum_fitness || !probabilities || !best_position || !best_idx_array || !temp_position) {
    ...
    alternatives_buffer = clCreateBuffer(new_context, CL_MEM_READ_WRITE, dim * alternatives_per_dim * sizeof(float), NULL, &err);
    accum_fitness_buffer = clCreateBuffer(new_context, CL_MEM_READ_WRITE, dim * alternatives_per_dim * sizeof(float), NULL, &err);
    probabilities_buffer = clCreateBuffer(new_context, CL_MEM_READ_WRITE, dim * alternatives_per_dim * sizeof(float), NULL, &err);
    ...
    if (err != CL_SUCCESS || !bounds_buffer || !alternatives_buffer || !accum_fitness_buffer || 
        !probabilities_buffer || !best_position_buffer || !best_idx_buffer || !opt->population_buffer || !opt->fitness_buffer) {
        fprintf(stderr, "Error creating DEA buffers: %d\n", err);
        goto cleanup;
    }
    ...
    // Initialize locations and alternatives
    uint seed = (uint)time(NULL);
    err = clSetKernelArg(init_kernel, 0, sizeof(cl_mem), &opt->population_buffer);
    err |= clSetKernelArg(init_kernel, 1, sizeof(cl_mem), &alternatives_buffer);
    ...
    err |= clSetKernelArg(init_kernel, 5, sizeof(int), &alternatives_per_dim);
    err |= clSetKernelArg(init_kernel, 6, sizeof(uint), &seed);
    if (err != CL_SUCCESS) {
    ...
    for (int loop = 0; loop < max_iter; loop++) {
        ...
        err = clEnqueueNDRangeKernel(new_queue, best_kernel, 1, NULL, &best_global_work_size, &local_work_size, 0, NULL, &events[4]);
        if (err != CL_SUCCESS) {
        ...
        // Calculate accumulative fitness
        err = clSetKernelArg(accum_fitness_kernel, 0, sizeof(cl_mem), &opt->population_buffer);
        err |= clSetKernelArg(accum_fitness_kernel, 1, sizeof(cl_mem), &opt->fitness_buffer);
        err |= clSetKernelArg(accum_fitness_kernel, 2, sizeof(cl_mem), &alternatives_buffer);
        err |= clSetKernelArg(accum_fitness_kernel, 3, sizeof(cl_mem), &accum_fitness_buffer);
        err |= clSetKernelArg(accum_fitness_kernel, 4, sizeof(int), &dim);
        err |= clSetKernelArg(accum_fitness_kernel, 5, sizeof(int), &population_size);
        err |= clSetKernelArg(accum_fitness_kernel, 6, sizeof(int), &alternatives_per_dim);
        err |= clSetKernelArg(accum_fitness_kernel, 7, sizeof(float), &effective_radius);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error setting accum_fitness kernel args: %d\n", err);
            goto cleanup;
        }

        size_t accum_global_work_size = ((population_size * dim + local_work_size - 1) / local_work_size) * local_work_size;
        err = clEnqueueNDRangeKernel(new_queue, accum_fitness_kernel, 1, NULL, &accum_global_work_size, &local_work_size, 0, NULL, &events[1]);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error enqueuing accum_fitness kernel: %d\n", err);
            goto cleanup;
        }

        // Update probabilities
        err = clSetKernelArg(update_probs_kernel, 0, sizeof(cl_mem), &best_position_buffer);
        err |= clSetKernelArg(update_probs_kernel, 1, sizeof(cl_mem), &alternatives_buffer);
        err |= clSetKernelArg(update_probs_kernel, 2, sizeof(cl_mem), &accum_fitness_buffer);
        err |= clSetKernelArg(update_probs_kernel, 3, sizeof(cl_mem), &probabilities_buffer);
        err |= clSetKernelArg(update_probs_kernel, 4, sizeof(int), &dim);
        err |= clSetKernelArg(update_probs_kernel, 5, sizeof(int), &alternatives_per_dim);
        err |= clSetKernelArg(update_probs_kernel, 6, sizeof(int), &loop);
        err |= clSetKernelArg(update_probs_kernel, 7, sizeof(int), &max_iter);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error setting update_probs kernel args: %d\n", err);
            goto cleanup;
        }

        size_t probs_global_work_size = ((dim + local_work_size - 1) / local_work_size) * local_work_size;
        err = clEnqueueNDRangeKernel(new_queue, update_probs_kernel, 1, NULL, &probs_global_work_size, &local_work_size, 0, NULL, &events[2]);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error enqueuing update_probs kernel: %d\n", err);
            goto cleanup;
        }

        // Select new locations
        err = clSetKernelArg(select_kernel, 0, sizeof(cl_mem), &opt->population_buffer);
        err |= clSetKernelArg(select_kernel, 1, sizeof(cl_mem), &alternatives_buffer);
        err |= clSetKernelArg(select_kernel, 2, sizeof(cl_mem), &probabilities_buffer);
        err |= clSetKernelArg(select_kernel, 3, sizeof(int), &dim);
        err |= clSetKernelArg(select_kernel, 4, sizeof(int), &population_size);
        err |= clSetKernelArg(select_kernel, 5, sizeof(int), &alternatives_per_dim);
        err |= clSetKernelArg(select_kernel, 6, sizeof(uint), &seed);
        err |= clSetKernelArg(select_kernel, 7, alternatives_per_dim * sizeof(float), NULL);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error setting select kernel args: %d\n", err);
            goto cleanup;
        }

        size_t select_global_work_size = ((population_size + local_work_size - 1) / local_work_size) * local_work_size;
        err = clEnqueueNDRangeKernel(new_queue, select_kernel, 1, NULL, &select_global_work_size, &local_work_size, 0, NULL, &events[3]);
        if (err != CL_SUCCESS) {
        ...
        double init_time = 0, accum_time = 0, probs_time = 0, select_time = 0, best_time = 0;
        ...
        if (err == CL_SUCCESS && (queue_properties & CL_QUEUE_PROFILING_ENABLE)) {
            ...
            if (events[1]) {
                ...
                if (err == CL_SUCCESS) {
                    ...
                    if (err == CL_SUCCESS) accum_time = (time_end - time_start) / 1e6;
                }
            }
            if (events[2]) {
                ...
                if (err == CL_SUCCESS) {
                    ...
                    if (err == CL_SUCCESS) probs_time = (time_end - time_start) / 1e6;
                }
            }
            if (events[3]) {
                ...
                if (err == CL_SUCCESS) {
                    ...
                    if (err == CL_SUCCESS) select_time = (time_end - time_start) / 1e6;
                }
            }
            if (events[4]) {
            ...
            printf("DEA|%5d -----> %9.16f | Total: %.3f ms | Init: %.3f ms | Accum: %.3f ms | Probs: %.3f ms | Select: %.3f ms | Best: %.3f ms\n", 
                   loop + 1, opt->best_solution.fitness, end_time - start_time, 
                   init_time, accum_time, probs_time, select_time, best_time);
        } else {
            end_time = get_time_ms();
            printf("DEA|%5d -----> %9.16f | Total: %.3f ms | Profiling disabled\n", 
                   loop + 1, opt->best_solution.fitness, end_time - start_time);
        }
    }
...
cleanup:
    ...
    if (alternatives) free(alternatives);
    if (accum_fitness) free(accum_fitness);
    ...
    if (alternatives_buffer) clReleaseMemObject(alternatives_buffer);
    if (accum_fitness_buffer) clReleaseMemObject(accum_fitness_buffer);
    ...
    if (accum_fitness_kernel) clReleaseKernel(accum_fitness_kernel);
    if (update_probs_kernel) clReleaseKernel(update_probs_kernel);
