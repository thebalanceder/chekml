...
void CMOA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU CMOA optimization...\n");
    if (!opt || !opt->context || !opt->queue || !opt->device || 
    ...
    FILE *fp = fopen("CMOA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open CMOA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: CMOA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read CMOA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: CMOA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }
    CMOACLContext cl_ctx = {0};
    CMOA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);
    CMOAContext ctx = {0};
    CMOA_init_context(&ctx, opt, &cl_ctx);
    int pop_size = opt->population_size;
    int dim = opt->dim;
    int func_count = 0;
    int iteration = 0;
    ...
    float *cos_table = (float *)malloc(opt->max_iter * sizeof(float));
    for (int t = 0; t < opt->max_iter; t++) {
        cos_table[t] = cos(M_PI * ((float)t / (float)opt->max_iter));
    }
    typedef struct {
    ...
    while (func_count < max_evals) {
        ...
        cl_float cos_t = cos_table[iteration - 1];
        ...
        clSetKernelArg(cl_ctx.update_kernel, 5, sizeof(cl_float), &cos_t);
    ...
    CMOA_cleanup_context(&ctx, &cl_ctx);
    CMOA_cleanup_cl(&cl_ctx);
