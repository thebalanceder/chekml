...
static const char* ffo_kernel_source =
...
"}\n"
"// Initialize swarm\n"
"__kernel void initialize_swarm(\n"
...
"        float swarm_axis[2];\n"
"        swarm_axis[0] = lcg_rand_float(&local_seed, local_bounds[0], local_bounds[1]);\n"
"        swarm_axis[1] = dim > 1 ? lcg_rand_float(&local_seed, local_bounds[2], local_bounds[3]) : swarm_axis[0];\n"
"        for (int d = 0; d < dim; d++) {\n"
"            float random_value = FFO_SMELL_RANGE * (2.0f * lcg_rand_float(&local_seed, 0.0f, 1.0f) - 1.0f);\n"
"            population[id * dim + d] = swarm_axis[d % 2] + random_value;\n"
...
"}\n"
"// Smell-based search\n"
"__kernel void smell_based_search(\n"
"    __global float* population,\n"
...
"        barrier(CLK_LOCAL_MEM_FENCE);\n"
"        float swarm_axis[2];\n"
"        swarm_axis[0] = best_position[0];\n"
"        swarm_axis[1] = dim > 1 ? best_position[1] : best_position[0];\n"
"        for (int d = 0; d < dim; d++) {\n"
"            float random_value = FFO_SMELL_RANGE * (2.0f * lcg_rand_float(&local_seed, 0.0f, 1.0f) - 1.0f);\n"
"            population[id * dim + d] = swarm_axis[d % 2] + random_value;\n"
"            float min = local_bounds[2 * d];\n"
...
void FFO_optimize(Optimizer* opt, ObjectiveFunction objective_function) {
    ...
    cl_kernel init_kernel = NULL, smell_kernel = NULL, best_kernel = NULL;
    cl_mem bounds_buffer = NULL, population_buffer = NULL, fitness_buffer = NULL;
    cl_mem best_idx_buffer = NULL, best_position_buffer = NULL;
    ...
    const int swarm_size = opt->population_size;
    const int max_iter = opt->max_iter;
    if (dim < 1 || swarm_size < 1 || max_iter < 1) {
        fprintf(stderr, "Error: Invalid dim (%d), swarm_size (%d), or max_iter (%d)\n", 
                dim, swarm_size, max_iter);
        exit(EXIT_FAILURE);
    }
    ...
    // Create program
    program = clCreateProgramWithSource(context, 1, &ffo_kernel_source, NULL, &err);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Error creating FFO program: %d\n", err);
    ...
    if (err != CL_SUCCESS) {
        ...
        if (log) {
            ...
            fprintf(stderr, "Error building FFO program: %d\nBuild log:\n%s\n", err, log);
            free(log);
        }
    ...
    // Create kernels
    init_kernel = clCreateKernel(program, "initialize_swarm", &err);
    smell_kernel = clCreateKernel(program, "smell_based_search", &err);
    best_kernel = clCreateKernel(program, "find_best", &err);
    if (err != CL_SUCCESS || !init_kernel || !smell_kernel || !best_kernel) {
        fprintf(stderr, "Error creating FFO kernels: %d\n", err);
        goto cleanup;
    }
    ...
    size_t local_work_size = max_work_group_size < FFO_LOCAL_WORK_SIZE ? max_work_group_size : FFO_LOCAL_WORK_SIZE;

    // Create buffers
    bounds_float = (float*)malloc(2 * dim * sizeof(float));
    population = (float*)malloc(swarm_size * dim * sizeof(float));
    fitness = (float*)malloc(swarm_size * sizeof(float));
    best_position = (float*)malloc(dim * sizeof(float));
    best_idx_array = (int*)malloc(swarm_size * sizeof(int));
    ...
    population_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, swarm_size * dim * sizeof(float), NULL, &err);
    fitness_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, swarm_size * sizeof(float), NULL, &err);
    best_idx_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, swarm_size * sizeof(int), NULL, &err);
    ...
    if (err != CL_SUCCESS || !bounds_buffer || !population_buffer || !fitness_buffer || 
        !best_idx_buffer || !best_position_buffer) {
        fprintf(stderr, "Error creating FFO buffers: %d\n", err);
        goto cleanup;
    }
    ...
    err |= clSetKernelArg(init_kernel, 3, sizeof(int), &swarm_size);
    ...
    size_t global_work_size = ((swarm_size + local_work_size - 1) / local_work_size) * local_work_size;
    ...
    // Evaluate initial swarm on host
    err = clEnqueueReadBuffer(queue, population_buffer, CL_TRUE, 0, 
                              swarm_size * dim * sizeof(float), population, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(queue, fitness_buffer, CL_TRUE, 0, 
                               swarm_size * sizeof(float), fitness, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err |= clSetKernelArg(best_kernel, 5, sizeof(int), &swarm_size);
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        // Smell-based search
        err = clSetKernelArg(smell_kernel, 0, sizeof(cl_mem), &population_buffer);
        err |= clSetKernelArg(smell_kernel, 1, sizeof(cl_mem), &best_position_buffer);
        err |= clSetKernelArg(smell_kernel, 2, sizeof(cl_mem), &bounds_buffer);
        err |= clSetKernelArg(smell_kernel, 3, sizeof(int), &dim);
        err |= clSetKernelArg(smell_kernel, 4, sizeof(int), &swarm_size);
        err |= clSetKernelArg(smell_kernel, 5, sizeof(uint), &seed);
        err |= clSetKernelArg(smell_kernel, 6, 2 * dim * sizeof(float), NULL);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error setting smell kernel args: %d\n", err);
            goto cleanup;
        }
        err = clEnqueueNDRangeKernel(queue, smell_kernel, 1, NULL, &global_work_size, &local_work_size, 0, NULL, &events[0]);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error enqueuing smell kernel: %d\n", err);
            goto cleanup;
        }

        // Evaluate swarm on host
        err = clEnqueueReadBuffer(queue, population_buffer, CL_TRUE, 0, 
                                  swarm_size * dim * sizeof(float), population, 0, NULL, NULL);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error reading population buffer: %d\n", err);
        ...
        double smell_time = 0, best_time = 0;
        if (events[0]) {
            ...
            smell_time = (time_end - time_start) / 1e6;
        }
        if (events[1]) {
        ...
        printf("FFO|%5d -----> %9.16f | Total: %.3f ms | Smell: %.3f ms | Best: %.3f ms\n", 
               iter + 1, opt->best_solution.fitness, end_time - start_time, smell_time, best_time);
    }

    // Update CPU-side population
    err = clEnqueueReadBuffer(queue, population_buffer, CL_TRUE, 0, swarm_size * dim * sizeof(float), population, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Error reading final population buffer: %d\n", err);
        goto cleanup;
    }
    err = clEnqueueReadBuffer(queue, fitness_buffer, CL_TRUE, 0, swarm_size * sizeof(float), fitness, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
...
cleanup:
    ...
    if (smell_kernel) clReleaseKernel(smell_kernel);
