...
void SaSA_init_cl(SaSACLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->update_kernel = clCreateKernel(cl_ctx->program, "update_salps", &err);
    check_cl_error(err, "clCreateKernel update_salps");
}
...
void SaSA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Salp Swarm Algorithm optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("SaSA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open SaSA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: SaSA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read SaSA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: SaSA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    SaSACLContext cl_ctx = {0};
    SaSA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    SaSAContext ctx = {0};
    SaSA_init_context(&ctx, opt, &cl_ctx);
    ...
    float c1_scale = SASA_C1_EXPONENT / opt->max_iter;
    ...
    while (func_count < max_evals) {
        ...
        float c1 = SASA_C1_FACTOR * exp(-powf(c1_scale * iteration, 2));
        ...
        clSetKernelArg(cl_ctx.update_kernel, 4, sizeof(cl_float), &c1);
        ...
        check_cl_error(err, "clEnqueueNDRangeKernel update_salps");
        clFinish(cl_ctx.queue);
    ...
    SaSA_cleanup_context(&ctx, &cl_ctx);
    SaSA_cleanup_cl(&cl_ctx);
