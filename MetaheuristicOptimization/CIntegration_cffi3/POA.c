...
// Preprocess kernel source (placeholder, as no specific preprocessing is needed for POA)
...
void POA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU POA optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("POA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open POA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: POA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read POA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: POA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    POACLContext cl_ctx = {0};
    POA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    POAContext ctx = {0};
    POA_init_context(&ctx, opt, &cl_ctx);
    ...
    while (func_count < max_evals) {
        ...
        cl_float step_size = POA_STEP_SIZE;
        ...
        clSetKernelArg(cl_ctx.update_kernel, 2, sizeof(cl_mem), &ctx.best_position);
        ...
        clSetKernelArg(cl_ctx.update_kernel, 5, sizeof(cl_float), &step_size);
    ...
    POA_cleanup_context(&ctx, &cl_ctx);
    POA_cleanup_cl(&cl_ctx);
