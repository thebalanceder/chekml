...
void ACO_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    ...
    for (int iter = 0; iter < opt->max_iter; iter++) {
        ...
        err = clEnqueueNDRangeKernel(opt->queue, construct_kernel, 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
        if (err != CL_SUCCESS) {
        ...
        global_work_size = ((global_work_size + local_work_size - 1) / local_work_size) * local_work_size;
        err = clEnqueueNDRangeKernel(opt->queue, evaporate_kernel, 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
        if (err != CL_SUCCESS) {
