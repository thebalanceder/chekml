/* CFO.c - GPU-Optimized Central Force Optimization */
#include "CFO.h"
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
// OpenCL kernel for CFO
static const char* cfo_kernel_source =
...
"    }\n"
"}\n"
"// Central force update\n"
"#define CFO_ALPHA 0.1f\n"
"__kernel void central_force_update(\n"
"    __global float* population,\n"
...
void CFO_optimize(Optimizer* opt, double (*objective_function)(double*)) {
    ...
    cl_kernel update_kernel = NULL, best_kernel = NULL;
    ...
    program = clCreateProgramWithSource(opt->context, 1, &cfo_kernel_source, NULL, &err);
    if (err != CL_SUCCESS || !program) {
        fprintf(stderr, "Error creating CFO program: %d\n", err);
        exit(EXIT_FAILURE);
    }
    ...
    if (err != CL_SUCCESS) {
        ...
        if (log) {
            ...
            fprintf(stderr, "Error building CFO program: %d\nBuild log:\n%s\n", err, log);
            free(log);
        }
    ...
    update_kernel = clCreateKernel(program, "central_force_update", &err);
    best_kernel = clCreateKernel(program, "find_best", &err);
    if (err != CL_SUCCESS || !init_kernel || !com_kernel || !finalize_com_kernel || 
        !update_kernel || !best_kernel) {
        fprintf(stderr, "Error creating CFO kernels: %d\n", err);
        goto cleanup;
    }
    ...
    if (err != CL_SUCCESS || !bounds_buffer || !com_buffer || !com_partial_buffer || 
        ...
        fprintf(stderr, "Error creating CFO buffers: %d\n", err);
        goto cleanup;
    }
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        err |= clSetKernelArg(update_kernel, 1, sizeof(cl_mem), &com_buffer);
        ...
        err |= clSetKernelArg(update_kernel, 5, dim * sizeof(float), NULL);
        ...
        // Debugging output every 10 iterations
        if (iter % 10 == 0) {
