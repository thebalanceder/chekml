...
static const char* gpc_kernel_source =
"#define G 9.8f\n"
"#define THETA (14.0f * 3.14159265358979323846f / 180.0f)\n"
"#define MU_MIN 0.3f\n"
"#define MU_MAX 0.5f\n"
"#define V_MIN 0.1f\n"
"#define V_MAX 2.0f\n"
...
"    int id = get_global_id(0);\n"
"    int local_id = get_local_id(0);\n"
"    if (id < population_size && id != best_idx) {\n"
"        uint local_seed = seed + id * dim;\n"
"        if (local_id < dim * 2) {\n"
"            local_bounds[local_id] = bounds[local_id];\n"
"        }\n"
"        barrier(CLK_LOCAL_MEM_FENCE);\n"
"        for (int d = 0; d < dim; d++) {\n"
"            float velocity = lcg_rand_float(&local_seed, V_MIN, V_MAX);\n"
"            float friction = lcg_rand_float(&local_seed, MU_MIN, MU_MAX);\n"
"            float d_move = (velocity * velocity) / (2.0f * G * (sin(THETA) + friction * cos(THETA)));\n"
"            float x_move = (velocity * velocity) / (2.0f * G * sin(THETA));\n"
"            float epsilon = lcg_rand_float(&local_seed, -0.5f * (V_MAX - V_MIN), 0.5f * (V_MAX - V_MIN));\n"
"            population[id * dim + d] = (population[id * dim + d] + d_move) * (x_move * epsilon);\n"
"            float min = local_bounds[2 * d];\n"
"            float max = local_bounds[2 * d + 1];\n"
...
void GPC_optimize(Optimizer* opt, ObjectiveFunction objective_function) {
    ...
    const int population_size = GPC_POPULATION_SIZE;
    ...
    // Create program
    program = clCreateProgramWithSource(context, 1, &gpc_kernel_source, NULL, &err);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Error creating GPC program: %d\n", err);
    ...
    if (err != CL_SUCCESS) {
        ...
        if (log) {
            ...
            fprintf(stderr, "Error building GPC program: %d\nBuild log:\n%s\n", err, log);
            free(log);
        }
    ...
    if (err != CL_SUCCESS || !init_kernel || !update_kernel || !best_kernel) {
        fprintf(stderr, "Error creating GPC kernels: %d\n", err);
        goto cleanup;
    }
    ...
    size_t local_work_size = max_work_group_size < GPC_LOCAL_WORK_SIZE ? max_work_group_size : GPC_LOCAL_WORK_SIZE;
    ...
    if (err != CL_SUCCESS || !bounds_buffer || !population_buffer || !fitness_buffer || 
        ...
        fprintf(stderr, "Error creating GPC buffers: %d\n", err);
        goto cleanup;
    }
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        err |= clSetKernelArg(update_kernel, 2, sizeof(cl_mem), &best_position_buffer);
        err |= clSetKernelArg(update_kernel, 3, sizeof(int), &dim);
        err |= clSetKernelArg(update_kernel, 4, sizeof(int), &population_size);
        err |= clSetKernelArg(update_kernel, 5, sizeof(int), &best_idx);
        ...
        double update_time = 0, best_time = 0;
        if (events[0]) {
        ...
        printf("GPC|%5d -----> %9.16f | Total: %.3f ms | Update: %.3f ms | Best: %.3f ms\n", 
               iter + 1, opt->best_solution.fitness, end_time - start_time, update_time, best_time);
    }
