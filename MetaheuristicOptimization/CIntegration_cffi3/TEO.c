...
// Preprocess kernel source to fix address space (if needed)
...
void TEO_init_cl(TEOCLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->perturb_kernel = clCreateKernel(cl_ctx->program, "perturb_and_accept", &err);
    check_cl_error(err, "clCreateKernel perturb_and_accept");
}

void TEO_cleanup_cl(TEOCLContext *cl_ctx) {
    if (cl_ctx->perturb_kernel) clReleaseKernel(cl_ctx->perturb_kernel);
...
void TEO_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Thermal Equilibrium Optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("TEO.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open TEO.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: TEO.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read TEO.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: TEO.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    TEOCLContext cl_ctx = {0};
    TEO_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    TEOContext ctx = {0};
    TEO_init_context(&ctx, opt, &cl_ctx);

    int pop_size = opt->population_size;
    int dim = opt->dim;
    int func_count = 0;
    int iteration = 0;
    float temperature = TEO_INITIAL_TEMPERATURE;
    ...
    while (func_count < max_evals && temperature >= TEO_FINAL_TEMPERATURE) {
        iteration++;
        printf("Iteration %d, Temperature = %f\n", iteration, temperature);

        // Perturb and accept solutions
        cl_event perturb_event;
        clSetKernelArg(cl_ctx.perturb_kernel, 0, sizeof(cl_mem), &ctx.population);
        clSetKernelArg(cl_ctx.perturb_kernel, 1, sizeof(cl_mem), &ctx.fitness);
        clSetKernelArg(cl_ctx.perturb_kernel, 2, sizeof(cl_mem), &ctx.bounds);
        clSetKernelArg(cl_ctx.perturb_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
        clSetKernelArg(cl_ctx.perturb_kernel, 4, sizeof(cl_float), &temperature);
        clSetKernelArg(cl_ctx.perturb_kernel, 5, sizeof(cl_mem), &ctx.best_position);
        clSetKernelArg(cl_ctx.perturb_kernel, 6, sizeof(cl_float), &ctx.best_fitness);
        clSetKernelArg(cl_ctx.perturb_kernel, 7, sizeof(cl_int), &dim);
        clSetKernelArg(cl_ctx.perturb_kernel, 8, sizeof(cl_int), &pop_size);

        err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.perturb_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &perturb_event);
        check_cl_error(err, "clEnqueueNDRangeKernel perturb_and_accept");
        clFinish(cl_ctx.queue);

        double perturb_time = 0.0;
        if (perturb_event) {
            err = clGetEventProfilingInfo(perturb_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            if (err == CL_SUCCESS) {
                err = clGetEventProfilingInfo(perturb_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                if (err == CL_SUCCESS) {
                    perturb_time = (end - start) / 1e6;
                    prof.perturb_time += perturb_time;
                }
            }
            clReleaseEvent(perturb_event);
        }
    ...
    TEO_cleanup_context(&ctx, &cl_ctx);
    TEO_cleanup_cl(&cl_ctx);
