...
void SEO_init_cl(SEOCLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->update_kernel = clCreateKernel(cl_ctx->program, "social_engineering_update", &err);
    check_cl_error(err, "clCreateKernel social_engineering_update");
}
...
void SEO_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Social Engineering Optimizer...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("SEO.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open SEO.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: SEO.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read SEO.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: SEO.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    SEOCLContext cl_ctx = {0};
    SEO_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    SEOContext ctx = {0};
    SEO_init_context(&ctx, opt, &cl_ctx);
    ...
    while (func_count < max_evals) {
        ...
        clSetKernelArg(cl_ctx.update_kernel, 4, sizeof(cl_float), &scale);
        ...
        check_cl_error(err, "clEnqueueNDRangeKernel social_engineering_update");
        clFinish(cl_ctx.queue);
        ...
        printf("Profiling (ms): Social Engineering Update = %.3f\n", update_time);
    }

    // Print profiling summary
    printf("\nProfiling Summary (Total ms):\n");
    printf("Initialize Population: %.3f\n", prof.init_pop_time);
    printf("Social Engineering Update: %.3f\n", prof.update_time);
    ...
    SEO_cleanup_context(&ctx, &cl_ctx);
    SEO_cleanup_cl(&cl_ctx);
