#include "WO.h"
#include <string.h>
#include <time.h>
#include <ctype.h>
#include <stdlib.h>
#include <math.h>

// Error checking utility
void check_cl_error(cl_int err, const char *msg);

// Preprocess kernel source (placeholder, as in FirefA)
char *preprocess_kernel_source(const char *source);

void WO_init_cl(WOCLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    cl_int err;

    // Validate inputs
    if (!opt || !opt->context || !opt->queue || !opt->device) {
        fprintf(stderr, "Error: Invalid Optimizer structure or null pointers\n");
        exit(1);
    }
    if (!kernel_source || strlen(kernel_source) == 0) {
        fprintf(stderr, "Error: Kernel source is empty or null\n");
        exit(1);
    }

    // Create command queue with profiling
    cl_command_queue_properties props[] = {
        CL_QUEUE_PROPERTIES, CL_QUEUE_PROFILING_ENABLE,
        0
    };
    cl_ctx->queue = clCreateCommandQueueWithProperties(opt->context, opt->device, props, &err);
    check_cl_error(err, "clCreateCommandQueueWithProperties");
    cl_ctx->owns_queue = CL_TRUE;

    // Log queue properties
    cl_command_queue_properties actual_props;
    err = clGetCommandQueueInfo(cl_ctx->queue, CL_QUEUE_PROPERTIES, sizeof(cl_command_queue_properties), &actual_props, NULL);
    check_cl_error(err, "clGetCommandQueueInfo");
    printf("Queue profiling enabled: %s\n", (actual_props & CL_QUEUE_PROFILING_ENABLE) ? "Yes" : "No");

    // Preprocess kernel source
    char *processed_source = preprocess_kernel_source(kernel_source);

    // Reuse OpenCL context and device
    cl_ctx->context = opt->context;
    cl_ctx->device = opt->device;

    // Create and build program
    cl_ctx->program = clCreateProgramWithSource(cl_ctx->context, 1, (const char **)&processed_source, NULL, &err);
    check_cl_error(err, "clCreateProgramWithSource");

    const char *build_options = "-cl-std=CL1.2 -cl-mad-enable";
    err = clBuildProgram(cl_ctx->program, 1, &cl_ctx->device, build_options, NULL, NULL);
    if (err != CL_SUCCESS) {
        size_t log_size;
        clGetProgramBuildInfo(cl_ctx->program, cl_ctx->device, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);
        char *log = (char *)malloc(log_size + 1);
        if (log) {
            clGetProgramBuildInfo(cl_ctx->program, cl_ctx->device, CL_PROGRAM_BUILD_LOG, log_size, log, NULL);
            log[log_size] = '\0';
            fprintf(stderr, "Error building program: %d\nDetailed Build Log:\n%s\n", err, log);
            free(log);
        } else {
            fprintf(stderr, "Error building program: %d\nFailed to allocate memory for build log\n", err);
        }
        clReleaseProgram(cl_ctx->program);
        free(processed_source);
        exit(1);
    }

    free(processed_source);

    // Create kernels
    cl_ctx->init_pop_kernel = clCreateKernel(cl_ctx->program, "initialize_population", &err);
    check_cl_error(err, "clCreateKernel init_pop");
    cl_ctx->migration_kernel = clCreateKernel(cl_ctx->program, "migration_phase", &err);
    check_cl_error(err, "clCreateKernel migration_phase");
    cl_ctx->male_update_kernel = clCreateKernel(cl_ctx->program, "male_position_update", &err);
    check_cl_error(err, "clCreateKernel male_position_update");
    cl_ctx->female_update_kernel = clCreateKernel(cl_ctx->program, "female_position_update", &err);
    check_cl_error(err, "clCreateKernel female_position_update");
    cl_ctx->child_update_kernel = clCreateKernel(cl_ctx->program, "child_position_update", &err);
    check_cl_error(err, "clCreateKernel child_position_update");
    cl_ctx->pos_adjust_kernel = clCreateKernel(cl_ctx->program, "position_adjustment_phase", &err);
    check_cl_error(err, "clCreateKernel position_adjustment_phase");
    cl_ctx->exploitation_kernel = clCreateKernel(cl_ctx->program, "exploitation_phase", &err);
    check_cl_error(err, "clCreateKernel exploitation_phase");
}

void WO_cleanup_cl(WOCLContext *cl_ctx) {
    if (cl_ctx->exploitation_kernel) clReleaseKernel(cl_ctx->exploitation_kernel);
    if (cl_ctx->pos_adjust_kernel) clReleaseKernel(cl_ctx->pos_adjust_kernel);
    if (cl_ctx->child_update_kernel) clReleaseKernel(cl_ctx->child_update_kernel);
    if (cl_ctx->female_update_kernel) clReleaseKernel(cl_ctx->female_update_kernel);
    if (cl_ctx->male_update_kernel) clReleaseKernel(cl_ctx->male_update_kernel);
    if (cl_ctx->migration_kernel) clReleaseKernel(cl_ctx->migration_kernel);
    if (cl_ctx->init_pop_kernel) clReleaseKernel(cl_ctx->init_pop_kernel);
    if (cl_ctx->program) clReleaseProgram(cl_ctx->program);
    if (cl_ctx->queue && cl_ctx->owns_queue) clReleaseCommandQueue(cl_ctx->queue);
}

void WO_init_context(WOContext *ctx, Optimizer *opt, WOCLContext *cl_ctx) {
    cl_int err;
    ctx->best_fitness = INFINITY;
    ctx->second_best_fitness = INFINITY;

    int pop_size = opt->population_size;
    int dim = opt->dim;

    // Initialize counts
    ctx->male_count = (cl_int)(pop_size * WO_FEMALE_PROPORTION);
    ctx->female_count = ctx->male_count;
    ctx->child_count = pop_size - ctx->male_count - ctx->female_count;

    // Compute levy sigma
    float levy_beta = WO_LEVY_BETA;
    ctx->levy_sigma = pow((tgamma(1 + levy_beta) * sin(M_PI * levy_beta / 2) /
                          (tgamma((1 + levy_beta) / 2) * levy_beta * pow(2, (levy_beta - 1) / 2))), 1 / levy_beta);

    // Allocate OpenCL buffers
    ctx->population = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, pop_size * dim * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer population");
    ctx->fitness = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, pop_size * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer fitness");
    ctx->best_position = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, dim * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer best_position");
    ctx->second_best = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, dim * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer second_best");
    ctx->bounds = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, dim * 2 * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer bounds");
    ctx->random_seeds = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, pop_size * sizeof(cl_uint), NULL, &err);
    check_cl_error(err, "clCreateBuffer random_seeds");
    ctx->temp_indices = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, pop_size * sizeof(cl_int), NULL, &err);
    check_cl_error(err, "clCreateBuffer temp_indices");
    ctx->temp_array1 = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, dim * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer temp_array1");
    ctx->temp_array2 = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, dim * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer temp_array2");

    // Initialize random seeds
    cl_uint *seeds = (cl_uint *)malloc(pop_size * sizeof(cl_uint));
    srand(time(NULL));
    for (int i = 0; i < pop_size; i++) seeds[i] = rand();
    err = clEnqueueWriteBuffer(cl_ctx->queue, ctx->random_seeds, CL_TRUE, 0, pop_size * sizeof(cl_uint), seeds, 0, NULL, NULL);
    check_cl_error(err, "clEnqueueWriteBuffer seeds");
    free(seeds);

    // Initialize bounds
    float *bounds_float = (float *)malloc(dim * 2 * sizeof(float));
    for (int i = 0; i < dim * 2; i++) bounds_float[i] = (float)opt->bounds[i];
    err = clEnqueueWriteBuffer(cl_ctx->queue, ctx->bounds, CL_TRUE, 0, dim * 2 * sizeof(cl_float), bounds_float, 0, NULL, NULL);
    check_cl_error(err, "clEnqueueWriteBuffer bounds");
    free(bounds_float);

    // Initialize temp indices
    cl_int *indices = (cl_int *)malloc(pop_size * sizeof(cl_int));
    for (int i = 0; i < pop_size; i++) indices[i] = i;
    err = clEnqueueWriteBuffer(cl_ctx->queue, ctx->temp_indices, CL_TRUE, 0, pop_size * sizeof(cl_int), indices, 0, NULL, NULL);
    check_cl_error(err, "clEnqueueWriteBuffer temp_indices");
    free(indices);
}

void WO_cleanup_context(WOContext *ctx, WOCLContext *cl_ctx) {
    if (ctx->temp_array2) clReleaseMemObject(ctx->temp_array2);
    if (ctx->temp_array1) clReleaseMemObject(ctx->temp_array1);
    if (ctx->temp_indices) clReleaseMemObject(ctx->temp_indices);
    if (ctx->random_seeds) clReleaseMemObject(ctx->random_seeds);
    if (ctx->bounds) clReleaseMemObject(ctx->bounds);
    if (ctx->second_best) clReleaseMemObject(ctx->second_best);
    if (ctx->best_position) clReleaseMemObject(ctx->best_position);
    if (ctx->fitness) clReleaseMemObject(ctx->fitness);
    if (ctx->population) clReleaseMemObject(ctx->population);
}

void WO_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Whale Optimization Algorithm...\n");

    // Validate inputs
    if (!opt || !opt->context || !opt->queue || !opt->device || 
        !opt->population_buffer || !opt->fitness_buffer || 
        !opt->bounds || !opt->best_solution.position) {
        fprintf(stderr, "Error: Invalid Optimizer structure or null pointers\n");
        exit(1);
    }

    // Read kernel source
    FILE *fp = fopen("WO.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open WO.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: WO.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    char *source_str = (char *)malloc(source_size + 1);
    if (!source_str) {
        fprintf(stderr, "Error: Memory allocation failed for kernel source\n");
        fclose(fp);
        exit(1);
    }
    size_t read_size = fread(source_str, 1, source_size, fp);
    fclose(fp);
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read WO.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    source_str[source_size] = '\0';

    int valid_content = 0;
    for (size_t i = 0; i < source_size; i++) {
        if (!isspace(source_str[i]) && isprint(source_str[i])) {
            valid_content = 1;
            break;
        }
    }
    if (!valid_content) {
        fprintf(stderr, "Error: WO.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    WOCLContext cl_ctx = {0};
    WO_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    WOContext ctx = {0};
    WO_init_context(&ctx, opt, &cl_ctx);

    int pop_size = opt->population_size;
    int dim = opt->dim;
    int func_count = 0;
    int iteration = 0;

    // Compute max evaluations
    int max_evals = opt->max_iter > 0 ? opt->max_iter * pop_size : MAX_EVALS_DEFAULT;
    printf("Maximum function evaluations: %d\n", max_evals);

    // Profiling data
    typedef struct {
        double init_pop_time;
        double migration_time;
        double male_update_time;
        double female_update_time;
        double child_update_time;
        double pos_adjust_time;
        double exploitation_time;
        double read_time;
        double write_time;
        int count;
    } ProfilingData;
    ProfilingData prof = {0};

    cl_int err;
    cl_mem best_fitness_buf = clCreateBuffer(cl_ctx.context, CL_MEM_READ_WRITE, sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer best_fitness");
    cl_mem second_best_fitness_buf = clCreateBuffer(cl_ctx.context, CL_MEM_READ_WRITE, sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer second_best_fitness");

    // Initialize population
    cl_event init_event;
    clSetKernelArg(cl_ctx.init_pop_kernel, 0, sizeof(cl_mem), &ctx.population);
    clSetKernelArg(cl_ctx.init_pop_kernel, 1, sizeof(cl_mem), &ctx.fitness);
    clSetKernelArg(cl_ctx.init_pop_kernel, 2, sizeof(cl_mem), &ctx.bounds);
    clSetKernelArg(cl_ctx.init_pop_kernel, 3, sizeof(cl_mem), &ctx.best_position);
    clSetKernelArg(cl_ctx.init_pop_kernel, 4, sizeof(cl_mem), &best_fitness_buf);
    clSetKernelArg(cl_ctx.init_pop_kernel, 5, sizeof(cl_mem), &ctx.random_seeds);
    clSetKernelArg(cl_ctx.init_pop_kernel, 6, sizeof(cl_int), &dim);
    clSetKernelArg(cl_ctx.init_pop_kernel, 7, sizeof(cl_int), &pop_size);

    size_t global_work_size = pop_size;
    err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.init_pop_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &init_event);
    check_cl_error(err, "clEnqueueNDRangeKernel init_pop");
    clFinish(cl_ctx.queue);

    cl_ulong start, end;
    if (init_event) {
        err = clGetEventProfilingInfo(init_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(init_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.init_pop_time += (end - start) / 1e6; // Convert ns to ms
            }
        }
        clReleaseEvent(init_event);
    }

    // Evaluate initial population on CPU
    cl_event read_event, write_event;
    float *positions = (float *)malloc(pop_size * dim * sizeof(float));
    err = clEnqueueReadBuffer(cl_ctx.queue, ctx.population, CL_TRUE, 0, pop_size * dim * sizeof(cl_float), positions, 0, NULL, &read_event);
    check_cl_error(err, "clEnqueueReadBuffer population");
    float *fitness = (float *)malloc(pop_size * sizeof(float));
    for (int i = 0; i < pop_size; i++) {
        double *pos = (double *)malloc(dim * sizeof(double));
        for (int j = 0; j < dim; j++) pos[j] = positions[i * dim + j];
        fitness[i] = (float)objective_function(pos);
        free(pos);
    }
    err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.fitness, CL_TRUE, 0, pop_size * sizeof(cl_float), fitness, 0, NULL, &write_event);
    check_cl_error(err, "clEnqueueWriteBuffer fitness");
    free(positions);

    if (read_event) {
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.read_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(read_event);
    }

    if (write_event) {
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.write_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(write_event);
    }

    // Find best and second-best solutions
    float best_fitness = fitness[0];
    int best_idx = 0;
    float second_best_fitness = INFINITY;
    int second_best_idx = 0;
    for (int i = 1; i < pop_size; i++) {
        if (fitness[i] < best_fitness) {
            second_best_fitness = best_fitness;
            second_best_idx = best_idx;
            best_fitness = fitness[i];
            best_idx = i;
        } else if (fitness[i] < second_best_fitness && fitness[i] != best_fitness) {
            second_best_fitness = fitness[i];
            second_best_idx = i;
        }
    }
    ctx.best_fitness = best_fitness;
    ctx.second_best_fitness = second_best_fitness;
    free(fitness);

    positions = (float *)malloc(dim * sizeof(float));
    err = clEnqueueReadBuffer(cl_ctx.queue, ctx.population, CL_TRUE, best_idx * dim * sizeof(cl_float), dim * sizeof(cl_float), positions, 0, NULL, &read_event);
    check_cl_error(err, "clEnqueueReadBuffer best_pos");
    err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.best_position, CL_TRUE, 0, dim * sizeof(cl_float), positions, 0, NULL, &write_event);
    check_cl_error(err, "clEnqueueWriteBuffer best_pos");

    if (read_event) {
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.read_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(read_event);
    }

    if (write_event) {
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.write_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(write_event);
    }

    err = clEnqueueWriteBuffer(cl_ctx.queue, best_fitness_buf, CL_TRUE, 0, sizeof(cl_float), &best_fitness, 0, NULL, &write_event);
    check_cl_error(err, "clEnqueueWriteBuffer best_fitness");

    if (write_event) {
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.write_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(write_event);
    }

    positions = (float *)malloc(dim * sizeof(float));
    err = clEnqueueReadBuffer(cl_ctx.queue, ctx.population, CL_TRUE, second_best_idx * dim * sizeof(cl_float), dim * sizeof(cl_float), positions, 0, NULL, &read_event);
    check_cl_error(err, "clEnqueueReadBuffer second_best_pos");
    err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.second_best, CL_TRUE, 0, dim * sizeof(cl_float), positions, 0, NULL, &write_event);
    check_cl_error(err, "clEnqueueWriteBuffer second_best_pos");
    free(positions);

    if (read_event) {
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.read_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(read_event);
    }

    if (write_event) {
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.write_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(write_event);
    }

    err = clEnqueueWriteBuffer(cl_ctx.queue, second_best_fitness_buf, CL_TRUE, 0, sizeof(cl_float), &second_best_fitness, 0, NULL, &write_event);
    check_cl_error(err, "clEnqueueWriteBuffer second_best_fitness");

    if (write_event) {
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.write_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(write_event);
    }

    // Main optimization loop
    while (func_count < max_evals) {
        iteration++;
        printf("Iteration %d\n", iteration);

        // Update parameters
        float alpha = 1.0f - (float)iteration / opt->max_iter;
        float beta = 1.0f - 1.0f / (1.0f + exp((0.5f * opt->max_iter - iteration) / opt->max_iter * 10.0f));
        float A = 2.0f * alpha;
        float r1 = (float)(rand() / (double)RAND_MAX);
        float R = 2.0f * r1 - 1.0f;
        float danger_signal = A * R;
        float safety_signal = (float)(rand() / (double)RAND_MAX);
        float r3 = (float)(rand() / (double)RAND_MAX);

        cl_event phase_event;
        double phase_time = 0.0;

        if (fabs(danger_signal) >= 1.0f) {
            // Migration phase
            clSetKernelArg(cl_ctx.migration_kernel, 0, sizeof(cl_mem), &ctx.population);
            clSetKernelArg(cl_ctx.migration_kernel, 1, sizeof(cl_mem), &ctx.temp_indices);
            clSetKernelArg(cl_ctx.migration_kernel, 2, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.migration_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
            clSetKernelArg(cl_ctx.migration_kernel, 4, sizeof(cl_float), &beta);
            clSetKernelArg(cl_ctx.migration_kernel, 5, sizeof(cl_float), &r3);
            clSetKernelArg(cl_ctx.migration_kernel, 6, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.migration_kernel, 7, sizeof(cl_int), &pop_size);

            err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.migration_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &phase_event);
            check_cl_error(err, "clEnqueueNDRangeKernel migration_phase");
            clFinish(cl_ctx.queue);

            if (phase_event) {
                err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        phase_time = (end - start) / 1e6;
                        prof.migration_time += phase_time;
                    }
                }
                clReleaseEvent(phase_event);
            }
        } else if (safety_signal >= 0.5f) {
            // Male position update
            clSetKernelArg(cl_ctx.male_update_kernel, 0, sizeof(cl_mem), &ctx.population);
            clSetKernelArg(cl_ctx.male_update_kernel, 1, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.male_update_kernel, 2, sizeof(cl_int), &ctx.male_count);
            clSetKernelArg(cl_ctx.male_update_kernel, 3, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.male_update_kernel, 4, sizeof(cl_int), &pop_size);

            err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.male_update_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &phase_event);
            check_cl_error(err, "clEnqueueNDRangeKernel male_position_update");
            clFinish(cl_ctx.queue);

            if (phase_event) {
                err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        phase_time = (end - start) / 1e6;
                        prof.male_update_time += phase_time;
                    }
                }
                clReleaseEvent(phase_event);
            }

            // Female position update
            clSetKernelArg(cl_ctx.female_update_kernel, 0, sizeof(cl_mem), &ctx.population);
            clSetKernelArg(cl_ctx.female_update_kernel, 1, sizeof(cl_mem), &ctx.best_position);
            clSetKernelArg(cl_ctx.female_update_kernel, 2, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.female_update_kernel, 3, sizeof(cl_float), &alpha);
            clSetKernelArg(cl_ctx.female_update_kernel, 4, sizeof(cl_int), &ctx.male_count);
            clSetKernelArg(cl_ctx.female_update_kernel, 5, sizeof(cl_int), &ctx.female_count);
            clSetKernelArg(cl_ctx.female_update_kernel, 6, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.female_update_kernel, 7, sizeof(cl_int), &pop_size);

            err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.female_update_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &phase_event);
            check_cl_error(err, "clEnqueueNDRangeKernel female_position_update");
            clFinish(cl_ctx.queue);

            if (phase_event) {
                err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        phase_time = (end - start) / 1e6;
                        prof.female_update_time += phase_time;
                    }
                }
                clReleaseEvent(phase_event);
            }

            // Child position update
            clSetKernelArg(cl_ctx.child_update_kernel, 0, sizeof(cl_mem), &ctx.population);
            clSetKernelArg(cl_ctx.child_update_kernel, 1, sizeof(cl_mem), &ctx.best_position);
            clSetKernelArg(cl_ctx.child_update_kernel, 2, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.child_update_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
            clSetKernelArg(cl_ctx.child_update_kernel, 4, sizeof(cl_mem), &ctx.temp_array1);
            clSetKernelArg(cl_ctx.child_update_kernel, 5, sizeof(cl_mem), &ctx.temp_array2);
            clSetKernelArg(cl_ctx.child_update_kernel, 6, sizeof(cl_float), &ctx.levy_sigma);
            clSetKernelArg(cl_ctx.child_update_kernel, 7, sizeof(cl_int), &ctx.child_count);
            clSetKernelArg(cl_ctx.child_update_kernel, 8, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.child_update_kernel, 9, sizeof(cl_int), &pop_size);

            err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.child_update_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &phase_event);
            check_cl_error(err, "clEnqueueNDRangeKernel child_position_update");
            clFinish(cl_ctx.queue);

            if (phase_event) {
                err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        phase_time = (end - start) / 1e6;
                        prof.child_update_time += phase_time;
                    }
                }
                clReleaseEvent(phase_event);
            }
        } else if (fabs(danger_signal) >= 0.5f) {
            // Position adjustment phase
            clSetKernelArg(cl_ctx.pos_adjust_kernel, 0, sizeof(cl_mem), &ctx.population);
            clSetKernelArg(cl_ctx.pos_adjust_kernel, 1, sizeof(cl_mem), &ctx.best_position);
            clSetKernelArg(cl_ctx.pos_adjust_kernel, 2, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.pos_adjust_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
            clSetKernelArg(cl_ctx.pos_adjust_kernel, 4, sizeof(cl_float), &R);
            clSetKernelArg(cl_ctx.pos_adjust_kernel, 5, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.pos_adjust_kernel, 6, sizeof(cl_int), &pop_size);

            err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.pos_adjust_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &phase_event);
            check_cl_error(err, "clEnqueueNDRangeKernel position_adjustment_phase");
            clFinish(cl_ctx.queue);

            if (phase_event) {
                err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        phase_time = (end - start) / 1e6;
                        prof.pos_adjust_time += phase_time;
                    }
                }
                clReleaseEvent(phase_event);
            }
        } else {
            // Exploitation phase
            clSetKernelArg(cl_ctx.exploitation_kernel, 0, sizeof(cl_mem), &ctx.population);
            clSetKernelArg(cl_ctx.exploitation_kernel, 1, sizeof(cl_mem), &ctx.best_position);
            clSetKernelArg(cl_ctx.exploitation_kernel, 2, sizeof(cl_mem), &ctx.second_best);
            clSetKernelArg(cl_ctx.exploitation_kernel, 3, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.exploitation_kernel, 4, sizeof(cl_mem), &ctx.random_seeds);
            clSetKernelArg(cl_ctx.exploitation_kernel, 5, sizeof(cl_float), &beta);
            clSetKernelArg(cl_ctx.exploitation_kernel, 6, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.exploitation_kernel, 7, sizeof(cl_int), &pop_size);

            err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.exploitation_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &phase_event);
            check_cl_error(err, "clEnqueueNDRangeKernel exploitation_phase");
            clFinish(cl_ctx.queue);

            if (phase_event) {
                err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(phase_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        phase_time = (end - start) / 1e6;
                        prof.exploitation_time += phase_time;
                    }
                }
                clReleaseEvent(phase_event);
            }
        }

        // Evaluate population on CPU
        positions = (float *)malloc(pop_size * dim * sizeof(float));
        err = clEnqueueReadBuffer(cl_ctx.queue, ctx.population, CL_TRUE, 0, pop_size * dim * sizeof(cl_float), positions, 0, NULL, &read_event);
        check_cl_error(err, "clEnqueueReadBuffer population");
        fitness = (float *)malloc(pop_size * sizeof(float));
        for (int i = 0; i < pop_size; i++) {
            double *pos = (double *)malloc(dim * sizeof(double));
            for (int j = 0; j < dim; j++) pos[j] = positions[i * dim + j];
            fitness[i] = (float)objective_function(pos);
            free(pos);
        }
        err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.fitness, CL_TRUE, 0, pop_size * sizeof(cl_float), fitness, 0, NULL, &write_event);
        check_cl_error(err, "clEnqueueWriteBuffer fitness");
        free(positions);
        func_count += pop_size;

        if (read_event) {
            err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            if (err == CL_SUCCESS) {
                err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                if (err == CL_SUCCESS) {
                    prof.read_time += (end - start) / 1e6;
                }
            }
            clReleaseEvent(read_event);
        }

        if (write_event) {
            err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            if (err == CL_SUCCESS) {
                err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                if (err == CL_SUCCESS) {
                    prof.write_time += (end - start) / 1e6;
                }
            }
            clReleaseEvent(write_event);
        }

        // Update best and second-best solutions
        float new_best = fitness[0];
        int new_best_idx = 0;
        float new_second_best = INFINITY;
        int new_second_best_idx = 0;
        for (int i = 1; i < pop_size; i++) {
            if (fitness[i] < new_best) {
                new_second_best = new_best;
                new_second_best_idx = new_best_idx;
                new_best = fitness[i];
                new_best_idx = i;
            } else if (fitness[i] < new_second_best && fitness[i] != new_best) {
                new_second_best = fitness[i];
                new_second_best_idx = i;
            }
        }

        if (new_best < ctx.best_fitness) {
            ctx.best_fitness = new_best;
            positions = (float *)malloc(dim * sizeof(float));
            err = clEnqueueReadBuffer(cl_ctx.queue, ctx.population, CL_TRUE, new_best_idx * dim * sizeof(cl_float), dim * sizeof(cl_float), positions, 0, NULL, &read_event);
            check_cl_error(err, "clEnqueueReadBuffer best_pos");
            err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.best_position, CL_TRUE, 0, dim * sizeof(cl_float), positions, 0, NULL, &write_event);
            check_cl_error(err, "clEnqueueWriteBuffer best_pos");

            if (read_event) {
                err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        prof.read_time += (end - start) / 1e6;
                    }
                }
                clReleaseEvent(read_event);
            }

            if (write_event) {
                err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        prof.write_time += (end - start) / 1e6;
                    }
                }
                clReleaseEvent(write_event);
            }

            err = clEnqueueWriteBuffer(cl_ctx.queue, best_fitness_buf, CL_TRUE, 0, sizeof(cl_float), &new_best, 0, NULL, &write_event);
            check_cl_error(err, "clEnqueueWriteBuffer best_fitness");

            if (write_event) {
                err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        prof.write_time += (end - start) / 1e6;
                    }
                }
                clReleaseEvent(write_event);
            }
        }

        if (new_second_best < ctx.second_best_fitness) {
            ctx.second_best_fitness = new_second_best;
            positions = (float *)malloc(dim * sizeof(float));
            err = clEnqueueReadBuffer(cl_ctx.queue, ctx.population, CL_TRUE, new_second_best_idx * dim * sizeof(cl_float), dim * sizeof(cl_float), positions, 0, NULL, &read_event);
            check_cl_error(err, "clEnqueueReadBuffer second_best_pos");
            err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.second_best, CL_TRUE, 0, dim * sizeof(cl_float), positions, 0, NULL, &write_event);
            check_cl_error(err, "clEnqueueWriteBuffer second_best_pos");
            free(positions);

            if (read_event) {
                err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        prof.read_time += (end - start) / 1e6;
                    }
                }
                clReleaseEvent(read_event);
            }

            if (write_event) {
                err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        prof.write_time += (end - start) / 1e6;
                    }
                }
                clReleaseEvent(write_event);
            }

            err = clEnqueueWriteBuffer(cl_ctx.queue, second_best_fitness_buf, CL_TRUE, 0, sizeof(cl_float), &new_second_best, 0, NULL, &write_event);
            check_cl_error(err, "clEnqueueWriteBuffer second_best_fitness");

            if (write_event) {
                err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                if (err == CL_SUCCESS) {
                    err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                    if (err == CL_SUCCESS) {
                        prof.write_time += (end - start) / 1e6;
                    }
                }
                clReleaseEvent(write_event);
            }
        }
        free(fitness);

        prof.count++;
        printf("Iteration %d: Function Evaluations = %d, Best Fitness = %f\n", iteration, func_count, ctx.best_fitness);
        printf("Profiling (ms): Phase Time = %.3f\n", phase_time);
    }

    // Print profiling summary
    printf("\nProfiling Summary (Total ms):\n");
    printf("Initialize Population: %.3f\n", prof.init_pop_time);
    printf("Migration Phase: %.3f\n", prof.migration_time);
    printf("Male Update: %.3f\n", prof.male_update_time);
    printf("Female Update: %.3f\n", prof.female_update_time);
    printf("Child Update: %.3f\n", prof.child_update_time);
    printf("Position Adjustment: %.3f\n", prof.pos_adjust_time);
    printf("Exploitation Phase: %.3f\n", prof.exploitation_time);
    printf("Data Reads: %.3f\n", prof.read_time);
    printf("Data Writes: %.3f\n", prof.write_time);
    if (prof.count > 0) {
        printf("Average per Iteration (ms):\n");
        printf("Migration Phase: %.3f\n", prof.migration_time / prof.count);
        printf("Male Update: %.3f\n", prof.male_update_time / prof.count);
        printf("Female Update: %.3f\n", prof.female_update_time / prof.count);
        printf("Child Update: %.3f\n", prof.child_update_time / prof.count);
        printf("Position Adjustment: %.3f\n", prof.pos_adjust_time / prof.count);
        printf("Exploitation Phase: %.3f\n", prof.exploitation_time / prof.count);
        printf("Data Reads: %.3f\n", prof.read_time / prof.count);
        printf("Data Writes: %.3f\n", prof.write_time / prof.count);
    }

    // Copy best solution to optimizer
    float *best_pos = (float *)malloc(dim * sizeof(float));
    err = clEnqueueReadBuffer(cl_ctx.queue, ctx.best_position, CL_TRUE, 0, dim * sizeof(cl_float), best_pos, 0, NULL, &read_event);
    check_cl_error(err, "clEnqueueReadBuffer final best_pos");
    opt->best_solution.fitness = ctx.best_fitness;
    for (int i = 0; i < dim; i++) opt->best_solution.position[i] = best_pos[i];
    free(best_pos);

    if (read_event) {
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        if (err == CL_SUCCESS) {
            err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            if (err == CL_SUCCESS) {
                prof.read_time += (end - start) / 1e6;
            }
        }
        clReleaseEvent(read_event);
    }

    // Cleanup
    clReleaseMemObject(best_fitness_buf);
    clReleaseMemObject(second_best_fitness_buf);
    WO_cleanup_context(&ctx, &cl_ctx);
    WO_cleanup_cl(&cl_ctx);

    printf("Optimization completed\n");
}
