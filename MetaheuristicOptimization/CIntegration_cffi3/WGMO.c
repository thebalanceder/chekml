...
// Error checking utility (same as FirefA)
void check_cl_error(cl_int err, const char *msg);

// Preprocess kernel source (same as FirefA)
...
void WGMO_init_cl(WGMOCLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->update_kernel = clCreateKernel(cl_ctx->program, "update_geese", &err);
    check_cl_error(err, "clCreateKernel update_geese");
}
...
void WGMO_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Wild Geese Migration Optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("WGMO.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open WGMO.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: WGMO.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read WGMO.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: WGMO.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    WGMOCLContext cl_ctx = {0};
    WGMO_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    WGMOContext ctx = {0};
    WGMO_init_context(&ctx, opt, &cl_ctx);
    ...
    while (func_count < max_evals) {
        ...
        check_cl_error(err, "clEnqueueNDRangeKernel update_geese");
        clFinish(cl_ctx.queue);
    ...
    WGMO_cleanup_context(&ctx, &cl_ctx);
    WGMO_cleanup_cl(&cl_ctx);
