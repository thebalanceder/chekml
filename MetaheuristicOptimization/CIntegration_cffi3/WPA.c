...
// Preprocess kernel source (placeholder, copies source as is)
...
void WPA_init_cl(WPACLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->stats_kernel = clCreateKernel(cl_ctx->program, "compute_stats", &err);
    check_cl_error(err, "clCreateKernel compute_stats");
    cl_ctx->exploration_kernel = clCreateKernel(cl_ctx->program, "wpa_exploration_phase", &err);
    check_cl_error(err, "clCreateKernel exploration");
    cl_ctx->exploitation_kernel = clCreateKernel(cl_ctx->program, "wpa_exploitation_phase", &err);
    check_cl_error(err, "clCreateKernel exploitation");
}

void WPA_cleanup_cl(WPACLContext *cl_ctx) {
    ...
    if (cl_ctx->stats_kernel) clReleaseKernel(cl_ctx->stats_kernel);
...
void WPA_init_context(WPAContext *ctx, Optimizer *opt, WPACLContext *cl_ctx) {
    ...
    ctx->stagnation_counts = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, pop_size * sizeof(cl_int), NULL, &err);
    check_cl_error(err, "clCreateBuffer stagnation_counts");
    ctx->mu_P = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, dim * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer mu_P");
    ctx->sigma = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, dim * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer sigma");
    ...
    // Initialize stagnation counts
    cl_int *stagnation_counts = (cl_int *)calloc(pop_size, sizeof(cl_int));
    err = clEnqueueWriteBuffer(cl_ctx->queue, ctx->stagnation_counts, CL_TRUE, 0, pop_size * sizeof(cl_int), stagnation_counts, 0, NULL, NULL);
    check_cl_error(err, "clEnqueueWriteBuffer stagnation_counts");
    free(stagnation_counts);
}

void WPA_cleanup_context(WPAContext *ctx, WPACLContext *cl_ctx) {
    if (ctx->sigma) clReleaseMemObject(ctx->sigma);
    if (ctx->mu_P) clReleaseMemObject(ctx->mu_P);
    if (ctx->stagnation_counts) clReleaseMemObject(ctx->stagnation_counts);
...
static inline float wpa_update_k(int t, int max_iter, float f) {
    return 1.0f + (2.0f * t * t / (max_iter * max_iter)) + f;
}

void WPA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Waterwheel Plant Algorithm optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("WPA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open WPA.cl\n");
        exit(EXIT_FAILURE);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: WPA.cl is empty\n");
        fclose(fp);
        exit(EXIT_FAILURE);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read WPA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(EXIT_FAILURE);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: WPA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(EXIT_FAILURE);
    }

    // Initialize OpenCL context
    WPACLContext cl_ctx = {0};
    WPA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    WPAContext ctx = {0};
    WPA_init_context(&ctx, opt, &cl_ctx);
    ...
    if (init_event) {
        ...
        check_cl_error(err, "clGetEventProfilingInfo init_pop start");
        err = clGetEventProfilingInfo(init_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
        check_cl_error(err, "clGetEventProfilingInfo init_pop end");
    ...
    if (read_event) {
        ...
        check_cl_error(err, "clGetEventProfilingInfo read_event start");
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
        check_cl_error(err, "clGetEventProfilingInfo read_event end");
    ...
    if (write_event) {
        ...
        check_cl_error(err, "clGetEventProfilingInfo write_event start");
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
        check_cl_error(err, "clGetEventProfilingInfo write_event end");
    ...
    if (read_event) {
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        check_cl_error(err, "clGetEventProfilingInfo best_pos read start");
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
        check_cl_error(err, "clGetEventProfilingInfo best_pos read end");
        prof.read_time += (end - start) / 1e6;
        clReleaseEvent(read_event);
    }
    ...
    if (write_event) {
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        check_cl_error(err, "clGetEventProfilingInfo best_pos write start");
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
        check_cl_error(err, "clGetEventProfilingInfo best_pos write end");
        prof.write_time += (end - start) / 1e6;
        clReleaseEvent(write_event);
    }
    ...
    if (write_event) {
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        check_cl_error(err, "clGetEventProfilingInfo best_fitness write start");
        err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
        check_cl_error(err, "clGetEventProfilingInfo best_fitness write end");
        prof.write_time += (end - start) / 1e6;
        clReleaseEvent(write_event);
    }
    ...
    while (func_count < max_evals) {
        ...
        float f = WPA_F_MIN + ((float)rand() / RAND_MAX) * (WPA_F_MAX - WPA_F_MIN);
        float K = wpa_update_k(iteration, opt->max_iter, f);

        if (r < 0.5f) {
            // Compute stats
            cl_event stats_event;
            clSetKernelArg(cl_ctx.stats_kernel, 0, sizeof(cl_mem), &ctx.population);
            clSetKernelArg(cl_ctx.stats_kernel, 1, sizeof(cl_mem), &ctx.mu_P);
            clSetKernelArg(cl_ctx.stats_kernel, 2, sizeof(cl_mem), &ctx.sigma);
            clSetKernelArg(cl_ctx.stats_kernel, 3, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.stats_kernel, 4, sizeof(cl_int), &pop_size);

            size_t stats_work_size = dim;
            err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.stats_kernel, 1, NULL, &stats_work_size, NULL, 0, NULL, &stats_event);
            check_cl_error(err, "clEnqueueNDRangeKernel compute_stats");
            clFinish(cl_ctx.queue);

            if (stats_event) {
                err = clGetEventProfilingInfo(stats_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
                check_cl_error(err, "clGetEventProfilingInfo stats start");
                err = clGetEventProfilingInfo(stats_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                check_cl_error(err, "clGetEventProfilingInfo stats end");
                prof.stats_time += (end - start) / 1e6;
                clReleaseEvent(stats_event);
            }
            ...
            clSetKernelArg(cl_ctx.exploration_kernel, 2, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.exploration_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
            clSetKernelArg(cl_ctx.exploration_kernel, 4, sizeof(cl_mem), &ctx.stagnation_counts);
            clSetKernelArg(cl_ctx.exploration_kernel, 5, sizeof(cl_mem), &ctx.mu_P);
            clSetKernelArg(cl_ctx.exploration_kernel, 6, sizeof(cl_mem), &ctx.sigma);
            clSetKernelArg(cl_ctx.exploration_kernel, 7, sizeof(cl_float), &K);
            clSetKernelArg(cl_ctx.exploration_kernel, 8, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.exploration_kernel, 9, sizeof(cl_int), &pop_size);
            ...
            if (exploration_event) {
                ...
                check_cl_error(err, "clGetEventProfilingInfo exploration start");
                err = clGetEventProfilingInfo(exploration_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                check_cl_error(err, "clGetEventProfilingInfo exploration end");
            ...
            clSetKernelArg(cl_ctx.exploitation_kernel, 2, sizeof(cl_mem), &ctx.bounds);
            clSetKernelArg(cl_ctx.exploitation_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
            clSetKernelArg(cl_ctx.exploitation_kernel, 4, sizeof(cl_mem), &ctx.stagnation_counts);
            clSetKernelArg(cl_ctx.exploitation_kernel, 5, sizeof(cl_mem), &ctx.best_position);
            clSetKernelArg(cl_ctx.exploitation_kernel, 6, sizeof(cl_float), &K);
            clSetKernelArg(cl_ctx.exploitation_kernel, 7, sizeof(cl_int), &dim);
            clSetKernelArg(cl_ctx.exploitation_kernel, 8, sizeof(cl_int), &pop_size);
            ...
            if (exploitation_event) {
                ...
                check_cl_error(err, "clGetEventProfilingInfo exploitation start");
                err = clGetEventProfilingInfo(exploitation_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                check_cl_error(err, "clGetEventProfilingInfo exploitation end");
        ...
        if (read_event) {
            err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            check_cl_error(err, "clGetEventProfilingInfo population read start");
            err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            check_cl_error(err, "clGetEventProfilingInfo population read end");
            prof.read_time += (end - start) / 1e6;
            clReleaseEvent(read_event);
        }
        ...
        if (write_event) {
            err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            check_cl_error(err, "clGetEventProfilingInfo population write start");
            err = clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
            check_cl_error(err, "clGetEventProfilingInfo population write end");
            prof.write_time += (end - start) / 1e6;
            clReleaseEvent(write_event);
        }
    ...
    if (read_event) {
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
        check_cl_error(err, "clGetEventProfilingInfo final best_pos read start");
        err = clGetEventProfilingInfo(read_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
        check_cl_error(err, "clGetEventProfilingInfo final best_pos read end");
        prof.read_time += (end - start) / 1e6;
        clReleaseEvent(read_event);
    }
    ...
    WPA_cleanup_context(&ctx, &cl_ctx);
    WPA_cleanup_cl(&cl_ctx);
