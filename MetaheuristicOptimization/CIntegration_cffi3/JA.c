...
void JA_init_cl(JACLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->cruising_kernel = clCreateKernel(cl_ctx->program, "cruising_phase", &err);
    check_cl_error(err, "clCreateKernel cruising_phase");
    cl_ctx->random_walk_kernel = clCreateKernel(cl_ctx->program, "random_walk_phase", &err);
    check_cl_error(err, "clCreateKernel random_walk_phase");
}
void JA_cleanup_cl(JACLContext *cl_ctx) {
    if (cl_ctx->random_walk_kernel) clReleaseKernel(cl_ctx->random_walk_kernel);
    if (cl_ctx->cruising_kernel) clReleaseKernel(cl_ctx->cruising_kernel);
...
void JA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Jaguar Algorithm optimization...\n");
    if (!opt || !opt->context || !opt->queue || !opt->device || 
    ...
    FILE *fp = fopen("JA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open JA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: JA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read JA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: JA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }
    JACLContext cl_ctx = {0};
    JA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);
    JAContext ctx = {0};
    JA_init_context(&ctx, opt, &cl_ctx);
    int pop_size = opt->population_size;
    int dim = opt->dim;
    int func_count = 0;
    int iteration = 0;
    ...
    while (func_count < max_evals) {
        ...
        cl_event cruising_event;
        cl_float cruising_distance = JA_CRUISING_DISTANCE * (1.0f - (float)iteration / JA_MAX_ITER);
        clSetKernelArg(cl_ctx.cruising_kernel, 0, sizeof(cl_mem), &ctx.population);
        clSetKernelArg(cl_ctx.cruising_kernel, 1, sizeof(cl_mem), &ctx.bounds);
        clSetKernelArg(cl_ctx.cruising_kernel, 2, sizeof(cl_mem), &ctx.random_seeds);
        clSetKernelArg(cl_ctx.cruising_kernel, 3, sizeof(cl_float), &cruising_distance);
        clSetKernelArg(cl_ctx.cruising_kernel, 4, sizeof(cl_int), &dim);
        clSetKernelArg(cl_ctx.cruising_kernel, 5, sizeof(cl_int), &num_cruising);
        err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.cruising_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &cruising_event);
        check_cl_error(err, "clEnqueueNDRangeKernel cruising_phase");
        clFinish(cl_ctx.queue);
        double cruising_time = 0.0;
        if (cruising_event) {
            err = clGetEventProfilingInfo(cruising_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            if (err == CL_SUCCESS) {
                err = clGetEventProfilingInfo(cruising_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                if (err == CL_SUCCESS) {
                    cruising_time = (end - start) / 1e6;
                    prof.cruising_time += cruising_time;
                }
            }
            clReleaseEvent(cruising_event);
        }
        cl_event random_walk_event;
        clSetKernelArg(cl_ctx.random_walk_kernel, 0, sizeof(cl_mem), &ctx.population);
        clSetKernelArg(cl_ctx.random_walk_kernel, 1, sizeof(cl_mem), &ctx.bounds);
        clSetKernelArg(cl_ctx.random_walk_kernel, 2, sizeof(cl_mem), &ctx.random_seeds);
        clSetKernelArg(cl_ctx.random_walk_kernel, 3, sizeof(cl_int), &dim);
        clSetKernelArg(cl_ctx.random_walk_kernel, 4, sizeof(cl_int), &num_cruising);
        clSetKernelArg(cl_ctx.random_walk_kernel, 5, sizeof(cl_int), &pop_size);
        err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.random_walk_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &random_walk_event);
        check_cl_error(err, "clEnqueueNDRangeKernel random_walk_phase");
        clFinish(cl_ctx.queue);
        double random_walk_time = 0.0;
        if (random_walk_event) {
            err = clGetEventProfilingInfo(random_walk_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            if (err == CL_SUCCESS) {
                err = clGetEventProfilingInfo(random_walk_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                if (err == CL_SUCCESS) {
                    random_walk_time = (end - start) / 1e6;
                    prof.random_walk_time += random_walk_time;
                }
            }
            clReleaseEvent(random_walk_event);
        }
        positions = (float *)malloc(pop_size * dim * sizeof(float));
        err = clEnqueueReadBuffer(cl_ctx.queue, ctx.population, CL_TRUE, 0, pop_size * dim * sizeof(cl_float), positions, 0, NULL, &read_event);
        check_cl_error(err, "clEnqueueReadBuffer population");
        ...
        float new_best = fitness[indices[0]];
        ...
        printf("Profiling (ms): Cruising Phase = %.3f, Random Walk Phase = %.3f\n", cruising_time, random_walk_time);
    }
    printf("\nProfiling Summary (Total ms):\n");
    printf("Initialize Population: %.3f\n", prof.init_pop_time);
    printf("Cruising Phase: %.3f\n", prof.cruising_time);
    printf("Random Walk Phase: %.3f\n", prof.random_walk_time);
    printf("Data Reads: %.3f\n", prof.read_time);
    printf("Data Writes: %.3f\n", prof.write_time);
    if (prof.count > 0) {
        printf("Average per Iteration (ms):\n");
        printf("Cruising Phase: %.3f\n", prof.cruising_time / prof.count);
        printf("Random Walk Phase: %.3f\n", prof.random_walk_time / prof.count);
    ...
    JA_cleanup_context(&ctx, &cl_ctx);
    JA_cleanup_cl(&cl_ctx);
