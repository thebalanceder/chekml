...
void initialize_fireflies(Optimizer *opt, ObjectiveFunction objective_function) {
    for (int i = 0; i < opt->population_size; i++) {
        opt->population[i].position = (double*)malloc(opt->dim * sizeof(double));  // 🛠️ Allocate memory!
    ...
    opt->best_solution.position = (double*)malloc(opt->dim * sizeof(double));  // 🛠️ Allocate memory for best_solution
    ...
    for (int i = 0; i < opt->population_size - 1; i++) {
        for (int j = 0; j < opt->population_size - i - 1; j++) {
            if (opt->population[j].fitness > opt->population[j + 1].fitness) {
                ...
                double* temp_pos = opt->population[j].position;
                opt->population[j].position = opt->population[j + 1].position;
                opt->population[j + 1].position = temp_pos;
            }
        }
    }
}
