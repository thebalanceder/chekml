...
from guided_local_search import GuidedLocalSearch
import time
...
# Dummy feature indicator function: 1 if |xi| > 1
def default_feature_indicator(x):
    return [int(abs(xi) > 1.0) for xi in x]

# Default feature costs (same as number of features)
def default_feature_costs(dim):
    return [1.0] * dim
...
# Run GLS on each benchmark function
for name, func in benchmark_functions.items():
    ...
    gls = GuidedLocalSearch(
        ...
        feature_costs=default_feature_costs(dim),
        feature_indicators=default_feature_indicator,
        dim=dim,
        bounds=bounds,
        lambda_=0.1,
        max_iterations=100
    ...
    best_solution, best_value, history = gls.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"GuidedLocalSearch on {name} Function")
    plt.legend()
    plt.savefig(f"GLS_{name}.png")
