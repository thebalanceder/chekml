...
from cultural_algorithm import CulturalAlgorithmOptimizer
import time
...
# Run Cultural Algorithm on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    ca = CulturalAlgorithmOptimizer(
    ...
    best_solution, best_cost, history = ca.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Cultural Algorithm on {name} Function")
    plt.legend()
    plt.savefig(f"CA_{name}.png")
    plt.close()
