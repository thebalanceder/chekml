...
from colliding_bodies_optimization import CollidingBodiesOptimizer
import time
...
# Run CBO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    cbo = CollidingBodiesOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"CBO on {name} Function")
    plt.legend()
    plt.savefig(f"CBO_{name}.png")
