...
from bumble_bee_mating import BumbleBeeMatingOptimizer
import time
...
# Run BBMO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    bbmo = BumbleBeeMatingOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"BBMO on {name} Function")
    plt.legend()
    plt.savefig(f"BBMO_{name}.png")
