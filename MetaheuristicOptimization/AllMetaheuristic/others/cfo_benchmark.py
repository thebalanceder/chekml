...
from central_force_optimization import CentralForceOptimizer
import time
...
bounds = (-5, 5)  # Uniform bounds for 2D
...
# Run CFO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    cfo = CentralForceOptimizer(
    ...
    best_solution, best_value, history = cfo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"CFO on {name} Function")
    plt.legend()
    plt.savefig(f"CFO_{name}.png")
