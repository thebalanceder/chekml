...
from dujiangyan_irrigation_optimizer import DujiangyanIrrigationOptimizer  # Import your custom optimizer
...
# Example values for the required parameters
diversion_factor = 0.5
flow_adjustment = 0.1
water_density = 1.0
fluid_distribution = 0.8
centrifugal_resistance = 0.3
...
for model_name, model in models.items():
    ...
    # Initialize Dujiangyan Irrigation Optimizer with the required arguments
    diso = DujiangyanIrrigationOptimizer(
        objective_function=lambda wolf: objective_function(wolf, model),  # Pass objective function
        dim=len(param_keys),  # Number of parameters
        bounds=bounds,  # Boundaries for each parameter
        population_size=10,  # Population size for the optimizer
        max_iter=20,  # Maximum number of iterations
        diversion_factor=diversion_factor,  # Required argument
        flow_adjustment=flow_adjustment,  # Required argument
        water_density=water_density,  # Required argument
        fluid_distribution=fluid_distribution,  # Required argument
        centrifugal_resistance=centrifugal_resistance,  # Required argument
        bottleneck_ratio=bottleneck_ratio,  # Required argument
        elimination_ratio=elimination_ratio  # Required argument
    )
    
    # Perform optimization
    best_wolf, best_mse= diso.optimize()
