...
from dujiangyan_irrigation_optimizer import DujiangyanIrrigationOptimizer
import time
...
# Run Dujiangyan Irrigation System Optimization
diso = DujiangyanIrrigationOptimizer(objective_function=rastrigin_function, dim=2, bounds=bounds)

# Measure execution time
start_time = time.time()
best_solution, best_value, history = diso.optimize()
end_time = time.time()
...
plt.xlabel("X1")
plt.ylabel("X2")
plt.title("Dujiangyan Irrigation System Optimization on Rastrigin Function")
plt.legend()
plt.savefig("DISOwithRCF.png")
