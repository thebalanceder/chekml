...
from glowworm_swarm_optimization import GlowwormSwarmOptimizer
import time
...
# Run GlowSO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    gso = GlowwormSwarmOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"GlowSO on {name} Function")
    plt.legend()
    plt.savefig(f"GlowSO_{name}.png")
    plt.close()
