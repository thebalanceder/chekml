...
from galactic_swarm_optimization import GalacticSwarmOptimizer
import time
...
# Run GalSO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    gso = GalacticSwarmOptimizer(objective_function=func, dim=2, bounds=bounds, 
                                 pop_size=5, subpop=10, epoch_number=5, 
                                 iteration1=198, iteration2=1000, trials=1)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, results, history = gso.optimize()
    end_time = time.time()
    execution_time = end_time - start_time
    execution_times[name] = execution_time

    # Extract search history from the last trial
    last_trial_history = history[-1][1]  # history is [(trial, trial_history), ...]
    x_vals = [h[1][0] for h in last_trial_history]  # h is (epoch, solution, value)
    y_vals = [h[1][1] for h in last_trial_history]
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"GalSO on {name} Function")
    plt.legend()
    plt.savefig(f"GalSO_{name}.png")
