...
from phototropic_algorithm import PhototropicOptimizer
import time
...
# Run POA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    poa = PhototropicOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = poa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"POA on {name} Function")
    plt.legend()
    plt.savefig(f"POA_{name}.png")
