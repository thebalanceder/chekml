...
from seeker_optimization_algorithm import SeekerOptimizationAlgorithm
import time
...
# Run SOA on each benchmark function
for name, func in benchmark_functions.items():
    ...
    x_vals = [h[1][0] for h in soa.history]
    y_vals = [h[1][1] for h in soa.history]
    ...
    plt.scatter(gbest_s[0][0], gbest_s[0][1], color="white", marker="*", s=100, label="Best Solution")

    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"SOA on {name} Function")
    plt.legend()
    plt.savefig(f"SOA_{name}.png")

    print(f"Best solution found: {gbest_s[0]}, Objective Value: {gbest_fun[0]}")
