...
from teaching_learning_based_optimization import TeachingLearningBasedOptimizer
import time
...
# Run TLBO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    tlbo = TeachingLearningBasedOptimizer(objective_function=func, dim=2, bounds=bounds, 
                                         population_size=50, max_iter=100)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = tlbo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"TLBO on {name} Function")
    plt.legend()
    plt.savefig(f"TLBO_{name}.png")
