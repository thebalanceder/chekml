...
from electromagnetic_field_optimization import ElectromagneticFieldOptimizer
import time
...
# Run EFO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    efo = ElectromagneticFieldOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = efo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"EFO on {name} Function")
    plt.legend()
    plt.savefig(f"EFO_{name}.png")
