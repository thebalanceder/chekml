...
# Run CSS on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    css = ChargedSystemSearch(objective_function=func, dim=2, bounds=bounds, 
                              population_size=16, max_iter=100)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"CSS on {name} Function")
    plt.legend()
    plt.savefig(f"CSS_{name}.png")
    plt.close()
