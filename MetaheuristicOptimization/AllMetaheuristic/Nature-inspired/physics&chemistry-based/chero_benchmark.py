...
from chemical_reaction_optimization import ChemicalReactionOptimizer
import time
...
# Run CRO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    cro = ChemicalReactionOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"CRO on {name} Function")
    plt.legend()
    plt.savefig(f"CRO_{name}.png")
