...
from snow_ablation_optimizer import MESAO_optimize
import time
...
# Expected optima for validation (approximate for 2D)
expected_optima = {
    "Schwefel": 0.0,  # Global minimum at x ≈ [420.9687, 420.9687], but scaled for [-5, 5]
    "Levy": 0.0,  # Global minimum at x = [1, 1]
    "Michalewicz": -1.8013,  # Known minimum for 2D with m=10
    "Shubert": -186.7309  # Known minimum for 2D
}
...
# Run MESAO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = MESAO_optimize(func, dim=2)
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"MESAO on {name} Function")
    plt.legend()
    plt.savefig(f"MESAO_{name}.png")
    plt.close()

    print(f"Best solution found: {best_solution}")
    print(f"Objective Value: {best_value:.4f}")
    print(f"Expected Optimum: {expected_optima[name]:.4f}")
