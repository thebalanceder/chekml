...
from lightning_search_algorithm import LightningSearchAlgorithm
import time
...
# Run LSA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    lsa = LightningSearchAlgorithm(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history, elapsed_time = lsa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"LSA on {name} Function")
    plt.legend()
    plt.savefig(f"LSA_{name}.png")
