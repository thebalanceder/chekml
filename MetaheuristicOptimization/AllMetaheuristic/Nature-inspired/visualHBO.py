...
from heap_based_optimizer import HeapBasedOptimizer
...
# Run Heap-Based Optimizer Algorithm
hbo = HeapBasedOptimizer(objective_function=rastrigin_function, dim=2, bounds=bounds)
best_solution, best_value, history = hbo.optimize()
...
plt.xlabel("X1")
plt.ylabel("X2")
plt.title("Heap-Based Optimizer on Rastrigin Function")
plt.legend()
plt.savefig("HBO.png")
