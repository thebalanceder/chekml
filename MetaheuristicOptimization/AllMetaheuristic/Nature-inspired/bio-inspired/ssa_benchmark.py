...
from squirrel_search_algorithm import SquirrelSearchAlgorithm
import time
...
for name, func in benchmark_functions.items():
    ...
    ssa = SquirrelSearchAlgorithm(objective_function=func, dim=2, bounds=bounds)
    ...
    # Simulate search path (since SSA doesn't store history, we'll use final positions)
    # For visualization, we can plot the best solution and initial positions
    initial_positions = ssa.squirrels.copy()
    x_vals = [pos[0] for pos in initial_positions] + [best_solution[0]]
    y_vals = [pos[1] for pos in initial_positions] + [best_solution[1]]
    ...
    # Plot search path (initial positions to best)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"SSA on {name} Function")
    plt.legend()
    plt.savefig(f"SSA_{name}.png")
