...
# Run Walrus Optimization Algorithm
wo = WalrusOptimizer(objective_function=rastrigin_function, dim=2, bounds=bounds)
best_solution, best_value, history = wo.optimize()
...
# Plot search path
plt.plot(x_vals, y_vals, marker="o", markersize=3, color="navy", linestyle="dashed", alpha=0.7, label="Search Path")
plt.scatter(best_solution[0], best_solution[1], color="white", marker="*", s=100, label="Best Solution")

plt.xlabel("X1")
plt.ylabel("X2")
plt.title("Walrus Optimizer on Rastrigin Function")
plt.legend()
plt.savefig("WO.png")
