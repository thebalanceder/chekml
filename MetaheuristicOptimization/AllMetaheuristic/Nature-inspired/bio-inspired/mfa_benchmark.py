...
from mayfly_optimization import MayflyOptimizer
import time
...
# Run Mayfly Optimizer on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    mayfly = MayflyOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = mayfly.optimize()
    end_time = time.time()
    execution_time = end_time - start_time
    execution_times[name] = execution_time

    # Extract search history
    x_vals = [mayfly.global_best['position'][0] for _ in history]  # Best position at each iteration
    y_vals = [mayfly.global_best['position'][1] for _ in history]
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Mayfly Optimizer on {name} Function")
    plt.legend()
    plt.savefig(f"Mayfly_{name}.png")
