...
from elephant_herding_optimization import ElephantHerdingOptimizer
import time
...
# Run EHO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    eho = ElephantHerdingOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = eho.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"EHO on {name} Function")
    plt.legend()
    plt.savefig(f"EHO_{name}.png")
