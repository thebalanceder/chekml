...
# Assuming DolphinEcholocationOptimizer is defined in dolphin_echolocation.py
from dolphin_echolocation import DolphinEcholocationOptimizer
...
# Run DEA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    dea = DolphinEcholocationOptimizer(
    ...
    best_solution, best_value, history = dea.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"DEA on {name} Function")
    plt.legend()
    plt.savefig(f"DEA_{name}.png")
