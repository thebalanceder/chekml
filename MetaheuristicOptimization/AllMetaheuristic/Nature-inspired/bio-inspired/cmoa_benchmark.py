...
import time
from coronavirus_metamorphosis_optimization_algorithm import CoronavirusMetamorphosisOptimizer
...
# Run CMOA on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    cmoa = CoronavirusMetamorphosisOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = cmoa.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"CMOA on {name} Function")
    plt.legend()
    plt.savefig(f"CMOA_{name}.png")
