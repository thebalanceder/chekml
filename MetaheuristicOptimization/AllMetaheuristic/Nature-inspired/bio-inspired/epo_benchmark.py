...
from emperor_penguin_optimizer import EmperorPenguinOptimizer
import time
...
# Run SA-EPO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    epo = EmperorPenguinOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = epo.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"SA-EPO on {name} Function")
    plt.legend()
    plt.savefig(f"SA_EPO_{name}.png")
