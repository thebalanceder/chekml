...
[best_solution, best_fitness] = MSA_advanced(objective_function, lb, ub, population_size, max_iter);

% Display the result
disp(['Best Solution: ', num2str(best_solution)]);
disp(['Best Fitness: ', num2str(best_fitness)]);
function [best_solution, best_fitness] = MSA_advanced(objective_function, lb, ub, population_size, max_iter)
    ...
    p_explore = 0.2; % Probability of exploration
    max_p_explore = 0.8; % Maximum probability of exploration
    min_p_explore = 0.1; % Minimum probability of exploration
    
    % Main loop
    for iter = 1:max_iter
        % Update probability of exploration adaptively
        p_explore = max(max_p_explore * exp(-0.1 * iter), min_p_explore);
        ...
        for i = 1:population_size
            for j = 1:numel(lb)
                if rand() < p_explore % Exploration
                    ...
                    population(i, j) = best_solution(j) + randn() * (ub(j) - lb(j)) / 10; % Local perturbation
                end
