...
from evolutionary_algorithm import NSGA2  # Make sure this is implemented correctly
import time
...
# Run evolutionary_algorithm on each benchmark function
for name, func in benchmark_functions.items():
    ...
    best_solution, best_value, history = nsga2.optimize()
    end_time = time.time()
    ...
    y_vals = [point[1] if point[1] is not None else np.nan for point in history] 
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Evolutionary Algorithm on {name} Function")
    plt.legend()
    plt.savefig(f"EA_{name}.png")
