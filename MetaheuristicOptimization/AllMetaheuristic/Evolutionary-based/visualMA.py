...
ma = MemeticAlgorithm(objective_function=rastrigin_function, dim=2, bounds=bounds)
best_solution, best_value, history = ma.optimize()
...
# Plot the results (similar to previous algorithms)
plt.scatter(x_vals, y_vals, color="red", marker="o", label="Search Path")
plt.scatter(best_solution[0], best_solution[1], color="white", marker="*", s=100, label="Best Solution")

plt.xlabel("X1")
plt.ylabel("X2")
plt.title("Memetic Algorithm on Rastrigin Function")
plt.scatter(x_vals, y_vals, color="red", marker="o", label="Search Path")
plt.scatter(best_solution[0], best_solution[1], color="white", marker="*", s=100, label="Best Solution")
plt.legend()
plt.savefig("MA.png")
