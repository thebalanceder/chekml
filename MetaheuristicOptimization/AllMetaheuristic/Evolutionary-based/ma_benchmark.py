...
from memetic_algorithm import MemeticAlgorithm  # Make sure this is implemented and accessible
import time
...
# Run MemeticAlgorithm on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    optimizer = MemeticAlgorithm(
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"MemeticAlgorithm on {name} Function")
    plt.legend()
    plt.savefig(f"MA_{name}.png")
