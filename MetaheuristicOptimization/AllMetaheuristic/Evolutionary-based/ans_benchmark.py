...
from across_neighborhood_search import AcrossNeighborhoodSearch  # Ensure this is correctly implemented
import time
...
# Run AcrossNeighborhoodSearch on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    ans = AcrossNeighborhoodSearch(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = ans.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"AcrossNeighborhoodSearch on {name} Function")
    plt.legend()
    plt.savefig(f"ANS_{name}.png")
