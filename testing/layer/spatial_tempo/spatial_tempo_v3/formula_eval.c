...
FormulaSet *formula_create(const KOFKernel *k) {
    ...
    // Dynamically allocate variable names for kernel encoding (w, d0, d1, ...)
    const char **k_vars = (const char**)malloc((1 + k->ndim) * sizeof(char*));
    char **dim_vars = (char**)malloc(k->ndim * sizeof(char*));
    k_vars[0] = "w";
    for (int i = 0; i < k->ndim; ++i) {
        // Generate variable name "d0", "d1", etc., with sufficient buffer size
        char *dim_name = (char*)malloc(16 * sizeof(char)); // Increased to 16 bytes
        snprintf(dim_name, 16, "d%d", i);
        dim_vars[i] = dim_name;
        k_vars[i + 1] = dim_name;
    }
    ...
    if (!f->op) {
        ...
        for (int i = 0; i < k->ndim; ++i) free(dim_vars[i]);
        free(dim_vars); free(k_vars); formula_free(f); return NULL;
    }
    ...
    // Clean up dynamically allocated dimension variable names
    for (int i = 0; i < k->ndim; ++i) free(dim_vars[i]);
    free(dim_vars);
    free(k_vars);
    return f;
