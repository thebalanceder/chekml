...
static int parse_boolean_formula(const char *input, BooleanFormula *out, const char *filename) {
    ...
    if ((op_pos = strstr(p, ">="))) {
        ...
        *op_pos = '\0';
        ...
        fprintf(stderr, "Invalid boolean formula '%s' in %s: missing relational operator\n", p, filename);
        free(temp);
        return -1;
    }

    if (!out->lhs || !out->rhs || !out->lhs[0] || !out->rhs[0]) {
        fprintf(stderr, "Invalid boolean formula '%s' in %s: empty LHS or RHS\n", p, filename);
        free(out->lhs); free(out->rhs); free(temp);
        out->lhs = NULL; out->rhs = NULL;
        return -1;
    }
...
int temporal_load(const char *filename, TemporalFormula *out) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (s[0] == '[' && s[strlen(s)-1] == ']') {
            ...
            section = strdup(s + 1);
            if (!section) {
                fclose(f); temporal_free(out); return -2;
            }
            continue;
        }
        ...
        if (strcmp(section, "vars") == 0) {
            ...
            if (!out->testing_formula || parse_boolean_formula(s, out->testing_formula, filename) != 0) {
                ...
                free(section); fclose(f); temporal_free(out); return -6;
            }
