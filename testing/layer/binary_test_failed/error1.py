...
def train(training_sets):
    # Determine maximum number of bits needed
    max_decimal = max(max(abs(int(a)), abs(int(b)), abs(int(out))) for a, b, out in training_sets)
    bits = math.ceil(math.log2(max_decimal + 1)) + 1  # +1 for extra bit
    ...
    return all_digit_tables, prob_logic_table, bits

# Step 2: Helper function to compute qubits probabilities
...
def predict(decimal_a, decimal_b, prob_logic_table, bits, firm=False):
...
if __name__ == "__main__":
    ...
    def generate_combinations():
        combinations = []
        for a in range(101):          # First number from 0 to 10
            for b in range(101):      # Second number from 0 to 10
    ...
    # Train model
    digit_tables, prob_logic_table, bits = train(training_sets)
    ...
    predicted_output = predict(a, b, prob_logic_table, bits, firm=True)
