...
static void print_shape(const char *name, const int *s, int ndim) {
    printf("%s=[", name);
    for (int i = 0; i < ndim; ++i) { printf("%d%s", s[i], (i+1<ndim)?",":""); }
    printf("]\n");
}
...
int main(int argc, char **argv) {
    ...
    int ndim = 3;
    int in_shape[]  = {6, 6, 6};
    int out_shape1[3], out_shape2[3];
    size_t in_total  = total_size(in_shape, ndim);
    size_t out_total1, out_total2;

    float *input    = (float*)calloc(in_total, sizeof(float));
    float *output1  = (float*)calloc(in_total, sizeof(float));
    float *output2  = (float*)calloc(in_total, sizeof(float));
    float *target   = (float*)calloc(in_total, sizeof(float));
    float *dLdy     = (float*)calloc(in_total, sizeof(float));
    float *d_output1= (float*)calloc(in_total, sizeof(float));
    ...
    if (!input || !output1 || !output2 || !target || !dLdy || !d_output1 || !d_input || !d_weight1 || !d_weight2) {
    ...
    fill_ramp(target, in_total);

    temporal_wrapper_forward(W1, output1, out_shape1, input, in_shape, ndim);
    out_total1 = total_size(out_shape1, ndim);
    temporal_wrapper_forward(W2, output2, out_shape2, output1, out_shape1, ndim);
    out_total2 = total_size(out_shape2, ndim);

    print_shape("in_shape", in_shape, ndim);
    print_shape("out_shape1", out_shape1, ndim);
    print_shape("out_shape2", out_shape2, ndim);
    ...
    for (int epoch = 1; epoch <= E; ++epoch) {
        ...
        sgd_update(W1->k, W1->k_grad, W1->k_count, lr, wd, clip);
        sgd_update(W2->k, W2->k_grad, W2->k_count, lr, wd, clip);
        ...
        if (epoch == 1 || epoch % 5 == 0 || epoch == E) {
            printf("epoch %3d: loss=%.6f, k1=[", epoch, loss);
            for (int i = 0; i < W1->k_count; ++i) {
                printf("%.6f%s", W1->k[i], (i + 1 < W1->k_count) ? "," : "");
            }
            printf("], k2=[");
            for (int i = 0; i < W2->k_count; ++i) {
                printf("%.6f%s", W2->k[i], (i + 1 < W2->k_count) ? "," : "");
            }
            printf("], memory1=[");
            for (int i = 0; i < K1.k_elems && i < 4; ++i) {
                printf("%.6f%s", W1->memory_state[i], (i + 1 < K1.k_elems && i + 1 < 4) ? "," : "");
            }
            printf("], memory2=[");
            for (int i = 0; i < K2.k_elems && i < 4; ++i) {
                printf("%.6f%s", W2->memory_state[i], (i + 1 < K2.k_elems && i + 1 < 4) ? "," : "");
            }
            printf("]\n");
        }
    }
