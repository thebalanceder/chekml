...
class CustomConvLayer(nn.Module):
    """Custom convolutional layer with multiple kernels, optimized convolution, and enhanced temporal processing."""
    def __init__(self, kof_path, tprl_path=None, in_channels=1, out_channels=1, padding=0, stride=1, kernel_idx=0, verbose=True):
        ...
        # Encoding functions for each kernel
        x_sym = symbols('x')
        w_sym = symbols('w')
        ...
        if tprl_path:
            ...
            # Avoid duplicate historical variables
    ...
    def _get_temporal_args(self, w_tensor, g_tensor, m_tensor):
        ...
        for i in range(self.history_length):
            args_dict[f'wh{i}'] = self.weight_history[i]
            args_dict[f'gh{i}'] = self.gradient_history[i]
        args = []
        k_idx = 0
    ...
    def forward(self, input, epoch=0):
        """Forward pass with multiple kernels, formula selection, and optimized convolution."""
        ...
        # Select encoding functions for the current kernel
        ...
        # Optimized F.conv3d for discrete filters
        if self.ndim == 3 and not self.need_interp:
        ...
        if not self.need_interp:
            ...
            in_flat_per_channel = torch.sum(input_coords * spatial_strides[None, None, :], dim=-1)
            ...
            flat_idx = torch.sum(corner_idx * spatial_strides[None, None, :], dim=-1)
            full_idx = (torch.arange(batch_size * self.in_channels, device=device)[:, None, None] * prod_spatial + flat_idx[None, :, :]).view(-1)
        ...
        if self.temporal:
            ...
            adjusted = SymPyFunction.apply(w_tensor, self.formula_funcs[formula_idx], *args[1:])
            # Clamp adjusted weights to prevent extreme values
            ...
            if self.testing_lhs_func:
                ...
                if self.rel_op == '>':
                    ...
                    mask = lhs != rhs
            weights = torch.where(mask, adjusted, w_tensor)
        ...
        # Compute x_enc with clamping for log(1 + x)
    ...
    def update_histories(self):
        ...
        self.weight_history[0] = self.weights_list[self.kernel_idx].detach()
        ...
        if self.memory_func:
            w_tensor = self.weights_list[self.kernel_idx].detach()
            g_tensor = self.grad_magnitudes
            m_tensor = self.memory_state
            args = self._get_temporal_args(w_tensor, g_tensor, m_tensor)
...
if __name__ == "__main__":
    ...
    layer1 = CustomConvLayer("test.kof", "temporal.tprl", in_channels=1, out_channels=1, padding=0, stride=1, kernel_idx=0, verbose=True)
    layer2 = CustomConvLayer("test.kof", "temporal.tprl", in_channels=1, out_channels=1, padding=0, stride=1, kernel_idx=0, verbose=True)
    ...
    optimizer = optim.SGD([
        {'params': layer1.parameters(), 'lr': 0.005, 'weight_decay': 0.001},
        {'params': layer2.parameters(), 'lr': 0.005, 'weight_decay': 0.001}
    ], lr=0.005)

    epochs = 30
    for epoch in range(1, epochs + 1):
        optimizer.zero_grad()
        output1 = layer1(input_data, epoch=epoch)
        output2 = layer2(output1, epoch=epoch)
        ...
        if torch.isnan(loss):
            warnings.warn(f"NaN loss detected at epoch {epoch}, stopping training")
            break
        loss.backward()
