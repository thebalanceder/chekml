...
def load_kof(path):
    """Load .kof file with validation and support for channel-specific taps."""
    ...
    reading_taps = False
    current_tap_index = -1
    channel_taps = {}  # Support for channel-specific taps
    ...
    try:
        for line in lines:
            ...
            if s.startswith('ndim:'):
                ...
                if 'current_channel' in locals():
                    channel_taps[current_channel].append(tap_dict)
                else:
                    taps.append(tap_dict)
                current_tap_index += 1
        ...
        if ndim is None or (not taps and not channel_taps):
            raise ValueError("Invalid .kof file: missing ndim or taps")
    except Exception as e:
        raise ValueError(f"Error loading .kof file '{path}': {str(e)}")

    return {'ndim': ndim, 'taps': taps, 'channel_taps': channel_taps, 'global_op': global_op, 'global_in': global_in, 'global_k': global_k}

def load_tprl(path):
    """Load .tprl file with validation and support for adaptive history and multiple formulas."""
    if not path:
        return None
    ...
    section = None
    vars_list = []
    formulas = []  # Support multiple formulas
    testing_lhs = None
    testing_rhs = None
    rel_op = None
    memory_formula = None
    ...
    param_inits = []
    history_length = 5
    adaptive_history = False  # New flag
    ...
    try:
        ...
        raise ValueError(f"Error loading .tprl file '{path}': {str(e)}")
...
class SymPyFunction(torch.autograd.Function):
    """Custom SymPy function evaluation.
    Note: For GPU acceleration, integrate sympytorch library to convert expressions to PyTorch modules.
    Example: from sympytorch import SymPyModule; module = SymPyModule(expressions=[expr])
    """
    @staticmethod
...
class CustomConvLayer(nn.Module):
    """Custom convolutional layer with support for continuous filters, temporal processing, and optimizations."""
    def __init__(self, kof_path, tprl_path=None, in_channels=1, out_channels=1, padding=0, stride=1):
        ...
        # Dynamic padding if insufficient
    ...
    def forward(self, input):
        """Forward pass with optimizations, validations, and output cropping to match input spatial shape."""
        ...
        # Get x with optimizations
        if not self.need_interp:
            # Check if standard conv can be used (for ndim=3, identity enc, * op)
            if self.ndim == 3 and all(f is None for f in self.op_funcs + self.input_encode_funcs + self.kernel_encode_funcs):
                # Fall back to optimized F.conv3d
                kernel = self.weights.view(self.out_channels, self.in_channels, 1, 1, 1)  # Assume pointwise for simplicity; extend if needed
                output = F.conv3d(input, kernel, stride=self.stride.tolist(), padding=self.padding.tolist())
                # Crop to match input spatial shape
                slices = [slice(0, s) for s in in_shape.tolist()]
                output = output[(slice(None), slice(None)) + tuple(slices)]  # [:, :, slice0, slice1, ...]
                print(f"F.conv3d path - Input shape: {input.shape}, Output shape before crop: {F.conv3d(input, kernel, stride=self.stride.tolist(), padding=self.padding.tolist()).shape}, Output shape after crop: {output.shape}")
                return output
            ...
            # Vectorized multilinear interpolation
            spatial_strides = torch.ones(self.ndim, dtype=torch.long, device=device)
            if self.ndim > 0:
                spatial_strides[-1] = 1
                for i in range(self.ndim - 2, -1, -1):
            ...
            corner_offs = torch.tensor(corner_offs_list, dtype=torch.long, device=device)  # [2**ndim, ndim]
            num_corners = corner_offs.shape[0]

            corner_idx = floor[None, :, :] + corner_offs[:, None, :]  # [num_corners, num_points, ndim]
            corner_idx = torch.clamp(corner_idx, min=torch.zeros_like(corner_idx), max=padded_shape[None, None, :] - 1)

            weights = torch.prod(torch.where(corner_offs[:, None, :] == 0, 1 - frac[None, :, :], frac[None, :, :]), dim=-1)  # [num_corners, num_points]

            flat_idx = torch.sum(corner_idx * spatial_strides[None, None, :], dim=-1)  # [num_corners, num_points]

            full_idx = (torch.arange(batch_size * self.in_channels, device=device)[:, None, None] * prod_spatial + flat_idx[None, :, :]).view(-1)  # [b*c * num_corners * num_points]
            full_idx = torch.clamp(full_idx, max=batch_size * self.in_channels * prod_spatial - 1)

            gathered_flat = input_padded.view(-1)[full_idx]
            full_gathered = gathered_flat.view(batch_size, self.in_channels, num_corners, num_points)  # [b, c, num_corners, num_points]

            weighted = full_gathered * weights[None, None, :, :]  # [b, c, num_corners, num_points]
            contrib = weighted.sum(dim=2)  # [b, c, num_points]

            x = contrib.permute(0, 2, 1).view(batch_size, out_spatial_total, self.k_elems, self.in_channels)

        # Temporal adjustment with multiple formulas
        ...
        if self.temporal:
            ...
            # Use first formula by default; extend logic for multiple if needed
            adjusted = SymPyFunction.apply(w_tensor, self.formula_funcs[0], *args[1:])
        ...
        # Crop output to match input spatial shape
        slices = [slice(0, s) for s in in_shape.tolist()]
        output = output[(slice(None), slice(None)) + tuple(slices)]  # [:, :, slice0, slice1, ...]
    ...
    def update_histories(self):
        ...
        if self.adaptive_history:
            # Prune history if average grad low
        ...
        if self.memory_func:
            ...
            m_tensor = self.memory_state
            args = self._get_temporal_args(w_tensor, g_tensor, m_tensor)
            # Iterative update for recurrent memory
            for _ in range(self.history_count):
                m_tensor = SymPyFunction.apply(w_tensor, self.memory_func, *args[1:])
            self.memory_state = m_tensor
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs + 1):
        ...
        if output2.shape != target.shape:
            raise ValueError(f"Output shape {output2.shape} does not match target shape {target.shape}")
