...
def load_kof(path):
    ...
    for line in lines:
        ...
        if s.startswith('ndim:'):
            ...
            reading_taps = True
        elif reading_taps and (s[0].isdigit() or (s[0] == '-' and len(s) > 1 and s[1].isdigit())):
            parts = re.split(r'\s+', s)
            offs = list(map(int, parts[:ndim]))
            w = float(parts[ndim])
...
class SymPyFunction(torch.autograd.Function):
    ...
    def forward(ctx, input_tensor, sympy_func, *args):
        # Convert input tensor to NumPy for SymPy evaluation
    ...
    def backward(ctx, grad_output):
        ...
        # Gradient w.r.t. tensor arguments
...
class CustomConvLayer(nn.Module):
    def __init__(self, kof_path, tprl_path=None):
        super().__init__()
        kof = load_kof(kof_path)
        self.ndim = kof['ndim']
        self.tap_offsets = torch.tensor([t['offsets'] for t in kof['taps']], dtype=torch.long)
        self.k_elems = len(kof['taps'])
        self.weights = nn.Parameter(torch.tensor([t['weight'] for t in kof['taps']], dtype=torch.float32))

        # Symbols
        x_sym = symbols('x')
        w_sym = symbols('w')
        d_syms = symbols(['d' + str(i) for i in range(self.ndim)])

        # Compile per-tap functions with 'numpy' backend
        self.op_funcs = []
        ...
        if tprl_path:
            ...
            self.weight_history = torch.zeros((self.history_length, self.k_elems), dtype=torch.float32)
            self.gradient_history = torch.zeros((self.history_length, self.k_elems), dtype=torch.float32)
            self.history_count = 0
            self.grad_magnitudes = torch.zeros(self.k_elems, dtype=torch.float32)
            self.memory_state = torch.zeros(self.k_elems, dtype=torch.float32)
    ...
    def forward(self, input):
        device = input.device
        in_shape = torch.tensor(input.shape, device=device)
        strides = torch.ones(self.ndim, dtype=torch.long, device=device)
        for i in range(self.ndim - 2, -1, -1):
            strides[i] = strides[i + 1] * in_shape[i + 1]

        min_off = self.tap_offsets.min(dim=0).values.to(device)
        max_off = self.tap_offsets.max(dim=0).values.to(device)
        out_min = -min_off
        out_max = in_shape - 1 - max_off
        out_shape = out_max - out_min + 1
        out_shape = torch.clamp(out_shape, min=0)
        if torch.any(out_shape <= 0):
            return torch.empty(tuple(out_shape.tolist()), device=device, dtype=input.dtype)

        # Generate output coordinates
        coords = torch.cartesian_prod(*[torch.arange(s.item(), device=device) for s in out_shape])
        # (out_total, ndim)

        # Input coordinates for each tap
        input_coords = coords[:, None, :] + out_min[None, None, :] + self.tap_offsets[None, :, :].to(device)
        # (out_total, k_elems, ndim)

        in_flat = torch.sum(input_coords * strides[None, None, :], dim=-1)  # (out_total, k_elems)

        x = torch.index_select(input.view(-1), 0, in_flat.view(-1)).view(in_flat.shape)
        # (out_total, k_elems)
        ...
        if self.temporal:
            ...
            mask = torch.ones(self.k_elems, dtype=torch.bool, device=device)
        ...
        # Compute w_enc per tap
        w_enc = torch.zeros(self.k_elems, device=device, dtype=weights.dtype)
        for t in range(self.k_elems):
            d_args = self.tap_offsets[t].tolist()
            w_enc[t] = SymPyFunction.apply(self.weights[t], self.kernel_encode_funcs[t], *d_args)

        # Compute x_enc per tap
        x_enc = torch.zeros_like(x)
        for t in range(self.k_elems):
            x_enc[:, t] = SymPyFunction.apply(x[:, t], self.input_encode_funcs[t])

        # Compute combined per tap
        combined = torch.zeros_like(x)
        for t in range(self.k_elems):
            combined[:, t] = SymPyFunction.apply(x_enc[:, t], self.op_funcs[t], w_enc[t])

        output_flat = combined.sum(dim=1)
        output = output_flat.view(tuple(out_shape.tolist()))
        return output
    ...
    def update_histories(self):
        ...
        if self.memory_func:
            ...
            m_tensor = self.memory_state
            args = self._get_temporal_args(w_tensor, g_tensor, m_tensor)
            self.memory_state = SymPyFunction.apply(w_tensor, self.memory_func, *args[1:]).to(self.weights.device)
...
if __name__ == "__main__":
    ...
    layer1 = CustomConvLayer("test.kof", "temporal.tprl")
    layer2 = CustomConvLayer("test.kof", "temporal.tprl")

    in_shape = torch.tensor([30, 20, 10])
    input_data = torch.arange(in_shape.prod().item(), dtype=torch.float32).reshape(tuple(in_shape)) % 17 / 16.0
