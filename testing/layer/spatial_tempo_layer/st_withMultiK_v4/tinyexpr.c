...
static te_expr *new_expr(const int type, const te_expr *parameters[]) {
    ...
    const int base_size = sizeof(te_expr) - sizeof(void*);
    const int closure_size = IS_CLOSURE(type) ? sizeof(void*) : 0;
    const int size = base_size + psize + closure_size;
    // Ensure minimum size to hold te_expr structure
    const int min_size = sizeof(te_expr);
    te_expr *ret = malloc(size > min_size ? size : min_size);
    CHECK_NULL(ret);
