...
void temporal_wrapper_update(TemporalWrapper *w, const float *gradients, float loss, const float *output, const int *out_shape, const float *input, const int *in_shape, int ndim) {
    ...
    // Select weights
    w->selected_count = 0;
    if (w->t_formula->testing_formula && w->testing_lhs_expr && w->testing_rhs_expr) {
        // Use custom boolean testing formula
        for (int i = 0; i < w->k_elems; ++i) {
            ...
            float lhs_val = exprset_eval(w->testing_lhs_expr, args);
            float rhs_val = exprset_eval(w->testing_rhs_expr, args);
            if (evaluate_boolean(w->t_formula->testing_formula->op, lhs_val, rhs_val)) {
