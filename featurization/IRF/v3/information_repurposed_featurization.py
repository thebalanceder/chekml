...
def compute_mic(x, y, max_bins=10):
    """Compute Maximal Information Coefficient (MIC), using C implementation if available."""
    if _lib is not None:
        n = len(x)
        x_arr = (ctypes.c_double * n)(*x)
        y_arr = (ctypes.c_double * n)(*y)
        return _lib.compute_mic(x_arr, y_arr, n, max_bins)
    else:
        n = len(x)
...
def compute_copula_measure(x, y):
    """Compute a simplified copula-based dependence measure, using C implementation if available."""
    if _lib is not None:
        n = len(x)
        x_arr = (ctypes.c_double * n)(*x)
        y_arr = (ctypes.c_double * n)(*y)
        return _lib.compute_copula_measure(x_arr, y_arr, n)
    else:
...
def compute_distance_correlation(x, y):
    """Compute distance correlation, using C implementation if available."""
    if _lib is not None:
        n = len(x)
        x_arr = (ctypes.c_double * n)(*x)
        y_arr = (ctypes.c_double * n)(*y)
        return _lib.compute_distance_correlation(x_arr, y_arr, n)
    else:
        n = len(x)
        if n < 2:
            return 0.0
...
def compute_hsic(x, y):
    """Compute Hilbert-Schmidt Independence Criterion (HSIC), using C implementation if available."""
    if _lib is not None:
        n = len(x)
        x_arr = (ctypes.c_double * n)(*x)
        y_arr = (ctypes.c_double * n)(*y)
        return _lib.compute_hsic(x_arr, y_arr, n)
    else:
        n = len(x)
        if n < 2:
            return 0.0
...
def compute_energy_distance_correlation(x, y):
    """Compute energy distance correlation, using C implementation if available."""
    if _lib is not None:
        n = len(x)
        x_arr = (ctypes.c_double * n)(*x)
        y_arr = (ctypes.c_double * n)(*y)
        return _lib.compute_energy_distance_correlation(x_arr, y_arr, n)
    else:
        n = len(x)
        if n < 2:
            return 0.0
...
def process_combination(combo, models, metrics, loss_functions, df, y, imputer, scaler):
    ...
    X = df[combo].values  # Convert to numpy for faster access
    ...
    for model_name, model_instance in models:
        model_instance = copy.deepcopy(model_instance)  # Ensure model is fresh for each process
...
def InformationRepurposedFeaturizer(
    ...
    - n_jobs (int): Number of parallel jobs for feature combinations. Default: -1 (use all cores).
    ...
    feature_mi = {}
    X_full = df.drop(columns=["target"]).values  # Convert to numpy for speed
    ...
    # Parallel processing of feature combinations
