...
def main():
    ...
    datasets = search_ckan_datasets(query=QUERY, page_size=10, max_pages=1)

    if not datasets:
        print("⚠️ No datasets found.")
        return

    # Filter datasets to only those with CSVs and remove already processed
    processed_ids = load_processed_dataset_ids(PROCESSED_DATASETS_FILE)
    filtered_datasets = filter_datasets_with_csv(datasets, processed_ids)

    if not filtered_datasets:
        print("⚠️ No new CSV datasets available for evaluation.")
        return

    # Evaluate datasets with randomly selected models
    query_keywords = QUERY.split()
    ranked_datasets = rank_datasets_parallel(filtered_datasets, query_keywords)
