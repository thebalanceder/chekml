...
void movement_induced_phase(KHO_Optimizer *opt, double w, double Kw_Kgb)
{
    ...
    for (i = 0; i < base->population_size; i++)
    {
        ...
        for (j = 0; j < base->dim; j++)
        {
            ...
            {
                RR[j * base->population_size + n] = base->population[n].position[j] - pos_i[j];
            }
        }
...
void foraging_motion_phase(KHO_Optimizer *opt, double w, double Kf, double Kw_Kgb)
{
    ...
    for (i = 0; i < base->population_size; i++)
    {
        ...
        double Beta_b = 0.0;
        if (sol->best_fitness < fit_i && norm_Rib > 1e-10)
        {
            Beta_b = -(sol->best_fitness - fit_i) * inv_Kw_Kgb / norm_Rib;
        }
...
void physical_diffusion_phase(KHO_Optimizer *opt, int iteration, double Kw_Kgb)
{
    Optimizer *base = opt->base;
    int i, j;
    double scale = DMAX * (1.0 - (double)iteration / base->max_iter);
    double inv_Kw_Kgb = Kw_Kgb > 1e-10 ? 1.0 / Kw_Kgb : 0.0;
...
void enforce_kho_bounds(KHO_Optimizer *opt, double *position, double *best)
{
    ...
    for (j = 0; j < base->dim; j++)
    {
        ...
        if (position[j] < lb)
        {
            double A = rand_double(0.0, 1.0);
            position[j] = A * lb + (1.0 - A) * best[j];
        }
        else if (position[j] > ub)
        {
            double B = rand_double(0.0, 1.0);
...
void KHO_optimize(Optimizer *base, double (*objective_function)(double *))
{
    ...
    for (j = 0; j < base->dim; j++)
    {
        bounds_diff_sum += fabs(bounds[2 * j + 1] - bounds[2 * j]);
    }
    opt.Dt = bounds_diff_sum / (2.0 * base->dim);

    opt.solutions = (KHO_Solution *)malloc(base->population_size * sizeof(KHO_Solution));
    opt.buffers.Rgb = (double *)malloc(base->dim * sizeof(double));
    opt.buffers.RR = (double *)malloc(base->dim * base->population_size * sizeof(double));
    opt.buffers.R = (double *)malloc(base->population_size * sizeof(double));
    opt.buffers.Rf = (double *)malloc(base->dim * sizeof(double));
    opt.buffers.Rib = (double *)malloc(base->dim * sizeof(double));
    opt.buffers.Xf = (double *)malloc(base->dim * sizeof(double));
    opt.buffers.Sf = (double *)malloc(base->dim * sizeof(double));
    opt.buffers.K = (double *)malloc(base->population_size * sizeof(double));
    opt.buffers.Kib = (double *)malloc(base->population_size * sizeof(double));
    ...
    for (opt.current_iter = 0; opt.current_iter < base->max_iter; opt.current_iter++)
    {
        ...
        double w = INERTIA_MIN + INERTIA_MAX * (1.0 - (double)opt.current_iter / base->max_iter);
        double max_K = K[0];
        for (i = 1; i < base->population_size; i++)
        {
            if (K[i] > max_K)
    ...
    free(opt.solutions);
    free(opt.buffers.Rgb);
    free(opt.buffers.RR);
    free(opt.buffers.R);
    free(opt.buffers.Rf);
    free(opt.buffers.Rib);
    free(opt.buffers.Xf);
    free(opt.buffers.Sf);
    free(opt.buffers.K);
    free(opt.buffers.Kib);
    free(opt.history);
}
