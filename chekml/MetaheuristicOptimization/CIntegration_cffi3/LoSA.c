...
void LoSA_init_cl(LoSACLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->program = (char *)malloc(source_size + 1);
    if (!source_str) {
...
void LoSA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU LoSA optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("LoSA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open LoSA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: LoSA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read LoSA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: LoSA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    LoSACLContext cl_ctx = {0};
    LoSA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    LoSAContext ctx = {0};
    LoSA_init_context(&ctx, opt, &cl_ctx);
    ...
    int max_evals = opt->max_iter > 0 ? opt->max_iter * pop_size : LOSA_MAX_ITERATIONS * pop_size;
    ...
    while (func_count < max_evals) {
        ...
        cl_float step_size = LOSA_STEP_SIZE;
    ...
    LoSA_cleanup_context(&ctx, &cl_ctx);
    LoSA_cleanup_cl(&cl_ctx);
