...
void SCA_init_cl(SCACLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    char *ctx_source = preprocess_kernel_source(kernel_source);
    cl_ctx->context = opt->context;
    cl_ctx->device = opt->device;
    cl_ctx->program = clCreateProgramWithSource(cl_ctx->context, 1, (const char **)&ctx_source, NULL, &err);
    ...
    if (err != CL_SUCCESS) {
        ...
        if (program_log) {
            clGetProgramBuildInfo(cl_ctx->program, cl_ctx->device, CL_PROGRAM_BUILD_LOG, log_size, program_log, NULL);
...
void SCA_cleanup_cl(SCACLContext *cl_ctx) {
    if (cl_ctx->update_kernel) {
        clReleaseKernel(cl_ctx->update_kernel);
    }
    if (cl_ctx->init_pop_kernel) {
        clReleaseKernel(cl_ctx->init_pop_kernel);
    }
    if (cl_ctx->program) {
        clReleaseProgram(cl_ctx->program);
    }
    if (cl_ctx->queue && cl_ctx->owns_queue) {
...
void SCA_cleanup_context(SCAContext *ctx, SCACLContext *cl_ctx) {
    if (ctx->random_seeds) {
        clReleaseMemObject(ctx->random_seeds);
    }
    if (ctx->bounds) {
        clReleaseMemObject(ctx->bounds);
    }
    if (ctx->best_position) {
        clReleaseMemObject(ctx->best_position);
    }
    if (ctx->fitness) {
        clReleaseMemObject(ctx->fitness);
    }
    if (ctx->population) {
        clReleaseMemObject(ctx->population);
    }
}
void SCA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Sine-Cosine Algorithm optimization...\n");
    if (!opt) {
        fprintf(stderr, "Error: Invalid Optimizer structure or null pointers\n");
        exit(1);
    }
    ...
    if (!opt->population_buffer) {
        fprintf(stderr, "Error: Invalid Optimizer structure or null pointers\n");
        exit(1);
    }
    if (!opt->fitness_buffer) {
        fprintf(stderr, "Error: Invalid Optimizer structure or null pointers\n");
        exit(1);
    }
    if (!opt->bounds) {
    ...
    FILE *fp = fopen("SCA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open SCA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: SCA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read SCA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: SCA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }
    SCACLContext cl_ctx = {0};
    SCA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);
    SCAContext ctx = {0};
    SCA_init_context(&ctx, opt, &cl_ctx);
    int pop_size = opt->population_size;
    int dim = opt->dim;
    int func_count = 0;
    int iteration = 0;
    ...
    const float r1_decrement = SCA_A / (float)opt->max_iter;
    while (func_count < max_evals) {
        iteration++;
        printf("Iteration %d\n", iteration);
        float r1_factor = SCA_A - iteration * r1_decrement;
        ...
        clSetKernelArg(cl_ctx.update_kernel, 5, sizeof(cl_float), &r1_factor);
...
printf("%f]\n", opt->best_solution.position[dim - 1]);
...
SCA_cleanup_context(&ctx, &cl_ctx);
SCA_cleanup_cl(&cl_ctx);
