...
static const char* gls_kernel_source =
...
"__kernel void perturb_population(\n"
"    __global float* population,\n"
"    __global const float* bounds,\n"
"    const int dim,\n"
"    const int population_size,\n"
...
"        barrier(CLK_LOCAL_MEM_FENCE);\n"
"        for (int d = 0; d < dim; d++) {\n"
"            float perturb = GLS_PERTURBATION * (2.0f * lcg_rand_float(&local_seed, 0.0f, 1.0f) - 1.0f);\n"
"            population[id * dim + d] += perturb;\n"
"            float min = local_bounds[2 * d];\n"
"            float max = local_bounds[2 * d + 1];\n"
...
"    }\n"
"}\n"
// Update penalties and feature indicators
"__kernel void update_penalties(\n"
...
"            float feature_indicator = pos > 0.5f ? 1.0f : 0.0f;\n"
"            float utility = feature_indicator * local_bounds[2 * d] / (1.0f + penalties[id * dim + d]);\n"
"            if (utility > 0.5f) {\n"
"                penalties[id * dim + d] += 1.0f;\n"
"            }\n"
"        }\n"
"    }\n"
"}\n"
...
void GLS_optimize(Optimizer* opt, ObjectiveFunction objective_function) {
    ...
    cl_kernel init_kernel = NULL, perturb_kernel = NULL, penalty_kernel = NULL, best_kernel = NULL;
    cl_mem bounds_buffer = NULL, population_buffer = NULL, fitness_buffer = NULL;
    cl_mem penalties_buffer = NULL, best_idx_buffer = NULL, best_position_buffer = NULL;
    float* bounds_float = NULL, *population = NULL, *fitness = NULL, *penalties = NULL, *best_position = NULL;
    ...
    const int population_size = GLS_POPULATION_SIZE;
    ...
    // Create program
    program = clCreateProgramWithSource(context, 1, &gls_kernel_source, NULL, &err);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Error creating GLS program: %d\n", err);
    ...
    if (err != CL_SUCCESS) {
        ...
        if (log) {
            ...
            fprintf(stderr, "Error building GLS program: %d\nBuild log:\n%s\n", err, log);
            free(log);
        }
    ...
    perturb_kernel = clCreateKernel(program, "perturb_population", &err);
    penalty_kernel = clCreateKernel(program, "update_penalties", &err);
    best_kernel = clCreateKernel(program, "find_best", &err);
    if (err != CL_SUCCESS || !init_kernel || !perturb_kernel || !penalty_kernel || !best_kernel) {
        fprintf(stderr, "Error creating GLS kernels: %d\n", err);
        goto cleanup;
    }
    ...
    size_t local_work_size = max_work_group_size < GLS_LOCAL_WORK_SIZE ? max_work_group_size : GLS_LOCAL_WORK_SIZE;
    ...
    penalties = (float*)malloc(population_size * dim * sizeof(float));
    ...
    if (!bounds_float || !population || !fitness || !penalties || !best_position || !best_idx_array || !temp_position) {
    ...
    for (int i = 0; i < population_size * dim; i++) {
        penalties[i] = 0.0f;
    }
    ...
    penalties_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, population_size * dim * sizeof(float), NULL, &err);
    ...
    if (err != CL_SUCCESS || !bounds_buffer || !population_buffer || !fitness_buffer || 
        !penalties_buffer || !best_idx_buffer || !best_position_buffer) {
        fprintf(stderr, "Error creating GLS buffers: %d\n", err);
        goto cleanup;
    }

    // Write bounds and initial penalties
    err = clEnqueueWriteBuffer(queue, bounds_buffer, CL_TRUE, 0, 2 * dim * sizeof(float), bounds_float, 0, NULL, NULL);
    err |= clEnqueueWriteBuffer(queue, penalties_buffer, CL_TRUE, 0, population_size * dim * sizeof(float), penalties, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueReadBuffer(queue, penalties_buffer, CL_TRUE, 0, 
                              population_size * dim * sizeof(float), penalties, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Error reading penalties buffer: %d\n", err);
        goto cleanup;
    }
    for (int i = 0; i < population_size; i++) {
        ...
        double g = objective_function(temp_position);
        float penalty_term = 0.0f;
        for (int d = 0; d < dim; d++) {
            penalty_term += penalties[i * dim + d] * population[i * dim + d];
        }
        fitness[i] = (float)(g + GLS_LAMBDA * penalty_term);
    }
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        // Perturb population
        err = clSetKernelArg(perturb_kernel, 0, sizeof(cl_mem), &population_buffer);
        err |= clSetKernelArg(perturb_kernel, 1, sizeof(cl_mem), &bounds_buffer);
        err |= clSetKernelArg(perturb_kernel, 2, sizeof(int), &dim);
        err |= clSetKernelArg(perturb_kernel, 3, sizeof(int), &population_size);
        err |= clSetKernelArg(perturb_kernel, 4, sizeof(uint), &seed);
        err |= clSetKernelArg(perturb_kernel, 5, 2 * dim * sizeof(float), NULL);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error setting perturb kernel args: %d\n", err);
            goto cleanup;
        }
        err = clEnqueueNDRangeKernel(queue, perturb_kernel, 1, NULL, &global_work_size, &local_work_size, 0, NULL, &events[0]);
        if (err != CL_SUCCESS) {
        ...
        // Update penalties
        err = clSetKernelArg(penalty_kernel, 0, sizeof(cl_mem), &population_buffer);
        err |= clSetKernelArg(penalty_kernel, 1, sizeof(cl_mem), &penalties_buffer);
        err |= clSetKernelArg(penalty_kernel, 2, sizeof(cl_mem), &bounds_buffer);
        err |= clSetKernelArg(penalty_kernel, 3, sizeof(int), &dim);
        err |= clSetKernelArg(penalty_kernel, 4, sizeof(int), &population_size);
        err |= clSetKernelArg(penalty_kernel, 5, 2 * dim * sizeof(float), NULL);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error setting penalty kernel args: %d\n", err);
            goto cleanup;
        }
        err = clEnqueueNDRangeKernel(queue, penalty_kernel, 1, NULL, &global_work_size, &local_work_size, 0, NULL, &events[1]);
        if (err != CL_SUCCESS) {
            fprintf(stderr, "Error enqueuing penalty kernel: %d\n", err);
            goto cleanup;
        }
        ...
        double perturb_time = 0, penalty_time = 0, best_time = 0;
        if (events[0]) {
            ...
            perturb_time = (time_end - time_start) / 1e6;
        }
        if (events[1]) {
            ...
            penalty_time = (time_end - time_start) / 1e6;
        }
        if (events[2]) {
        ...
        printf("GLS|%5d -----> %9.16f | Total: %.3f ms | Perturb: %.3f ms | Penalty: %.3f ms | Best: %.3f ms\n", 
               iter + 1, opt->best_solution.fitness, end_time - start_time, perturb_time, penalty_time, best_time);
    }
...
cleanup:
    ...
    if (penalties_buffer) clReleaseMemObject(penalties_buffer);
    ...
    if (penalty_kernel) clReleaseKernel(penalty_kernel);
