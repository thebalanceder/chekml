...
void TLBO_init_cl(TLBOCLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    // Create kernels
    cl_ctx->init_population_kernel = clCreateKernel(cl_ctx->program, "initialize_population", &err);
    check_cl_error(err, "clCreateKernel initialize_population");
    cl_ctx->compute_mean_kernel = clCreateKernel(cl_ctx->program, "compute_mean", &err);
    check_cl_error(err, "clCreateKernel compute_mean");
    cl_ctx->teacher_phase_kernel = clCreateKernel(cl_ctx->program, "teacher_phase", &err);
    check_cl_error(err, "clCreateKernel teacher_phase");
    cl_ctx->learner_phase_kernel = clCreateKernel(cl_ctx->program, "learner_phase", &err);
    check_cl_error(err, "clCreateKernel learner_phase");
}

void TLBO_cleanup_cl(TLBOCLContext *cl_ctx) {
    if (cl_ctx->learner_phase_kernel) clReleaseKernel(cl_ctx->learner_phase_kernel);
    if (cl_ctx->teacher_phase_kernel) clReleaseKernel(cl_ctx->teacher_phase_kernel);
    if (cl_ctx->compute_mean_kernel) clReleaseKernel(cl_ctx->compute_mean_kernel);
    if (cl_ctx->init_population_kernel) clReleaseKernel(cl_ctx->init_population_kernel);
...
void TLBO_init_context(TLBOContext *ctx, Optimizer *opt, TLBOCLContext *cl_ctx) {
    ...
    ctx->partners = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, population_size * sizeof(cl_int), NULL, &err);
    check_cl_error(err, "clCreateBuffer partners");
    ctx->mean_student = clCreateBuffer(cl_ctx->context, CL_MEM_READ_WRITE, dim * sizeof(cl_float), NULL, &err);
    check_cl_error(err, "clCreateBuffer mean_student");
    ...
    // Initialize partners
    cl_int *partners = (cl_int *)malloc(population_size * sizeof(cl_int));
    for (int i = 0; i < population_size; i++) partners[i] = i;
    err = clEnqueueWriteBuffer(cl_ctx->queue, ctx->partners, CL_TRUE, 0, population_size * sizeof(cl_int), partners, 0, NULL, NULL);
    check_cl_error(err, "clEnqueueWriteBuffer partners");
    free(partners);
}

void TLBO_cleanup_context(TLBOContext *ctx, TLBOCLContext *cl_ctx) {
    if (ctx->mean_student) clReleaseMemObject(ctx->mean_student);
    if (ctx->partners) clReleaseMemObject(ctx->partners);
...
void TLBO_optimize(Optimizer *opt, ObjectiveFunction objective_function) {
    printf("Starting GPU TLBO optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("TLBO.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open TLBO.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: TLBO.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read TLBO.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: TLBO.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    TLBOCLContext cl_ctx = {0};
    TLBO_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    TLBOContext ctx = {0};
    TLBO_init_context(&ctx, opt, &cl_ctx);
    ...
    int max_evals = opt->max_iter > 0 ? opt->max_iter * population_size * 2 : TLBO_MAX_EVALS_DEFAULT;
    printf("Maximum function evaluations: %d\n", max_evals);

    // Profiling data
    typedef struct {
        double init_population_time;
        double compute_mean_time;
        double teacher_phase_time;
        double learner_phase_time;
        double read_time;
        double write_time;
        int count;
    } ProfilingData;
    ...
    cl_event init_event;
    clSetKernelArg(cl_ctx.init_population_kernel, 0, sizeof(cl_mem), &ctx.positions);
    clSetKernelArg(cl_ctx.init_population_kernel, 1, sizeof(cl_mem), &ctx.bounds);
    clSetKernelArg(cl_ctx.init_population_kernel, 2, sizeof(cl_mem), &ctx.random_seeds);
    clSetKernelArg(cl_ctx.init_population_kernel, 3, sizeof(cl_int), &dim);
    clSetKernelArg(cl_ctx.init_population_kernel, 4, sizeof(cl_int), &population_size);
    size_t global_work_size = population_size;
    err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.init_population_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &init_event);
    check_cl_error(err, "clEnqueueNDRangeKernel init_population");
    ...
    if (init_event) {
        ...
        if (err == CL_SUCCESS) {
            ...
            if (err == CL_SUCCESS) {
                prof.init_population_time += (end - start) / 1e6;
            }
        }
    ...
    while (func_count < max_evals) {
        ...
        err = clEnqueueWriteBuffer(cl_ctx.queue, ctx.partners, CL_TRUE, 0, population_size * sizeof(cl_int), partners, 0, NULL, &write_event);
        check_cl_error(err, "clEnqueueWriteBuffer partners");
        free(partners);
        ...
        cl_event mean_event;
        clSetKernelArg(cl_ctx.compute_mean_kernel, 0, sizeof(cl_mem), &ctx.positions);
        clSetKernelArg(cl_ctx.compute_mean_kernel, 1, sizeof(cl_mem), &ctx.mean_student);
        clSetKernelArg(cl_ctx.compute_mean_kernel, 2, sizeof(cl_int), &dim);
        clSetKernelArg(cl_ctx.compute_mean_kernel, 3, sizeof(cl_int), &population_size);
        size_t mean_work_size = dim;
        err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.compute_mean_kernel, 1, NULL, &mean_work_size, NULL, 0, NULL, &mean_event);
        check_cl_error(err, "clEnqueueNDRangeKernel compute_mean");
        clFinish(cl_ctx.queue);

        if (mean_event) {
            err = clGetEventProfilingInfo(mean_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            if (err == CL_SUCCESS) {
                err = clGetEventProfilingInfo(mean_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                if (err == CL_SUCCESS) {
                    prof.compute_mean_time += (end - start) / 1e6;
                }
            }
            clReleaseEvent(mean_event);
        }

        // Teacher phase
        cl_event teacher_event;
        clSetKernelArg(cl_ctx.teacher_phase_kernel, 0, sizeof(cl_mem), &ctx.positions);
        clSetKernelArg(cl_ctx.teacher_phase_kernel, 1, sizeof(cl_mem), &ctx.costs);
        clSetKernelArg(cl_ctx.teacher_phase_kernel, 2, sizeof(cl_mem), &ctx.bounds);
        clSetKernelArg(cl_ctx.teacher_phase_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
        clSetKernelArg(cl_ctx.teacher_phase_kernel, 4, sizeof(cl_mem), &ctx.mean_student);
        clSetKernelArg(cl_ctx.teacher_phase_kernel, 5, sizeof(cl_int), &best_idx);
        clSetKernelArg(cl_ctx.teacher_phase_kernel, 6, sizeof(cl_int), &dim);
        clSetKernelArg(cl_ctx.teacher_phase_kernel, 7, sizeof(cl_int), &population_size);
        err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.teacher_phase_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &teacher_event);
        check_cl_error(err, "clEnqueueNDRangeKernel teacher_phase");
        clFinish(cl_ctx.queue);

        if (teacher_event) {
            err = clGetEventProfilingInfo(teacher_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            if (err == CL_SUCCESS) {
                err = clGetEventProfilingInfo(teacher_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                if (err == CL_SUCCESS) {
                    prof.teacher_phase_time += (end - start) / 1e6;
                }
            }
            clReleaseEvent(teacher_event);
        }

        // Evaluate updated population (teacher phase)
        ...
        for (int i = 0; i < population_size; i++) {
            double *pos = (double *)malloc(dim * sizeof(double));
            for (int j = 0; j < dim; j++) pos[j] = positions[i * dim + j];
            float new_cost = (float)objective_function(pos);
            if (new_cost < costs[i]) {
                costs[i] = new_cost;
            }
            free(pos);
        ...
        // Learner phase
        cl_event learner_event;
        clSetKernelArg(cl_ctx.learner_phase_kernel, 0, sizeof(cl_mem), &ctx.positions);
        clSetKernelArg(cl_ctx.learner_phase_kernel, 1, sizeof(cl_mem), &ctx.costs);
        clSetKernelArg(cl_ctx.learner_phase_kernel, 2, sizeof(cl_mem), &ctx.bounds);
        clSetKernelArg(cl_ctx.learner_phase_kernel, 3, sizeof(cl_mem), &ctx.random_seeds);
        clSetKernelArg(cl_ctx.learner_phase_kernel, 4, sizeof(cl_mem), &ctx.partners);
        clSetKernelArg(cl_ctx.learner_phase_kernel, 5, sizeof(cl_int), &dim);
        clSetKernelArg(cl_ctx.learner_phase_kernel, 6, sizeof(cl_int), &population_size);
        err = clEnqueueNDRangeKernel(cl_ctx.queue, cl_ctx.learner_phase_kernel, 1, NULL, &global_work_size, NULL, 0, NULL, &learner_event);
        check_cl_error(err, "clEnqueueNDRangeKernel learner_phase");
        clFinish(cl_ctx.queue);

        if (learner_event) {
            err = clGetEventProfilingInfo(learner_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
            if (err == CL_SUCCESS) {
                err = clGetEventProfilingInfo(learner_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
                if (err == CL_SUCCESS) {
                    prof.learner_phase_time += (end - start) / 1e6;
                }
            }
            clReleaseEvent(learner_event);
        }

        // Evaluate updated population (learner phase)
        positions = (float *)malloc(population_size * dim * sizeof(float));
        err = clEnqueueReadBuffer(cl_ctx.queue, ctx.positions, CL_TRUE, 0, population_size * dim * sizeof(cl_float), positions, 0, NULL, &read_event);
        check_cl_error(err, "clEnqueueReadBuffer positions");
        for (int i = 0; i < population_size; i++) {
        ...
        if (new_best < ctx.best_cost) {
            ...
            if (fabs(prev_best_cost - new_best) < TLBO_CONVERGENCE_TOL) {
                stagnation_count++;
            } else {
                stagnation_count = 0;
            }
            prev_best_cost = new_best;
            if (stagnation_count >= TLBO_STAGNATION_THRESHOLD) {
    ...
    printf("Initialize Population: %.3f\n", prof.init_population_time);
    printf("Compute Mean: %.3f\n", prof.compute_mean_time);
    printf("Teacher Phase: %.3f\n", prof.teacher_phase_time);
    printf("Learner Phase: %.3f\n", prof.learner_phase_time);
    printf("Data Reads: %.3f\n", prof.read_time);
    printf("Data Writes: %.3f\n", prof.write_time);
    if (prof.count > 0) {
        printf("Average per Iteration (ms):\n");
        printf("Compute Mean: %.3f\n", prof.compute_mean_time / prof.count);
        printf("Teacher Phase: %.3f\n", prof.teacher_phase_time / prof.count);
        printf("Learner Phase: %.3f\n", prof.learner_phase_time / prof.count);
    ...
    TLBO_cleanup_context(&ctx, &cl_ctx);
    TLBO_cleanup_cl(&cl_ctx);
