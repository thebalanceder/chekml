...
void FirefA_init_cl(FirefACLContext *cl_ctx, const char *kernel_source, Optimizer *opt) {
    ...
    cl_ctx->update_kernel = clCreateKernel(cl_ctx->program, "update_fireflies", &err);
    check_cl_error(err, "clCreateKernel update_fireflies");
}
...
void FirefA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    printf("Starting GPU Firefly Algorithm optimization...\n");
    ...
    // Read kernel source
    FILE *fp = fopen("FirefA.cl", "r");
    if (!fp) {
        fprintf(stderr, "Failed to open FirefA.cl\n");
        exit(1);
    }
    fseek(fp, 0, SEEK_END);
    size_t source_size = ftell(fp);
    if (source_size == 0) {
        fprintf(stderr, "Error: FirefA.cl is empty\n");
        fclose(fp);
        exit(1);
    }
    rewind(fp);
    ...
    if (read_size != source_size) {
        fprintf(stderr, "Error: Failed to read FirefA.cl completely (%zu of %zu bytes)\n", read_size, source_size);
        free(source_str);
        exit(1);
    }
    ...
    if (!valid_content) {
        fprintf(stderr, "Error: FirefA.cl contains only whitespace or invalid characters\n");
        free(source_str);
        exit(1);
    }

    // Initialize OpenCL context
    FirefACLContext cl_ctx = {0};
    FirefA_init_cl(&cl_ctx, source_str, opt);
    free(source_str);

    // Initialize algorithm context
    FirefAContext ctx = {0};
    FirefA_init_context(&ctx, opt, &cl_ctx);
    ...
    while (func_count < max_evals) {
        ...
        cl_float alpha = FA_ALPHA * powf(FA_THETA, iteration);
        ...
        clSetKernelArg(cl_ctx.update_kernel, 4, sizeof(cl_float), &alpha);
        ...
        check_cl_error(err, "clEnqueueNDRangeKernel update_fireflies");
        clFinish(cl_ctx.queue);
        ...
        printf("Profiling (ms): Update Fireflies = %.3f\n", update_time);
    }

    // Print profiling summary
    printf("\nProfiling Summary (Total ms):\n");
    printf("Initialize Population: %.3f\n", prof.init_pop_time);
    printf("Update Fireflies: %.3f\n", prof.update_time);
    ...
    FirefA_cleanup_context(&ctx, &cl_ctx);
    FirefA_cleanup_cl(&cl_ctx);
