...
double rand_double_galso(double min, double max) {
...
void dhlo_initialize_population(Optimizer *opt, int num_leaders, Solution **leaders, Solution **pbest) {
    if (!opt || !leaders || !pbest || !opt->bounds || !opt->population) return;
    ...
    for (int i = 0; i < opt->population_size; i++) {
        for (int j = 0; j < opt->dim; j++) {
            ...
            opt->population[i].position[j] = lb + (ub - lb) * rand_double_galso(0.0, 1.0);
        }
    ...
    *leaders = (Solution *)malloc(num_leaders * sizeof(Solution));
    ...
    for (int i = 0; i < num_leaders; i++) {
        ...
        for (int j = 0; j < opt->dim; j++) {
            double lb = opt->bounds[2 * j];
            double ub = opt->bounds[2 * j + 1];
            (*leaders)[i].position[j] = lb + (ub - lb) * rand_double_galso(0.0, 1.0);
        }
...
void dhlo_update_leaders(Optimizer *opt, int iter, double *leader_fitness_history, int num_leaders, Solution *leaders, Solution *pbest) {
    if (!opt || !leader_fitness_history || !leaders || !pbest) return;

    for (int i = 0; i < opt->population_size; i++) {
        double sa_fitness = opt->population[i].fitness;
        if (isinf(sa_fitness) || isnan(sa_fitness)) continue;
        ...
        if (j < num_leaders) {
            leader_fitness_history[iter * DHLO_INITIAL_LEADERS + j] = leaders[j].fitness;
        }
    }
    enforce_bound_constraints(opt);
}
...
void dhlo_adjust_num_leaders(Optimizer *opt, int iter, double *leader_fitness_history, int *num_leaders) {
    if (!opt || !leader_fitness_history || !num_leaders) return;

    int n_L = *num_leaders;
    double tol_iter = opt->max_iter * DHLO_TOLERANCE_PERCENT / 100.0;

    if (DHLO_VARIANT[0] == 'V' && DHLO_VARIANT[1] == '1') {
        double AA = 1.0 - ((double)iter / (opt->max_iter / 2.0));
        if (AA > 0) {
            n_L = (int)round(3.0 + AA * (DHLO_INITIAL_LEADERS - 3));
        } else {
        ...
        n_L = (int)round(1.0 + AA * (DHLO_INITIAL_LEADERS - 1));
    } else if (DHLO_VARIANT[0] == 'V' && DHLO_VARIANT[1] == '3') {
        double AA = -((double)iter * 10.0 / opt->max_iter);
        n_L = (int)round(DHLO_INITIAL_LEADERS * exp(AA) + 1.0);
    } else {  // V4 or fallback
        if (iter >= tol_iter + 1) {
            double curr_fit = leader_fitness_history[iter * DHLO_INITIAL_LEADERS + (n_L - 1)];
            if (curr_fit == INFINITY) {
                n_L--;
            } else {
                int prev_idx = (n_L - 1) + (iter - (int)tol_iter) * DHLO_INITIAL_LEADERS;
                if (prev_idx < DHLO_INITIAL_LEADERS * opt->max_iter) {
    ...
    *num_leaders = n_L < 1 ? 1 : n_L > DHLO_INITIAL_LEADERS ? DHLO_INITIAL_LEADERS : n_L;
}
...
void dhlo_update_positions(Optimizer *opt, int iter, int num_leaders, Solution *leaders, Solution *pbest, double *temp_pos, double *velocity) {
    if (!opt || !leaders || !pbest || !temp_pos || !velocity) return;

    if (POSITION_UPDATE_STRATEGY[0] == 'G' && POSITION_UPDATE_STRATEGY[1] == 'W') {  // GWO
        double a = DHLO_A_MAX - iter * ((DHLO_A_MAX - DHLO_A_MIN) / opt->max_iter);
        for (int i = 0; i < opt->population_size; i++) {
            double *pos = opt->population[i].position;
            for (int j = 0; j < opt->dim; j++) {
                double XX_sum = 0.0;
                for (int k = 0; k < num_leaders; k++) {
                    double r1 = rand_double_galso(0.0, 1.0);
                    double r2 = rand_double_galso(0.0, 1.0);
        ...
        for (int i = 0; i < opt->population_size; i++) {
            ...
            int leader_idx = (int)(rand_double_galso(0.0, num_leaders));
            for (int j = 0; j < opt->dim; j++) {
                double r1 = rand_double_galso(0.0, 1.0);
                double r2 = rand_double_galso(0.0, 1.0);
                vel[j] = DHLO_PSO_INERTIA * vel[j] +
                         DHLO_PSO_COGNITIVE * r1 * (pbest[i].position[j] - pos[j]) +
                         DHLO_PSO_SOCIAL * r2 * (leaders[leader_idx].position[j] - pos[j]);
        ...
        double scale = DHLO_RW_STEP_SCALE * (1.0 - (double)iter / opt->max_iter);
        for (int i = 0; i < opt->population_size; i++) {
            double *pos = opt->population[i].position;
            int leader_idx = (int)(rand_double_galso(0.0, num_leaders));
            for (int j = 0; j < opt->dim; j++) {
                double step = scale * rand_double_galso(-1.0, 1.0);
...
void DHLO_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    ...
    int num_leaders = DHLO_INITIAL_LEADERS;
    double *leader_fitness_history = (double *)malloc(DHLO_INITIAL_LEADERS * opt->max_iter * sizeof(double));
    if (!leader_fitness_history) return;
    ...
    for (int i = 0; i < DHLO_INITIAL_LEADERS * opt->max_iter; i++) {
    ...
    dhlo_initialize_population(opt, num_leaders, &leaders, &pbest);
    if (!leaders || !pbest) {
        free(velocity);
        free(temp_pos);
        free(leader_fitness_history);
    ...
    for (int iter = 0; iter < opt->max_iter; iter++) {
        ...
        dhlo_update_leaders(opt, iter, leader_fitness_history, num_leaders, leaders, pbest);
        dhlo_adjust_num_leaders(opt, iter, leader_fitness_history, &num_leaders);
        dhlo_update_positions(opt, iter, num_leaders, leaders, pbest, temp_pos, velocity);
