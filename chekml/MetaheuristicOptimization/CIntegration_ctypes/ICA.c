...
void create_initial_empires(Optimizer *opt, ICAParams *params, Empire **empires, int *num_empires, ICAOptimizerData *data) {
    ...
    if (opt->population_size < NUM_IMPERIALISTS) {
        printf("Population size (%d) too small for %d imperialists\n", opt->population_size, NUM_IMPERIALISTS);
        exit(1);
    }

    // Adjust number of imperialists if population size is too small
    ...
    if (num_colonies_total < effective_imperialists) {
        effective_imperialists = opt->population_size / 2; // Ensure at least some colonies
    ...
    *empires = (Empire *)calloc(*num_empires, sizeof(Empire));
    ...
    double *costs = (double *)malloc(opt->population_size * sizeof(double));
    int *indices = (int *)malloc(opt->population_size * sizeof(int));
    if (!costs || !indices) {
        printf("Memory allocation failed for costs or indices\n");
        free(costs);
        free(indices);
        free(*empires);
        exit(1);
    ...
    for (int i = 0; i < effective_imperialists; i++) {
        power[i] = max_cost - costs[indices[i]]; // Lower cost = higher power
        power_sum += power[i];
    }
    if (power_sum == 0.0) {
        printf("Power sum is zero, cannot assign colonies\n");
        free(indices);
        free(costs);
        free(*empires);
        exit(1);
    ...
    for (int i = 0; i < effective_imperialists - 1; i++) {
        double proportion = power[i] / power_sum;
        num_colonies[i] = (int)(proportion * num_colonies_total);
    ...
    if (num_colonies[effective_imperialists - 1] < 0) {
        printf("Negative colonies assigned to last empire: %d\n", num_colonies[effective_imperialists - 1]);
        free(indices);
        free(costs);
        free(*empires);
        exit(1);
    ...
    for (int i = 0; i < *num_empires; i++) {
        ...
        if (emp->num_colonies > 0) {
            ...
            for (int j = 0; j < emp->num_colonies; j++) {
                ...
                if (!emp->colonies_position[j]) {
                    printf("Memory allocation failed for colony_position at empire %d, colony %d\n", i, j);
                    for (int k = 0; k < j; k++) free(emp->colonies_position[k]);
                    free(emp->colonies_position);
                    free(emp->colonies_cost);
                    free(emp->imperialist_position);
            ...
            if (!emp->colonies_position || !emp->colonies_cost) {
                printf("Memory allocation failed for empty colonies at empire %d\n", i);
                free(emp->imperialist_position);
                for (int k = 0; k < i; k++) free_empires(*empires, k + 1, opt->dim);
                free(indices);
                free(costs);
        ...
        mean_colony_cost /= emp->num_colonies > 0 ? emp->num_colonies : 1;
...
// Assimilation: Move colonies toward imperialist
void assimilate_colonies(Optimizer *opt, Empire *empire) {
    if (!opt || !empire || empire->num_colonies <= 0 || !empire->colonies_position || !empire->imperialist_position) {
        printf("Invalid arguments or empty empire in assimilate_colonies\n");
        return;
    }

    double *imperialist_pos = empire->imperialist_position;
    double *bounds = opt->bounds;
    for (int i = 0; i < empire->num_colonies; i++) {
        if (!empire->colonies_position[i]) {
            printf("Invalid colony position at index %d\n", i);
            return;
        }
        double *colony_pos = empire->colonies_position[i];
        for (int j = 0; j < opt->dim; j++) {
            double dist = imperialist_pos[j] - colony_pos[j];
            double move = ASSIMILATION_COEFF * rand_double(0, 1) * dist;
            colony_pos[j] += move;
            // Clamp within bounds
...
// Revolution: Randomly perturb some colonies
void revolve_colonies(Optimizer *opt, Empire *empire, double revolution_rate) {
    if (!opt || !empire || empire->num_colonies <= 0 || !empire->colonies_position) {
        printf("Invalid arguments or empty empire in revolve_colonies\n");
        return;
    }

    int num_revolving = (int)(revolution_rate * empire->num_colonies + 0.5);
    if (num_revolving <= 0) return;

    double *bounds = opt->bounds;
    for (int i = 0; i < num_revolving; i++) {
        int idx = (int)(rand_double(0, empire->num_colonies));
        if (idx >= empire->num_colonies) continue;
        if (!empire->colonies_position[idx]) {
            printf("Invalid colony position at index %d\n", idx);
            return;
        }
        double *colony_pos = empire->colonies_position[idx];
        for (int j = 0; j < opt->dim; j++) {
            colony_pos[j] = bounds[2 * j] + rand_double(0, 1) * (opt->bounds[2 * j + 1] - bounds[2 * j]);
        }
    }
}

// Possession: Swap imperialist with best colony if better
void possess_empire(Optimizer *opt, ICAParams *params, Empire *empire, ICAOptimizerData *data) {
    if (!opt || !params || !empire || empire->num_colonies <= 0 || !empire->colonies_position || !empire->colonies_cost) {
        printf("Invalid arguments or empty empire in possess_empire\n");
        return;
    }

    // Update colony costs
    for (int i = 0; i < empire->num_colonies; i++) {
        empire->colonies_cost[i] = params->objective_function(empire->colonies_position[i]);
        data->fes++;
    }

    int best_colony_idx = 0;
    double min_colony_cost = empire->colonies_cost[0];
    for (int i = 1; i < empire->num_colonies; i++) {
        if (empire->colonies_cost[i] < min_colony_cost) {
            min_colony_cost = empire->colonies_cost[i];
            best_colony_idx = i;
        }
    }
    ...
    if (min_colony_cost < empire->imperialist_cost) {
        ...
        empire->imperialist_cost = empire->colonies_cost[best_colony_idx];
...
void unite_similar_empires(Optimizer *opt, Empire **empires, int *num_empires) {
    if (!opt || !empires || !num_empires || *num_empires <= 1) {
        return;
    }
    ...
    while (i < *num_empires - 1) {
        int j = i + 1;
        while (j < *num_empires) {
            if (!(*empires)[i].imperialist_position || !(*empires)[j].imperialist_position) {
                printf("Invalid imperialist positions at empire %d or %d\n", i, j);
                return;
            }
            double distance = 0.0;
            for (int k = 0; k < opt->dim; k++) {
                double diff = (*empires)[i].imperialist_position[k] - (*empires)[j].imperialist_position[k];
                distance += diff * diff;
            }
            ...
            if (distance <= threshold) {
                ...
                if (!new_positions || !new_costs) {
                    printf("Memory allocation failed for new colonies in unite_similar_empires\n");
                    free(new_positions);
                    free(new_costs);
                    return;
                }
                ...
                for (int k = 0; k < better->num_colonies; k++) {
                    new_positions[k] = better->colonies_position[k];
                    new_costs[k] = better->colonies_cost[k];
                }
                new_positions[better->num_colonies] = worse->imperialist_position;
                new_costs[better->num_colonies] = worse->imperialist_cost;
                for (int k = 0; k < worse->num_colonies; k++) {
                    new_positions[better->num_colonies + 1 + k] = worse->colonies_position[k];
                ...
                mean_colony_cost /= better->num_colonies > 0 ? better->num_colonies : 1;
                better->total_cost = better->imperialist_cost + ZETA * mean_colony_cost;

                for (int k = worse_idx; k < *num_empires - 1; k++) {
                    (*empires)[k] = (*empires)[k + 1];
                }
                (*num_empires)--;
                if (*num_empires <= 0) {
                    printf("No empires remaining after uniting\n");
                    return;
                }
                continue;
            }
...
void imperialistic_competition(Optimizer *opt, Empire **empires, int *num_empires) {
    if (!opt || !empires || !num_empires || *num_empires <= 1 || rand_double(0, 1) > 0.11) {
        return;
    }

    double total_costs[NUM_IMPERIALISTS];
    double powers[NUM_IMPERIALISTS];
    double possession_prob[NUM_IMPERIALISTS];
    ...
    for (int i = 0; i < *num_empires; i++) possession_prob[i] = powers[i] / power_sum;

    // Select empire to gain colony
    double max_diff = possession_prob[0] - rand_double(0, 1);
    int selected_idx = 0;
    for (int i = 1; i < *num_empires; i++) {
        double diff = possession_prob[i] - rand_double(0, 1);
        if (diff > max_diff) {
            max_diff = diff;
            selected_idx = i;
        }
    ...
    if (!new_positions || !new_costs) {
        printf("Memory allocation failed for selected empire\n");
        free(new_positions);
        free(new_costs);
        return;
    }
    for (int i = 0; i < selected->num_colonies; i++) {
        new_positions[i] = selected->colonies_position[i];
        new_costs[i] = selected->colonies_cost[i];
    }
    new_positions[selected->num_colonies] = weakest->colonies_position[colony_idx];
    ...
    if (new_weakest_colonies > 0) {
        ...
        if (!new_weakest_positions || !new_weakest_costs) {
            printf("Memory allocation failed for weakest empire\n");
            free(new_positions);
            free(new_costs);
        ...
        for (int i = 0, k = 0; i < weakest->num_colonies; i++) {
            if (i != colony_idx) {
                new_weakest_positions[k] = weakest->colonies_position[i];
    ...
    if (weakest->num_colonies <= 0) {
        ...
        if (!new_positions || !new_costs) {
            printf("Memory allocation failed for empire collapse\n");
            free(new_positions);
            free(new_costs);
            return;
        }
        ...
        new_positions[selected->num_colonies] = weakest->imperialist_position;
        ...
        for (int k = weakest_idx; k < *num_empires - 1; k++) {
            (*empires)[k] = (*empires)[k + 1];
        }
        (*num_empires)--;
        if (*num_empires <= 0) {
            printf("No empires remaining after competition\n");
            return;
        }
    ...
    for (int i = 0; i < *num_empires; i++) {
        ...
        for (int j = 0; j < (*empires)[i].num_colonies; j++) {
            mean_colony_cost += (*empires)[i].colonies_cost[j];
        }
        mean_colony_cost /= (*empires)[i].num_colonies > 0 ? (*empires)[i].num_colonies : 1;
...
void free_empires(Empire *empires, int num_empires, int dim) {
    if (!empires) return;
    for (int i = 0; i < num_empires; i++) {
        if (empires[i].imperialist_position) free(empires[i].imperialist_position);
        if (empires[i].colonies_position && empires[i].num_colonies > 0) {
            for (int j = 0; j < empires[i].num_colonies; j++) {
                if (empires[i].colonies_position[j]) free(empires[i].colonies_position[j]);
            }
            free(empires[i].colonies_position);
        }
        if (empires[i].colonies_cost) free(empires[i].colonies_cost);
    }
    free(empires);
}

// Update best solution
void update_best_solution_ica(Optimizer *opt, Empire *empires, int num_empires, ICAParams *params, ICAOptimizerData *data) {
    for (int i = 0; i < num_empires; i++) {
        double cost = params->objective_function(empires[i].imperialist_position);
        empires[i].imperialist_cost = cost;
        data->fes++;
        if (cost < opt->best_solution.fitness) {
            opt->best_solution.fitness = cost;
            memcpy(opt->best_solution.position, empires[i].imperialist_position, opt->dim * sizeof(double));
        }
        for (int j = 0; j < empires[i].num_colonies; j++) {
            cost = params->objective_function(empires[i].colonies_position[j]);
            empires[i].colonies_cost[j] = cost;
            data->fes++;
            if (cost < opt->best_solution.fitness) {
                opt->best_solution.fitness = cost;
                memcpy(opt->best_solution.position, empires[i].colonies_position[j], opt->dim * sizeof(double));
            }
        }
    }
}
...
void ICA_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    ...
    update_best_solution_ica(opt, empires, num_empires, &params, &data);
    ...
    for (int decade = 0; decade < MAX_DECADES && num_empires > 0; decade++) {
        ...
        for (int i = 0; i < num_empires; i++) {
            ...
            possess_empire(opt, &params, &empires[i], &data);
            // Update total cost
            double mean_colony_cost = 0.0;
            for (int j = 0; j < empires[i].num_colonies; j++) {
                mean_colony_cost += empires[i].colonies_cost[j];
...
// Wrapper to match Optimizer's optimize field signature
