...
from gwo_algorithm import GreyWolfOptimizer
import time
...
# Run GWO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    gwo = GreyWolfOptimizer(objective_function=func, dim=2, bounds=bounds)

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, convergence_curve, alpha_history = gwo.optimize()
    end_time = time.time()
    execution_time = end_time - start_time
    execution_times[name] = execution_time

    # Extract search history from alpha_history
    x_vals = [pos[0] for pos in alpha_history]
    y_vals = [pos[1] for pos in alpha_history]
    ...
    # Plot search path
    plt.plot(x_vals, y_vals, marker="o", markersize=3, color="cyan", linestyle="dashed", alpha=0.7, label="Alpha Search Path")
    plt.scatter(best_solution[0], best_solution[1], color="white", marker="*", s=100, label="Best Solution")

    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"GWO on {name} Function")
    plt.legend()
    plt.savefig(f"GWO_{name}.png")
    plt.close()
