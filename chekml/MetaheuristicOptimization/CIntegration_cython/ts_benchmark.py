...
from ts_algorithm import TabuSearch #dujiangyan_irrigation_optimizer
import time
...
# Run TS on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    diso = TabuSearch(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"TS on {name} Function")
    plt.legend()
    plt.savefig(f"TS_{name}.png")
