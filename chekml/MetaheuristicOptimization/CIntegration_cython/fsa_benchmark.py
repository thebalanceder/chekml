...
from fsa_algorithm import FutureSearchOptimizer  # Import from compiled Cython module
import time
...
bounds = np.array([(-5, 5), (-5, 5)], dtype=np.float64)  # Convert to NumPy array
...
# Run Future Search Algorithm on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    fsa = FutureSearchOptimizer(
        objective_function=func,
        dim=2,
        bounds=bounds,
        population_size=30,
        max_iter=100,
        num_runs=1  # Single run for benchmarking
    )

    # Measure execution time
    start_time = time.time()
    best_solution, best_value, history = fsa.optimize()
    end_time = time.time()
    execution_time = end_time - start_time
    execution_times[name] = execution_time

    # Extract search history (use the best run's history)
    best_run_idx = np.argmin([min(h) for h in history])
    best_history = history[best_run_idx]
    x_vals = [fsa.best_positions[best_run_idx][0] for _ in best_history]
    y_vals = [fsa.best_positions[best_run_idx][1] for _ in best_history]
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Future Search on {name} Function")
    plt.legend()
    plt.savefig(f"FSA_{name}.png")
    plt.close()
