...
import time
from vns_algorithm import VariableNeighborhoodSearch
...
# Run VNS on each benchmark function
for name, func in benchmark_functions.items():
    ...
    best_solution, best_value, history = vns.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"VariableNeighborhoodSearch on {name} Function")
    plt.legend()
    plt.savefig(f"VNS_{name}.png")
