...
from so_algorithm import SpiralOptimizationAlgorithm
import time
...
# Run SO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    so = SpiralOptimizationAlgorithm(
    ...
    best_solution, best_value, history = so.optimize()
    end_time = time.time()
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"SO on {name} Function")
    plt.legend()
    plt.savefig(f"SO_{name}.png")
