...
from cso_algorithm import ChickenSwarmOptimizer
import time
...
# Run CSO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    cso = ChickenSwarmOptimizer(objective_function=func, dim=2, bounds=bounds)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"CSO on {name} Function")
    plt.legend()
    plt.savefig(f"CSO_{name}.png")
    plt.close()
