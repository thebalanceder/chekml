...
from iwo_algorithm import InvasiveWeedOptimization
import time
...
# Run IWO on each benchmark function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    # Initialize optimizer
    iwo = InvasiveWeedOptimization(objective_function=func, dim=2, bounds=bounds)
    ...
    # Extract search history (track best solution per iteration)
    x_vals = []
    y_vals = []
    for i in range(iwo.max_iter):
        # Find best solution up to this iteration
        min_cost_idx = np.argmin([h[2] for h in iwo.history[:i+1]])
        best_sol = iwo.history[min_cost_idx][1]
        x_vals.append(best_sol[0])
        y_vals.append(best_sol[1])
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"IWO on {name} Function")
    plt.legend()
    plt.savefig(f"IWO_{name}.png")
