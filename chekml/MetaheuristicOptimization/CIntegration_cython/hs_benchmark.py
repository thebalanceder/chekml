...
from hs_algorithm import HarmonySearch
import time
...
# Run Harmony Search on each function
for name, func in benchmark_functions.items():
    print(f"\nOptimizing {name} function...")

    hs = HarmonySearch(objective_function=func, dim=2, bounds=bounds,
                       memory_size=100, max_iterations=100, minimize=True)
    ...
    plt.xlabel("X1")
    plt.ylabel("X2")
    plt.title(f"Harmony Search on {name} Function")
    plt.legend()
    plt.savefig(f"HarmonySearch_{name}.png")
