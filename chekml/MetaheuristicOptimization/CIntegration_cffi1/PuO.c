...
void puma_exploration_phase(Optimizer *opt, PumaOptimizerState *state, double (*objective_function)(double *)) {
    ...
    for (int i = 0; i < opt->population_size; i++) {
        int idx = indices[i];
        double *pop_pos = opt->population[idx].position;
        memcpy(temp_position, pop_pos, opt->dim * sizeof(double));
        ...
        if (rand_double(0.0, 1.0) < 0.5) {
            ...
            double *pd = opt->population[d].position;
            double *pe = opt->population[e].position;
            double *pf = opt->population[f].position;
            for (int j = 0; j < opt->dim; j++) {
                y[j] = pa[j] + G * (pa[j] - pb[j]) +
                       G * ((pa[j] - pb[j]) - (pc[j] - pd[j]) + (pc[j] - pd[j]) - (pe[j] - pf[j]));
            }
        }
        ...
        if (new_fitness < opt->population[idx].fitness) {
            memcpy(pop_pos, z, opt->dim * sizeof(double));
...
void puma_exploitation_phase(Optimizer *opt, PumaOptimizerState *state, int iter) {
    ...
    for (int i = 0; i < opt->population_size; i++) {
        ...
        for (int j = 0; j < opt->dim; j++) {
            ...
            S2[j] = F1[j] * (2.0 * rand_double(0.0, 1.0) - 1.0) * pos[j] +
                    F2[j] * (1.0 - (2.0 * rand_double(0.0, 1.0) - 1.0)) * best_pos[j];
            VEC[j] = S1[j] != 0.0 ? S2[j] / S1[j] : S2[j]; // Avoid division by zero
        }
        ...
        if (rand_val <= 0.5) {
            ...
            if (rand_double(0.0, 1.0) > state->q_probability) {
                ...
                double *pr1 = opt->population[r1].position;
                for (int j = 0; j < opt->dim; j++) {
                    pos[j] = best_pos[j] + beta1 * exp(beta2[j]) * (pr1[j] - pos[j]);
                }
            } else {
                for (int j = 0; j < opt->dim; j++) {
                    pos[j] = beta1 * Xatack[j] - best_pos[j];
                }
            }
        } else {
            ...
            for (int j = 0; j < opt->dim; j++) {
                pos[j] = (mbest[j] * pr1[j] - sign * pos[j]) / denom;
            }
        }
    }
    enforce_puma_bounds(opt);
...
void PuO_optimize(Optimizer *opt, double (*objective_function)(double *)) {
    ...
    for (int iter = 3; iter < opt->max_iter; iter++) {
        ...
        state.score_explore = state.mega_explore * (f1_explore + f2_explore) + lmn_explore * (min_pf_f3 * state.f3_explore);
        state.score_exploit = state.mega_exploit * (f1_exploit + f2_exploit) + lmn_exploit * (min_pf_f3 * state.f3_exploit);
    }
