#pragma once

#ifdef __cplusplus
extern "C" {
#endif

#include <immintrin.h> // For AVX intrinsics
#include <math.h>
#include <stdlib.h>
#include <stdio.h>
#include "generaloptimizer.h"

// Optimization parameters
#define CRO_INITIAL_KE 1000.0
#define CRO_MOLE_COLL 0.5
#define CRO_BUFFER_INITIAL 0.0
#define CRO_ALPHA 10.0
#define CRO_BETA 0.2
#define CRO_SPLIT_RATIO 0.5
#define CRO_ELIMINATION_RATIO 0.2
#define CRO_MAX_POPULATION 256 // Fixed max population for extreme speed
#define CRO_RNG_BUFFER_SIZE 1024 // Precomputed random numbers
#define CRO_ALIGN 32 // 32-byte alignment for AVX

// CRO-specific optimizer structure
typedef struct {
    Optimizer *opt;                // Base optimizer
    double *positions;             // Contiguous positions [pop_size * dim]
    double *pes;                   // Potential energies [pop_size]
    double *kes;                   // Kinetic energies [pop_size]
    int population_size;           // Current population size
    double buffer;                 // Buffer energy
    double (*objective_function)(double *); // Objective function
    double *temp_solution1;        // Reusable buffer for new solutions
    double *temp_solution2;        // Second reusable buffer
    double *rng_buffer;            // Precomputed random numbers
    int rng_index;                 // Current index in RNG buffer
    unsigned long long rng_state[2]; // Xorshift128+ state
} CROOptimizer;

// Function prototypes
void cro_init_rng(CROOptimizer *cro);
void cro_fill_rng_buffer(CROOptimizer *cro);
static inline double cro_rand_double(CROOptimizer *cro, double min, double max);
void initialize_molecules(CROOptimizer *cro);
void evaluate_molecules(CROOptimizer *cro);
void on_wall_collision(CROOptimizer *cro, int index);
void decomposition(CROOptimizer *cro, int index);
void inter_molecular_collision(CROOptimizer *cro, int index1, int index2);
void synthesis(CROOptimizer *cro, int index1, int index2);
void elimination_phase_chero(CROOptimizer *cro);
void CheRO_optimize(Optimizer *opt, double (*objective_function)(double *));

#ifdef __cplusplus
}
#endif
