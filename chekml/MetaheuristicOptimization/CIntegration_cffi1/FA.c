...
static inline uint64_t xorshift_next(XorshiftState_Fa *state) {
        ...
        for (int i = 0; i < pop_size; i++) {
            ...
            for (; j <= dim - 4; j += 4) {
                ...
                __m256d rand = _mm256_set_pd(
                    xorshift_double_fa(&thread_rng, 0.0, 1.0),
                    xorshift_double_fa(&thread_rng, 0.0, 1.0),
                    xorshift_double_fa(&thread_rng, 0.0, 1.0),
                    xorshift_double_fa(&thread_rng, 0.0, 1.0)
                );
            ...
            for (; j < dim; j++) {
                ...
                opt->population[i].position[j] = xorshift_double_fa(&thread_rng, min_bound, max_bound);
            }
        }
    }
