...
void initialize_squirrels(Optimizer *opt, ObjectiveFunction objective_function, SSAContext *ctx) {
    ...
    for (i = 0; i < pop_size; i++) {
        ...
        ctx->pulse_flying_rates[i] = rand_double(0.0, 1.0);
    }
    ...
    // Debug: Print initial best
    printf("Initial Best Fitness: %f, Position: [", opt->best_solution.fitness);
    for (j = 0; j < dim; j++) {
        printf("%f%s", opt->best_solution.position[j], j < dim - 1 ? ", " : "]\n");
    }
}
...
void SSA_optimize(Optimizer *opt, ObjectiveFunction objective_function) {
    ...
    for (iter = 0; iter < opt->max_iter; iter++) {
        ...
        printf("Iteration %d: Best Fitness: %f, Position: [", iter + 1, opt->best_solution.fitness);
        for (int j = 0; j < dim; j++) {
            printf("%f%s", opt->best_solution.position[j], j < dim - 1 ? ", " : "]\n");
        }
    }
