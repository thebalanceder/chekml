...
def ask_deepseek_about_geo(csv_preview, model="deepseek-r1:7b", port=11500):
    ...
    prompt = (
        "Analyze the following CSV data preview. Return ONLY a score from 0 to 10 based on whether it contains geo data. "
        "Geo data means columns like latitude, longitude, coordinates, geospatial identifiers, etc.\n\n"
        f"CSV Preview:\n{csv_preview}\n\n"
        "Your score (just a number):"
    )
    ...
    try:
        ...
        print(f"✅ DeepSeek geo score: {score}")
        return score
