...
def search_ckan_datasets(query, page_size=2, max_pages=1):
    all_datasets = []
    start = 0
...
# Model Parallelism
# ----------------------------
def get_model_ports(num_models, starting_port=11500):
    return [starting_port + i for i in range(num_models)]

def evaluate_dataset_with_deepseek(dataset, query_keywords, port):
    # Reuse the existing rank_dataset logic, modified to pass the port
    score = rank_dataset(dataset, query_keywords, deepseek_enabled=True, port=port)
...
def parallel_filter_and_rank_datasets(datasets, query, target_csv_count, num_models=3):
    processed_ids = load_processed_dataset_ids()
    query_keywords = query.split()

    print(f"🔎 Skipping {len(processed_ids)} previously processed datasets.")
    
    # Filter datasets that are new and have CSVs
    datasets_to_evaluate = []
    for dataset in datasets:
        dataset_id = dataset.get("id")

        if dataset_id in processed_ids:
            print(f"⚠️ Skipping already processed dataset: {dataset.get('title', 'Unknown')}")
            continue
    ...
    print(f"🔧 {len(datasets_to_evaluate)} new datasets to evaluate...")

    ports = get_model_ports(num_models)
    print(f"🚀 Running evaluations across {num_models} DeepSeek models on ports {ports}")

    # Distribute datasets across ports using ThreadPoolExecutor
    ranked_csv_datasets = []
    with ThreadPoolExecutor(max_workers=num_models) as executor:
        futures = []
        for idx, dataset in enumerate(datasets_to_evaluate):
            port = ports[idx % num_models]
            futures.append(executor.submit(evaluate_dataset_with_deepseek, dataset, query_keywords, port))

        for future in as_completed(futures):
            result = future.result()
            if result:
                ranked_csv_datasets.append(result)
...
def rank_dataset(dataset, query_keywords, deepseek_enabled=True, port=11500):
    base_score = 0
    ...
    if deepseek_enabled and csv_count > 0:
        ...
        if first_csv_url:
            ...
            if csv_preview:
                deepseek_geo_score = ask_deepseek_about_geo(csv_preview, port=port)

    final_score = (traditional_score * 0.5) + (deepseek_geo_score * 0.5)

    print(f"⭐ Final dataset score (Port {port}): {final_score:.2f} (Traditional: {traditional_score}, DeepSeek: {deepseek_geo_score})")
...
# ----------------------------
def filter_and_rank_datasets(datasets, query, target_csv_count, deepseek_enabled=False):
    processed_ids = load_processed_dataset_ids()
    print(f"🔎 Skipping {len(processed_ids)} previously processed datasets.")
...
def main():
    ...
    ranked_csv_datasets = parallel_filter_and_rank_datasets(
        datasets=datasets,
        query=QUERY,
        target_csv_count=TARGET_CSV_COUNT,
        num_models=2
    )
